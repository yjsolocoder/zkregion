...
skg = SingleKeyBatchGuard(prover.public_key)            # 固定公钥与群参数
sk_binding = skg.bind_once(sk_entries, b"session-1")    # 非空批次，保序留重
assert skg.check(sk_entries, sk_binding)                 # 核摘要、期限并批验后消费
assert not skg.check(sk_entries, sk_binding)             # 二次提交被拒

store = SQLiteReplayStore(tempfile.mktemp(suffix=".db"))
skg_a = SingleKeyBatchGuard(prover.public_key, store=store)
sk_binding = skg_a.bind_once(sk_entries, b"session-3")
skg_b = SingleKeyBatchGuard(prover.public_key, store=store)  # 另一个独立实例
assert skg_b.check(sk_entries, sk_binding)               # 认领、批验并消费（行键域 b"zr/skbr/v1"）
assert not skg_a.check(sk_entries, sk_binding)           # 已消费，二次提交被拒
store.close()
...
- `SingleKeyBatchGuard(public_key, *, prime=DEFAULT_PRIME, generator=DEFAULT_GENERATOR, store=None)` — 同一公钥 `SchnorrBatchEntry` 批次的防重放登记册（线程安全）；构造时固定公钥与群参数并持有固定 `SchnorrVerifier`（`public_key`/`prime`/`generator` 错型含 `bool` 抛 `TypeError`，非法群值抛 `ValueError`）；无参时待用/已消费状态隔离在本实例内存中，传入 `SQLiteReplayStore` 时待用/认领/已消费状态落在该存储中（行键域为 `b"zr/skbr/v1"`），同文件同命名空间的独立实例（含重启后、跨进程）共享状态；摘要为 `SHA-256(F(D) || F(session_id) || S(entries,L) || F(E))`，其中 `D = b"zr/skbr/v1"`、`L(entry)` 逐字节复用以本守卫公钥和群参数构造的 BoundSchnorr 叶原字节（`_bound_schnorr_leaf`），F/U/S/E 逐字节沿用各批守卫
...
- **实例隔离或共享存储**：无参构造时，待用/校验中/已消费状态只存在于单个守卫实例内，不同实例（即便同 id）完全独立、互不阻塞；传入同一 `SQLiteReplayStore`（同文件、同命名空间）的守卫实例则经由短事务与唯一认领 token 共享三种状态，跨实例、跨进程并在重启后保持一致，认领租约过期后只能由等值 `binding` 的 `check` 接管，不同守卫域（`zr/r/v1`、`zr/sbr/v1`、`zr/rbr/v1`、`zr/rgbr/v1`、`zr/rr/v1`、`zr/rg/v1`、`zr/brg/v1`、`zr/brr/v1`、`zr/bsr/v1`、`zr/mccr/v1`、`zr/mcr/v1`、`zr/mir/v1`、`zr/mmr/v1`、`zr/mcbr/v1`）的行互不冲突。

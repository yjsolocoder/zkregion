...
class BoundMerkleMultiBatchReplayGuardTest(unittest.TestCase):
    ...
    def test_bind_nested_type_errors(self):
        ...
        item = entries[0]
        bad_item_cases = [
            dataclasses.replace(item, root="root"),
            dataclasses.replace(item, proof="p"),
            dataclasses.replace(item, entries=list(item.entries)),
            dataclasses.replace(item, entries=((0, b"x", 1),)),
            dataclasses.replace(item, entries=((True, b"x"),)),
            dataclasses.replace(item, entries=((0, "leaf"),)),
            dataclasses.replace(
                item,
                proof=MerkleMultiProof(
                    True, item.proof.indices, item.proof.siblings
                ),
            ),
            dataclasses.replace(
                item,
    ...
    def test_bind_value_errors(self):
        ...
        with self.assertRaises(ValueError):
            guard.bind_once(
                BoundMerkleMultiBatch(batch.entries, 2**64, batch.proof), root, b"t"
            )
        negative_index = MerkleMultiProof(1, (-1,), ())
        with self.assertRaises(ValueError):
            guard.bind_once(
    ...
    def test_delegated_batch_rejection_does_not_consume(self):
        # the outer leaves commit under the outer root, but one item's own
        # claimed leaves do not match its root: the root passes and the
        # unchanged inner batch fails
        items = self._items()
        broken_item = MerkleMultiBatchEntry(
            tuple(
                (index, b"bogus" if index == 2 else leaf)
                for index, leaf in items[0].entries
            ),
            items[0].proof,
            items[0].root,
        )
        batch, root = self.build([broken_item, items[1]])
        guard = self.guard()
        binding = guard.bind_once(batch, root, b"s")
        self.assertFalse(guard.check(batch, root, binding, now=1))
        with self.assertRaises(ValueError):

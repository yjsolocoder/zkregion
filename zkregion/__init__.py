...
def prove_region_batch_bound(
    ...
    The second return value is the outer tree's :func:`merkle_root` of
    the encoded leaves, which is exactly the root the batch verifies
    under: ``verify_region_bound(batch, root)`` returns ``True``.
    Single-item, odd- and even-sized batches and duplicate entries are
    all deterministic and byte for byte compatible with the previous
    manual construction.

    A type preflight over the whole batch — every entry and every nested
    field, including ``bool`` integers and later entries — raises
    :class:`TypeError` before anything is built; an empty batch, a count
    that cannot be ``U``-framed as an unsigned 64-bit integer, or
    :func:`verify_region_batch` returning ``False`` raises
    :class:`ValueError`.
    """
    items = _check_region_batch_entries_types(entries)
    if not items:
        raise ValueError("entries must not be empty")
    if not 0 <= len(items) <= _UINT64_MAX:
        raise ValueError("batch length must be an unsigned 64-bit integer")
    if not verify_region_batch(items):
...
def prove_range_batch_bound(
    ...
    The second return value is the outer tree's :func:`merkle_root` of
    the encoded leaves, which is exactly the root the batch verifies
    under: ``verify_range_bound(batch, root)`` returns ``True``.
    ...
    :class:`TypeError` before anything is built; an empty batch, a count
    that cannot be ``U``-framed as an unsigned 64-bit integer, or
    :func:`verify_range_batch` returning ``False`` raises
    :class:`ValueError`.
    """
    ...
    if not 0 <= len(items) <= _UINT64_MAX:
        raise ValueError("batch length must be an unsigned 64-bit integer")
    if not verify_range_batch(items):
        raise ValueError("entries must pass verify_range_batch")
    ordered = tuple(items)
    leaves = [_bound_range_leaf(entry) for entry in ordered]
    root = merkle_root(leaves)
    proof = prove_multi_inclusion(leaves, tuple(range(len(ordered))))
    batch = BoundRangeBatch(
        entries=ordered,

...
mcer_entry = MerkleConsistencyBatchEntry(
    merkle_root(mcbr_leaves[:3]),
    merkle_root(mcbr_leaves),
    prove_consistency(mcbr_leaves, 3),
)
...
- `MerkleConsistencyEntryReplayGuard(*, store=None)` — 单条一致性批验条目（`MerkleConsistencyBatchEntry`）的防重放登记册（线程安全）；无参时待用/已消费状态隔离在本实例内存中，传入 `SQLiteReplayStore` 时待用/认领/已消费状态落在该存储中（行键域为 `b"zr/mcer/v1"`，该名称同时是摘要域与存储行键域），同文件同命名空间的独立实例（含重启后、跨进程）共享状态；摘要为 `SHA-256(F(D) || F(session_id) || L(entry) || F(E))`，其中 `D = b"zr/mcer/v1"`、`L(entry)` 复用一致性完整批的外层叶原字节（`_bound_consistency_leaf`，即域 `b"zkregion/consistency-bound/v1"` 的既有叶编码：域、`old_root`、`new_root`、十进制 ASCII 的 `old_count`/`new_count` 与各 `nodes` 逐项四字节长度成帧），F/E 逐字节沿用既有单条守卫
  - `bind_once(entry: MerkleConsistencyBatchEntry, session_id: bytes, *, expires_at=None) -> ReplayBinding` — 把单条 `MerkleConsistencyBatchEntry`（`old_root`/`new_root` 为 `bytes`、类型良好的 `MerkleConsistencyProof`，逐层类型规则沿用 `verify_consistency_batch` 的逐项口径）一次性绑定到 `session_id`；空 id、uint64 越界的期限或待用/校验中（存储后端下含过期认领）/已消费 id 重绑（任意状态重绑）抛 `ValueError`，entry、根、证明或嵌套字段错型（含 `bool` 计数）抛 `TypeError`
  - `check(entry, binding, *, now=None) -> bool` — 先原子认领等值待用绑定（存储后端可在认领租约过期后由持等值绑定的检查接管），再重算摘要、核期限，随后按旧根、新根、证明的次序原样委托 `verify_consistency(entry.old_root, entry.new_root, entry.proof)`；只有一致性校验通过才消费 `session_id`，未登记、已消费、绑定不等、摘要不符、过期或一致性不过一律返回 `False` 且撤销认领、复原待用（被拒标识稍后仍可成功一次），委托验证抛出的异常与数据库错误先复原认领再原样透传；并发检查同一标识至多一次成功，某个标识上的耗时校验不会把其他标识的登记或检查全程串行阻塞；存储后端下只有持有当前认领 token 的一方能消费，拒绝或异常时以该 token 把 id 复原为待用（token 已失效则为无操作）

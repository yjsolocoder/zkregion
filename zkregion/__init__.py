...
verify_multi_inclusion / BoundRegionBatch / verify_region_bound.
"""
...
# ---------------------------------------------------------------------------
# Merkle-committed region-proof batches
#
# A complete region batch frozen together with the Merkle proof that commits
# to every entry. Each Merkle leaf starts with the domain separator
# b"zkregion/region-bound/v1", then frames the x and y commitment fields, the
# region bounds, the context and both sub-proofs' t/e/s sequences. Every
# atomic item carries a four-byte unsigned big-endian length prefix and every
# integer is encoded as decimal ASCII (the sign of negative values is
# preserved), so the leaf can be built for any integer field values.
# Verification first checks every leaf against the Merkle root, then runs
# the unchanged region batch verification.
...
def _bound_region_leaf(entry: RegionBatchEntry) -> bytes:
    ...
    y commitment (dataclass field order), the four region bounds and the
    ``context``, then for the x and the y sub-proof each of the ``t``, ``e``
    and ``s`` sequences — every sequence framed as its decimal element count
    followed by the values one by one. Every atomic item is prefixed with
    its four-byte unsigned big-endian length; integers are encoded as
    decimal ASCII (the sign of negative values is preserved).
    """
...
def verify_region_bound(
    ...
    *,
    ...
    unchanged randomness contract: it is called once per structurally valid
    range-proof branch as ``randbelow(prime - 1)``.
    ...
    for position, entry in enumerate(entries):
        ...
        if not isinstance(region, Region):
            raise TypeError(f"entries[{position}] region must be a Region")
        _check_region_fields(region)
        if not isinstance(region_proof, RegionProof):
            raise TypeError(f"entries[{position}] proof must be a RegionProof")
        if not isinstance(region_proof.x_proof, RangeProof):
            raise TypeError(f"entries[{position}] proof x_proof must be a RangeProof")
        if not isinstance(region_proof.y_proof, RangeProof):
            raise TypeError(f"entries[{position}] proof y_proof must be a RangeProof")
        for axis_name, sub_proof in (("x", region_proof.x_proof), ("y", region_proof.y_proof)):
            for field_name in ("t", "e", "s"):
                field = getattr(sub_proof, field_name)
                if not isinstance(field, tuple):
                    raise TypeError(

...
x_wide_commitment, x_wide_blinding = pedersen_commit(40, 0, 255)
y_wide_commitment, y_wide_blinding = pedersen_commit(60, -2048, 2047)
region_wide_proof = prove_region_wide(
    x_wide_commitment, y_wide_commitment, 40, 60,
    x_wide_blinding, y_wide_blinding, wide_region, context=b"session-1",
)
wide_batch = [
    RegionWideBatchEntry(
...
assert not verify_region_wide_batch([])

bound_wide_region, wide_region_root = prove_region_wide_batch_bound(wide_batch)
assert verify_region_wide_batch_bound(bound_wide_region, wide_region_root)
assert not verify_region_wide_batch_bound(bound_wide_region, b"\x00" * 32)
...
# 单条宽区间批验条目的一次性绑定
from zkregion import WideRangeBatchEntry, WideRangeReplayGuard

wide_entry = WideRangeBatchEntry(wide_commitment, wide_proof, b"session-1")
wrr = WideRangeReplayGuard()
wrr_binding = wrr.bind_once(wide_entry, b"session-1")
assert wrr.check(wide_entry, wrr_binding)                  # 认领、重算摘要核期限、验宽区间证明后消费：首次判真
assert not wrr.check(wide_entry, wrr_binding)              # 二次判假
try:
    wrr.bind_once(wide_entry, b"session-1")                # 换绑判假：待用或已消费 id 不能重绑
except ValueError:
    pass

store = SQLiteReplayStore(tempfile.mktemp(suffix=".db"))
wrr_a = WideRangeReplayGuard(store=store)
wrr_binding = wrr_a.bind_once(wide_entry, b"session-3")
wrr_b = WideRangeReplayGuard(store=store)                  # 另一个独立实例
assert wrr_b.check(wide_entry, wrr_binding)                # 认领、验宽区间证明并消费（行键域 b"zr/wrr/v1"）
assert not wrr_a.check(wide_entry, wrr_binding)            # 已消费，二次提交被拒
store.close()

# 宽区间批验条目的一次性绑定
from zkregion import WideRangeBatchReplayGuard

wide_batch = [wide_entry]
wrbr = WideRangeBatchReplayGuard()
wrbr_binding = wrbr.bind_once(wide_batch, b"session-1")
assert wrbr.check(wide_batch, wrbr_binding)                # 认领、重算摘要核期限、批量验宽区间证明后消费：首次判真
assert not wrbr.check(wide_batch, wrbr_binding)            # 二次判假
try:
    wrbr.bind_once(wide_batch, b"session-1")              # 换绑判假：待用或已消费 id 不能重绑
except ValueError:
    pass

store = SQLiteReplayStore(tempfile.mktemp(suffix=".db"))
wrbr_a = WideRangeBatchReplayGuard(store=store)
wrbr_binding = wrbr_a.bind_once(wide_batch, b"session-3")
wrbr_b = WideRangeBatchReplayGuard(store=store)             # 另一个独立实例
assert wrbr_b.check(wide_batch, wrbr_binding)               # 认领、批量验宽区间证明并消费（行键域 b"zr/wrbr/v1"）
assert not wrbr_a.check(wide_batch, wrbr_binding)           # 已消费，二次提交被拒
store.close()
...
# 整批 BoundWideRangeBatch 与外层 bytes 根的一次性绑定（可选 SQLite 后端跨实例共享）
from zkregion import (
    WideRangeBatchEntry,
...
wide_entries = [
    WideRangeBatchEntry(wide_commitment, wide_proof, b"session-1"),
]
wide_bound, wide_root = prove_range_wide_batch_bound(wide_entries)
bwrr = BoundWideRangeReplayGuard()
bwrr_binding = bwrr.bind_once(wide_bound, wide_root, b"session-1")
assert bwrr.check(wide_bound, wide_root, bwrr_binding)       # 认领、验根与内层宽区间批验后消费：首次判真
assert not bwrr.check(wide_bound, wide_root, bwrr_binding)   # 二次判假
try:
    bwrr.bind_once(wide_bound, wide_root, b"session-1")      # 换绑判假：待用或已消费 id 不能重绑
except ValueError:
    pass

store = SQLiteReplayStore(tempfile.mktemp(suffix=".db"))
bwrr_a = BoundWideRangeReplayGuard(store=store)
bwrr_binding = bwrr_a.bind_once(wide_bound, wide_root, b"session-3")
...
- `BoundWideRangeBatch(entries, leaf_count, proof)` — 冻结的宽区间完整批对象；字段依次为 `tuple[WideRangeBatchEntry, ...]`、正的非 `bool` `int`、`MerkleMultiProof`，均可位置构造、按值相等且不可变
- `prove_range_wide_batch_bound(entries, *, randbelow=secrets.randbelow) -> tuple[BoundWideRangeBatch, bytes]` — 顶层构造 Merkle 承诺的规范宽区间证明完整批：`entries` 沿用 `verify_range_wide_batch` 的全批嵌套类型规则（非 `bytes`/`bytearray`/`str` 序列）且必须非空，完整预检后按原顺序转为元组并保留重复项，输入不变；每项外层叶逐字节复用宽区间完整批叶编码（域 `b"zkregion/wrb/v1"`、四字节长度帧、十进制 ASCII 整数与既有字段顺序），Merkle 摘要规则不变；令 `n = len(entries)`，对编码叶按全索引 `tuple(range(n))` 调用 `prove_multi_inclusion` 得到完整多包含证明（`indices` 覆盖每片叶、`siblings` 为空），返回批的 `leaf_count = n`、`proof` 为该证明，第二返回值为编码叶的 `merkle_root`；返回批满足 `verify_range_wide_batch_bound(batch, root) is True`，单项、奇偶批与重复项均确定，重复构造批、根与证明逐字节一致；`randbelow` 原样透传给 `verify_range_wide_batch`；全批或任一嵌套字段错型（含后项错型与 `bool` 计数）抛 `TypeError`，空批、U 成帧批次数越出 uint64 或 `verify_range_wide_batch` 返回 `False`（内层证明无效）抛 `ValueError`
...
- `BoundRegionWideBatch(entries, leaf_count, proof)` — 冻结的宽区间二维区域完整批对象；字段依次为 `tuple[RegionWideBatchEntry, ...]`、正的非 `bool` `int`、`MerkleMultiProof`，均可位置构造、按值相等且不可变，构造时不做校验
- `prove_region_wide_batch_bound(entries, *, randbelow=secrets.randbelow) -> tuple[BoundRegionWideBatch, bytes]` — 顶层构造 Merkle 承诺的规范宽区间二维区域完整批：`entries` 沿用 `verify_region_wide_batch` 的全批嵌套类型规则（非 `bytes`/`bytearray`/`str` 序列）且必须非空，完整预检后按原顺序转为元组并保留重复项，输入不变；每项外层叶逐字节复用宽区间二维区域完整批叶编码（域 `b"zkregion/rwb/v1"`、四字节长度帧、十进制 ASCII 整数保留负号与既有字段顺序：两承诺六字段、区域四边界、`context`，再 x 后 y 各子证明的 `commitments`/`challenges`/`responses` 计数成帧），Merkle 摘要规则不变；令 `n = len(entries)`，对编码叶按全索引 `tuple(range(n))` 调用 `prove_multi_inclusion` 得到完整多包含证明（`indices` 覆盖每片叶、`siblings` 为空），返回批的 `leaf_count = n`、`proof` 为该证明，第二返回值为编码叶的 `merkle_root`；返回批满足 `verify_region_wide_batch_bound(batch, root) is True`，单项、奇偶批与重复项均确定，保序留重，同一份输入重复构造批、根与证明逐字节一致；`randbelow` 原样透传给 `verify_region_wide_batch`；全批或任一嵌套字段错型（含后项错型与 `bool`）抛 `TypeError`，空批、批次数越出 uint64 或 `verify_region_wide_batch` 返回 `False`（内层证明无效）抛 `ValueError`
...
- `WideRangeBatchReplayGuard(*, store=None)` — 宽区间批验条目（`WideRangeBatchEntry`）批次的防重放登记册（线程安全）；无参构造的状态只在本实例互不影响，传入 `SQLiteReplayStore` 时待用/认领/已消费状态落在该存储中（行键域为 `b"zr/wrbr/v1"`，该名称同时是摘要域与存储行键域，与其余各行键域互不冲突），同文件同命名空间的独立实例、跨进程与重启共享三种状态；摘要为 `SHA-256(F(D) || F(session_id) || S(entries,L) || F(E))`，其中 `D = b"zr/wrbr/v1"`、`L` 复用宽区间完整批的逐项外层叶原字节（`_bound_wide_range_leaf`，即域 `b"zkregion/wrb/v1"` 的既有外层叶编码），F/U/S/E 逐字节沿用各批守卫，批次保序留重、不改写输入
  - `bind_once(entries: Sequence[WideRangeBatchEntry], session_id, *, expires_at=None) -> ReplayBinding` — 把非空批次（沿用 `verify_range_wide_batch` 的序列与嵌套类型规则，承诺六字段非 `bool` 整数、`WideRangeProof` 三层元组整数、`context` 为 `bytes`，保序、留重、不丢项）一次性绑定到 `session_id`，返回既有的冻结 `ReplayBinding`；空批、空 id、uint64 越界（期限或批次长度）或待用/校验中（存储后端下含过期认领）/已消费 id 重绑（任意状态重绑）抛 `ValueError`，任一层字段给错类型（含用布尔值冒充整数）一律抛唯一的 `TypeError`
  - `check(entries, binding, *, now=None, randbelow=secrets.randbelow) -> bool` — 次序固定：先原子认领等值待用绑定（存储后端沿用短事务认领与租约过期接管，被接管的旧 token 无权改状态），再重算摘要、核期限（`now` 缺省取当前 Unix 秒，显式给出时须为非 `bool` uint64，越界抛 `ValueError`），随后把同一随机源原样交给 `verify_range_wide_batch`；只有整批验证通过才消费 `session_id`，未登记、已消费、绑定不等、摘要不符、过期或批验不过一律返回 `False` 并把标识复原待用（被拒标识稍后仍可成功一次），委托异常与数据库错误先复原认领再原样透传；存储后端下只有持有当前认领 token 的一方能消费；并发检查同一标识至多一次成功，不同标识不应被一次耗时校验全程串行阻塞
- `WideRangeReplayGuard(*, store=None)` — 单条宽区间批验条目（`WideRangeBatchEntry`）的防重放登记册（线程安全）；无参构造的状态只在本实例互不影响，传入 `SQLiteReplayStore` 时待用/认领/已消费状态落在该存储中（行键域为 `b"zr/wrr/v1"`，该名称同时是摘要域与存储行键域，与其余各行键域互不冲突），同文件同命名空间的独立实例、跨进程与重启共享三种状态；摘要逐字节复用 `RangeReplayGuard` 等单条守卫的 F/E 与 SHA-256 公式（`SHA-256(F(D) || F(session_id) || L(entry) || F(E))`），仅域改为 `b"zr/wrr/v1"`、`L(entry)` 改为宽区间完整批的逐项外层叶原字节（`_bound_wide_range_leaf`，即域 `b"zkregion/wrb/v1"` 的既有外层叶编码）
  - `bind_once(entry: WideRangeBatchEntry, session_id, *, expires_at=None) -> ReplayBinding` — 登记待用绑定并返回冻结的 `ReplayBinding`；`entry` 的嵌套类型规则与 `verify_range_wide_batch` 的单条条目一致（承诺六字段非 `bool` 整数、`WideRangeProof` 三层元组整数、`context` 为 `bytes`）；空 id、uint64 越界期限或待用/校验中（存储后端下含过期认领）/已消费 id 重绑（任意状态重绑）抛 `ValueError`，任一层字段给错类型（含用 `bool` 冒充整数）一律抛唯一的 `TypeError`
  - `check(entry, binding, *, now=None) -> bool` — 次序固定：先原子认领等值待用绑定（存储后端沿用短事务认领与租约过期接管），再重算摘要、核期限（`now` 缺省取当前 Unix 秒，显式给出时须为非 `bool` uint64，越界抛 `ValueError`），随后按条目字段次序以 `commitment`、`proof`、`context` 原样委托 `verify_range_wide`；只有宽区间验证通过才消费 `session_id`，未登记、已消费、绑定不等、摘要不符、过期或验证不过一律返回 `False` 并把标识复原待用（被拒标识稍后仍可成功一次），委托异常与数据库错误先复原认领再原样透传；存储后端下只有持有当前认领 token 的一方能消费；并发检查同一标识至多一次成功，不同标识不被一次耗时校验全程串行阻塞
...
- `BoundWideRangeReplayGuard(*, store=None)` — Merkle 承诺宽区间证明完整批（`BoundWideRangeBatch`）与外层 bytes 根的一次性防重放登记册（线程安全）；无参构造的状态只在本实例互不影响，传入 `SQLiteReplayStore` 时待用/认领/已消费状态落在该存储中（行键域为 `b"zr/bwrr/v1"`，与其余各行键域互不冲突），同文件同命名空间的独立实例、跨进程与重启共享三种状态；摘要逐字节复用既有 Bound 批守卫的 F/U/S/E 与 SHA-256 公式，仅域改为 `b"zr/bwrr/v1"`、逐项 `L(entry)` 改为宽区间完整批的外层叶原字节（`_bound_wide_range_leaf`，即域 `b"zkregion/wrb/v1"` 的既有外层叶编码），批次保序留重、外层多包含证明的 `leaf_count`、`indices`、`siblings` 三字段均原样纳入摘要，不改写输入
  - `bind_once(batch: BoundWideRangeBatch, root: bytes, session_id: bytes, *, expires_at=None) -> ReplayBinding` — 只登记待用并做全量预检（不跑验证），返回既有的冻结 `ReplayBinding`；`root`、`session_id` 为 bytes 且 id 非空；空 id、uint64 越界的时间或成帧计数（外层 `leaf_count`、`proof.leaf_count`、各索引）或任意状态（待用/校验中（存储后端下含过期认领）/已消费）重绑均抛 `ValueError`，任一层字段给错类型（含用布尔值冒充整数）一律抛唯一的 `TypeError`
  - `check(batch, root, binding, *, now=None, randbelow=secrets.randbelow) -> bool` — 次序固定：先原子认领等值待用绑定（存储后端沿用短事务认领与租约过期接管，被接管的旧 token 无权改状态），再重算摘要并核对期限，随后把同一随机源原样交给 `verify_range_wide_batch_bound`（先验外层根、再内层宽区间批验）；只有验根与内层批验全部通过才消费 `session_id`，未登记、已消费、绑定不等、摘要不符、过期、错根或内层批验不过一律返回 `False` 并把标识复原待用（被拒标识稍后仍可成功一次），委托验证逃出的异常与数据库错误先复原认领再原样透传（随机源自身的异常按既有边界透传）；存储后端下只有持有当前认领 token 的一方能消费，拒绝或异常时以该 token 把 id 复原为待用（token 已失效则为无操作）；`now` 越出 uint64 抛 `ValueError`，错型抛 `TypeError`；宽区间单条、批量与完整批入口及全部旧守卫行为保持不变
...
1. `entries: tuple[WideRangeBatchEntry, ...]` —— 必须是元组（不是列表），每项是 `WideRangeBatchEntry`；
2. `leaf_count: int` —— 正的非 `bool` 整数，且必须同时等于 `len(entries)` 与 `proof.leaf_count`；
3. `proof: MerkleMultiProof` —— 其 `indices` 必须无缺口、无重复、无乱序地恰好覆盖 `0 .. leaf_count - 1`（即等于 `tuple(range(leaf_count))`）。

三字段均可位置构造，对象按值相等且不可变（冻结 dataclass）。空批（`leaf_count < 1` 或 `entries` 为空）、缺项（数量不符）、`leaf_count` 与任一方不一致、索引乱序/重复/有缺口均返回 `False`。

每个条目的 Merkle 叶字节以新域标签 `b"zkregion/wrb/v1"` 开始，再按条目字段顺序展开：承诺六字段（`element`、`lower`、`upper`、`prime`、`generator`、`h`，按数据类字段顺序）与 `context`，随后是宽区间证明的三个字段——`commitments` 先写十进制元素数再逐项写比特承诺，`challenges` 与 `responses` 各自先写十进制对数再按对顺序把每对的两个整数逐项展开。成帧、整数口径（每个原子项前置四字节无符号大端长度、整数编码为十进制 ASCII 且负号保留）与 Merkle 摘要规则逐项沿用区间完整批的同一套约定，不改帧不重排。

`verify_range_wide_batch_bound(batch, root, *, randbelow=secrets.randbelow) -> bool` 的验证分两步、次序固定：

1. 先对**整批**做嵌套类型预检（批对象、元组条目、承诺六字段、`WideRangeProof` 三层元组整数、`context`、`leaf_count` 与 `proof` 字段，错型含用 `bool` 冒充计数与后项错型，一律抛唯一的 `TypeError`）；结构校验（空批、计数与 `proof.leaf_count`/条目数相符、`indices` 无缺口）通过后，才以全部 `(index, leaf)`（`index` 即 0 起的条目位置）调用 `verify_multi_inclusion` 校验 Merkle 根；
2. 根通过后，以**同一个 `randbelow`** 原样调用 `verify_range_wide_batch(entries, randbelow=randbelow)` 验证明，在根校验通过前不消费任何随机数；随机源不可调用或返回非整数抛 `TypeError`、越界抛 `ValueError`，契约与内层批验完全一致。
...
`verify_region_wide_batch(entries, *, randbelow=secrets.randbelow)` 一次验证一批宽区间版二维区域证明，每个条目就是 `verify_region_wide` 的五个入参（`x_commitment`、`y_commitment`、`region`、`proof`、`context`，后者缺省 `b""`）冻结成的不可变数据类 `RegionWideBatchEntry`，构造条目不做任何校验。`entries` 须为非 `bytes`/`bytearray`/`str` 的非空序列：空批返回 `False`，重复条目合法并各自独立取系数；条目彼此独立、可乱序，漏项无法被发现，批次完整性由调用方保证。

验证分两个固定阶段。先对**整批**做嵌套类型预检：序列本身错型、任一条目不是 `RegionWideBatchEntry`、两个承诺不是 `PedersenCommitment` 或其六字段不是非 `bool` 整数、区域不是 `Region` 或四边界不是非 `bool` 整数、证明不是 `RegionWideProof` 或两轴子证明不是 `WideRangeProof` 或其任一层字段不是整数元组（含用 `bool` 冒充整数）、`context` 不是 `bytes`，都抛同一个 `TypeError`；预检走完整批，错型哪怕出现在最后一个条目也照样抛出。预检通过后才逐条核对，任一条无效即短路返回 `False`：两轴承诺的声明区间必须分别恰为区域的 `(min_x, max_x)` 与 `(min_y, max_y)`，随后逐轴沿用宽区间单条验证的同一口径——群参数合法、声明区间恰含 `2**k` 个整数（`1 <= k <= 24`，两轴可不同）、三个证明字段长度均为 `k`、各对长度恰为 2、比特承诺/挑战份额/响应取值在界，以及**逐条**核对的比特承诺按 `2**i` 加权乘积等于该轴承诺值、各比特两个挑战份额之和等于该轴派生 context 下的转录挑战（派生 context 与 `verify_region_wide` 逐字节一致，轴标签、区域四边界、外部上下文与两份承诺均纳入转录）——这两项绑定不参与聚合。

通过逐条核对后，**两轴子证明每个比特的两条 OR 子分支**各恰调用一次 `randbelow(prime - 1)` 得 `r`，取非零系数 `a = r + 1`。跨轴、跨条目的所有子分支按 `(prime, generator, h)` 分组，每组只检查一次聚合等式
...
每个条目的 Merkle 叶字节以新域标签 `b"zkregion/rwb/v1"` 开始，再按条目字段顺序逐项成帧：x 承诺六字段、y 承诺六字段（`element`、`lower`、`upper`、`prime`、`generator`、`h`，按数据类字段顺序）、Region 四边界（`min_x`、`max_x`、`min_y`、`max_y`）与 `context`，随后按 x 后 y 的顺序展开两轴宽区间子证明——每个子证明的 `commitments` 先写十进制元素数再逐项写比特承诺，`challenges` 与 `responses` 各自先写十进制对数再按对顺序把每对的两个整数逐项展开。每个原子项前置四字节无符号大端长度，整数编码为十进制 ASCII 且负号保留，Merkle 摘要规则沿用既有约定，不改帧不重排。

`verify_region_wide_batch_bound(batch, root, *, randbelow=secrets.randbelow) -> bool` 的验证分两步、次序固定：

1. 先对**整批**做嵌套类型预检（批对象、元组条目、两承诺六字段、区域四边界、两轴 `WideRangeProof` 三层元组整数、`context`、`leaf_count` 与 `proof` 字段，错型含用 `bool` 冒充整数与后项错型，一律抛唯一的 `TypeError`）；结构校验（空批、计数与 `proof.leaf_count`/条目数相符、`indices` 无缺口）通过后，才以全部 `(index, leaf)` 调用 `verify_multi_inclusion` 校验外层 Merkle 根；
2. 外层根通过后，以**同一个 `randbelow`** 原样调用 `verify_region_wide_batch(entries, randbelow=randbelow)` 验证明，在根校验通过前不消费任何随机数；随机源不可调用或返回非整数抛 `TypeError`、越界抛 `ValueError`，契约与内层批验完全一致。
...
`WideRangeReplayGuard(*, store=None)` 为单个 :class:`WideRangeBatchEntry` 提供与 `RangeReplayGuard` 同构的一次性会话绑定，复用同一个 `ReplayBinding` 冻结类型，`bind_once` 的入参同为 `(entry, session_id, *, expires_at=None)`、`check` 的入参同为 `(entry, binding, *, now=None)`。无参构造时待用/认领/已消费三种状态只在本实例互不影响，传入 `SQLiteReplayStore` 时三种状态落在该存储中，同文件同命名空间的独立实例、跨进程与重启共享三种状态。绑定摘要逐字节沿用既有单条守卫（`RangeReplayGuard`）的 F/E 成帧与 SHA-256 公式
...
- 域改为独立的 `D = b"zr/wrr/v1"`，该名称同时兼作存储后端的行键域；
- `L(entry)` 逐项叶取宽区间完整批的外层叶原字节（`_bound_wide_range_leaf`），即"Merkle 承诺的宽区间证明完整批验"一节定义、域为 `b"zkregion/wrb/v1"` 的既有外层叶编码；它也是 `verify_range_wide_batch_bound` 验根时重算的同一份叶（**不**再套 `F`）。`E` 的过期编码与既有守卫逐字节相同。

`bind_once` 登记待用绑定并返回冻结的 `ReplayBinding`，只登记与全量预检、不跑验证：`entry` 的逐层嵌套类型规则与 `verify_range_wide_batch` 的单条条目完全一致（承诺六字段、`WideRangeProof` 三层元组整数、`context` 为 `bytes`，含用 `bool` 冒充整数时抛 `TypeError`）；空 `session_id`、`expires_at` 越出 64 位无符号范围、或任意状态（待用/校验中（存储后端下含过期认领）/已消费）重绑均抛 `ValueError`。

`check` 返回布尔结果且次序固定：先原子认领等值的待用绑定，再重算摘要确认条目一致，再核对期限（`now` 缺省取当前 Unix 秒，显式给出时须为非 `bool` uint64，越界抛 `ValueError`），最后按条目字段次序以 `entry.commitment`、`entry.proof`、`entry.context` 原样调用 `verify_range_wide` 验单条宽区间证明。只有宽区间验证通过才消费 `session_id`；未登记（含已消费）、绑定不等、摘要不符、过期或验证不过一律返回 `False` 并把标识复原待用，被拒的标识稍后仍可成功一次；委托验证逃出的异常与数据库错误先复原认领再原样透传。对照看：首次判真、二次判假、换绑判假（任意状态重绑抛 `ValueError`），并发检查同一标识至多一次成功，不同标识不被一次耗时校验全程串行阻塞。

存储后端下 `WideRangeReplayGuard` 沿用 `SQLiteReplayStore` 的短事务认领、租约过期接管与 token 所有权口径，仅行键的域段为 `b"zr/wrr/v1"`（与同一存储上 `RangeReplayGuard` 的 `b"zr/rr/v1"`、`BoundWideRangeReplayGuard` 的 `b"zr/bwrr/v1"` 等行互不冲突）：认领、换发 token、消费与异常复原语义与 `RangeReplayGuard` 逐字节一致，被接管的旧 token 无权改状态，数据库错误原样透传 `sqlite3.Error`。宽区间的单条生成验证、批量验证与完整批入口（`verify_range_wide`、`verify_range_wide_batch`、`verify_range_wide_batch_bound`）及全部旧守卫行为保持不变，不引入任何新的落盘或持久化口径。

### 宽区间批验条目批次的一次性绑定

`WideRangeBatchReplayGuard(*, store=None)` 为整批 :class:`WideRangeBatchEntry` 提供一次性会话绑定，形态与既有批守卫（`RangeBatchReplayGuard`）对齐，复用同一个 `ReplayBinding` 冻结类型；`bind_once` 的入参为 `(entries, session_id, *, expires_at=None)`，`entries` 沿用 :func:`verify_range_wide_batch` 的序列与嵌套类型规则（非 `bytes`/`bytearray`/`str` 的 `Sequence`，每项为 :class:`PedersenCommitment` 六字段、:class:`WideRangeProof` 的 commitments/challenges/responses 三层整数元组与 `context` 嵌套类型正确的 `WideRangeBatchEntry`），批次保序、留重、不丢项、不改写输入；无参构造时待用/认领/已消费三种状态只在本实例互不影响，传入 `SQLiteReplayStore` 时三种状态落在该存储中，同文件同命名空间的独立实例、跨进程与重启共享三种状态。绑定摘要逐字节沿用既有批守卫（`RangeBatchReplayGuard`）的 F/U/S/E 成帧与 SHA-256 公式
...
- 域 `D = b"zr/wrbr/v1"`，该名称同时兼作存储后端的行键域，与既有各行键域互不冲突；
- `F(x)` 是四字节无符号大端长度前缀加 `x`，`U(n)` 是八字节无符号大端整数编码；
- `S(a, f) = F(U(|a|)) || Σ F(f(a_i))`，即先写以 `U` 编码的元素数，再逐项成帧；
- `L(entry)` 取宽区间完整批的逐项外层叶**原字节**（`_bound_wide_range_leaf`：域为 `b"zkregion/wrb/v1"`、按承诺六字段、`context` 与证明的 commitments/challenges/responses 序列以十进制 ASCII（负号保留）逐项成帧的既有外层叶编码），再整体套一层 `F`；
- `E` 的过期编码与既有守卫逐字节相同：无期 `E = b"\x00"`，有期 `E = b"\x01" + uint64be(expires_at)`。

批次顺序与重复项原样纳入摘要，外层叶编码与摘要公式都不改动。

`bind_once` 只登记、不跑验证，返回既有的冻结 `ReplayBinding`。类型错误（序列错型、非 `WideRangeBatchEntry` 项、嵌套字段错型，含用布尔值冒充整数）一律抛唯一的 `TypeError`；登记空批、`session_id` 为空、`expires_at` 越出 uint64、`U` 成帧的批次长度越界，或任意状态（待用/校验中（存储后端下含过期认领）/已消费）重绑均抛 `ValueError`。

`check(entries, binding, *, now=None, randbelow=secrets.randbelow) -> bool` 返回布尔且处理次序固定：先原子认领等值的待用绑定（存储后端下认领租约过期后可被等值 `check` 接管并换发 token），再重算摘要确认提交的整批条目（含顺序与重复项）就是绑定时的批次，再核对期限（有期绑定要求 `now < expires_at`；`now` 缺省取当前 Unix 秒，显式给出时须为非 `bool` uint64，`bool`/非整数抛 `TypeError`、越界抛 `ValueError`），随后把**同一个 `randbelow` 原样**交给宽区间的批量验证入口 `verify_range_wide_batch(entries, randbelow=randbelow)`——每个结构合法的宽区间 OR 子分支恰好抽取一次 `randbelow(prime - 1)` 随机系数，随机源不可调用或返回非整数（含 `bool`）抛 `TypeError`、返回值越界抛 `ValueError`，结构非法或批验失败返回 `False`，均不做包装或改写。只有整批验证通过才消费 `session_id`；未登记（含已消费）、竞争失败、绑定不等、摘要不符、空批或超长批、过期或批验返回 `False` 一律返回 `False` 且撤销认领、把标识**复原为待用**，被拒的标识稍后仍可成功一次；委托验证逃出的异常（含随机源的 `TypeError` / `ValueError`）与数据库错误同样先复原认领再原样透传。存储后端下只有持有当前认领 token 的一方能消费，过期认领被接管后旧 token 既不能消费也不能复原。对照看：首次判真、二次判假、换绑判假（任意状态重绑抛 `ValueError`），并发检查同一标识至多一次成功，不同标识不应被一次耗时校验全程串行阻塞。

存储后端下 `WideRangeBatchReplayGuard` 沿用 `SQLiteReplayStore` 的短事务认领、租约过期接管与 token 所有权口径，仅行键的域段为 `b"zr/wrbr/v1"`（与同一存储上 `WideRangeReplayGuard` 的 `b"zr/wrr/v1"`、`BoundWideRangeReplayGuard` 的 `b"zr/bwrr/v1"`、`RangeBatchReplayGuard` 的 `b"zr/rbr/v1"` 等行互不冲突）：认领、换发 token、消费与异常复原语义与 `RangeBatchReplayGuard` 逐字节一致，被接管的旧 token 无权改状态，数据库错误原样透传 `sqlite3.Error`。宽区间的单条生成验证、批量验证、完整批入口（`verify_range_wide`、`verify_range_wide_batch`、`verify_range_wide_batch_bound`）及全部旧守卫行为保持不变，本次不引入任何新的落盘或持久化口径。
...
### Merkle 承诺宽区间证明完整批的一次性绑定

`BoundWideRangeReplayGuard(*, store=None)` 为整批 :class:`BoundWideRangeBatch` 连同其外层 Merkle `root` 提供一次性会话绑定，形态与既有完整批守卫对齐，复用同一个 `ReplayBinding` 冻结类型，`bind_once` 的入参同为 `(batch, root, session_id, *, expires_at=None)`、`check` 的入参为 `(batch, root, binding, *, now=None, randbelow=secrets.randbelow)`。无参构造时待用/认领/已消费三种状态只在本实例互不影响，传入 `SQLiteReplayStore` 时待用/认领/已消费状态落在该存储中，同文件同命名空间的独立实例、跨进程与重启共享三种状态。绑定摘要逐字节沿用既有 Bound 批守卫（`BoundRangeReplayGuard`）的 F/U/S 成帧与 SHA-256 公式，批次保序留重、不改写输入，仅两处不同：

- 域改为独立的 `D = b"zr/bwrr/v1"`，该名称同时兼作存储后端的行键域；
- 逐项成帧的 `L(entry_i)` 逐项叶取宽区间完整批的外层叶原字节（`_bound_wide_range_leaf`），即"Merkle 承诺的宽区间证明完整批验"一节定义、域为 `b"zkregion/wrb/v1"` 的既有外层叶编码；它也是 `verify_range_wide_batch_bound` 验根时重算的同一份叶。外层多包含证明的 `leaf_count`、`indices`、`siblings` 三字段原样纳入摘要，各自行键域互不冲突。

`bind_once` 只登记待用并做全量预检（不跑验证），返回既有的冻结 `ReplayBinding`：任一层字段给错类型，含用布尔值冒充整数，一律抛出唯一的 `TypeError`；空 `session_id`、时间（`expires_at`）或 U 成帧计数（外层 `leaf_count`、`proof.leaf_count`、各索引）越出 64 位无符号范围、或任意状态（待用/校验中（存储后端下含过期认领）/已消费）重绑均抛 `ValueError`。

`check` 返回布尔结果且次序固定：先原子认领等值的待用绑定，再重算摘要并核对期限（`now` 缺省取当前 Unix 秒，显式给出时须为非 `bool` uint64，越界抛 `ValueError`），再把同一随机源原样交给宽区间完整批的验根入口 `verify_range_wide_batch_bound(batch, root, randbelow=randbelow)`——即先验外层多包含根、再内层宽区间批验，随机源不做包装或改写，其自身异常按既有边界透传。只有验根与内层批验全部通过才消费 `session_id`；未登记、已消费、绑定不等、摘要不符、过期、错根或内层批验不过一律返回 `False` 并把标识复原待用，被拒的标识稍后仍可成功一次；委托验证逃出的异常与数据库错误先复原认领再原样透传。对照看：首次判真、二次判假、换绑判假（任意状态重绑抛 `ValueError`），并发检查同一标识至多一次成功，不同标识不被一次耗时校验全程串行阻塞。

存储后端下 `BoundWideRangeReplayGuard` 沿用 `SQLiteReplayStore` 的短事务认领、租约过期接管与 token 所有权口径，仅行键的域段为 `b"zr/bwrr/v1"`（与同一存储上 `BoundRangeReplayGuard` 的 `b"zr/brr/v1"`、`BoundRegionReplayGuard` 的 `b"zr/brg/v1"` 等行互不冲突）：认领、换发 token、消费与异常复原语义与 `BoundRangeReplayGuard` 逐字节一致，被接管的旧 token 无权改状态，数据库错误原样透传 `sqlite3.Error`。宽区间单条、批量与完整批入口（`verify_range_wide`、`verify_range_wide_batch`、`verify_range_wide_batch_bound`）及全部旧守卫行为保持不变，不引入新的落盘口径。

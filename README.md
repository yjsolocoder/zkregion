...
- `BoundWideRangeReplayGuard(*, store=None)` — Merkle 承诺宽区间完整批（`BoundWideRangeBatch`）与外层 bytes 根的防重放登记册（线程安全）；无参时待用/已消费状态隔离在本实例内存中，传入 `SQLiteReplayStore` 时待用/认领/已消费状态落在该存储中（行键域为 `b"zr/bwrr/v1"`），同文件同命名空间的独立实例（含重启后、跨进程）共享状态；摘要逐字节复用既有 Bound 批守卫的 F/U/S/E 与 SHA-256 公式，仅域改为 `b"zr/bwrr/v1"`、逐项 `L(entry)` 改为宽区间完整批的外层叶原字节（`_bound_wide_range_leaf`，域 `b"zkregion/wrb/v1"` 的既有外层叶编码），批次保序留重、外层 `MerkleMultiProof` 的 `leaf_count`、`indices`、`siblings` 三字段均原样绑定
  - `bind_once(batch: BoundWideRangeBatch, root: bytes, session_id: bytes, *, expires_at=None) -> ReplayBinding` — 只登记待用并做全量预检（不跑验证），返回冻结的既有 `ReplayBinding`；`root`、`session_id` 为 bytes 且 id 非空；空 id、uint64 越界的期限或成帧计数（外层 `leaf_count`、`proof.leaf_count`、各索引）或待用/校验中（存储后端下含过期认领）/已消费 id 重绑抛 `ValueError`，任一层字段错型（含用 `bool` 冒充整数）抛 `TypeError`
  - `check(batch, root, binding, *, now=None, randbelow=secrets.randbelow) -> bool` — 先原子认领等值待用绑定（存储后端可在认领租约过期后由持等值绑定的检查接管），再重算摘要、核期限，随后原样委托 `verify_range_wide_batch_bound` 并透传同一随机源；只有验根与内层批验全部通过才消费 `session_id`，未登记、已消费、绑定不等、摘要不符、过期、错根或内层批验不过一律返回 `False` 且撤销认领、保持待用（被拒标识稍后仍可成功一次），委托验证抛出的异常先复原认领再原样透传；存储后端下只有持有当前认领 token 的一方能消费，`sqlite3.Error` 原样透传

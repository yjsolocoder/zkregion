...
# ---------------------------------------------------------------------------
# Replay protection for single region-contains decision entries
#
# A RegionContainsReplayGuard binds one RegionContainsEntry — the same entry
...
# single-entry guards; only the domain separator and the per-entry leaf
# differ. The binding is single-use: without a store the pending and consumed
# state lives on this guard instance and is never shared between instances,
# while an SQLiteReplayStore keeps pending, claimed and consumed ids in the
...
#   F(x) = four-byte unsigned big-endian length prefix of x, followed by x
#   L(entry) = the BoundRegionContains outer-leaf bytes
#              (_bound_region_contains_leaf), raw (no F framing)
#   E is the same expiry encoding as the other guards:
#     b"\x00" when expires_at is None, b"\x01" + uint64be(expires_at) otherwise


def _check_region_contains_entry(entry: object, name: str = "entry") -> RegionContainsEntry:
    """Validate a RegionContainsEntry and its nested field types."""
    if not isinstance(entry, RegionContainsEntry):
...
class RegionContainsReplayGuard:
    """Single-use replay protection for region-contains decision entries.

    A fresh guard has no registrations. :meth:`bind_once` registers a pending
    :class:`ReplayBinding` that commits a single :class:`RegionContainsEntry`
    — the same entry shape accepted by
    :func:`verify_region_contains_batch`, under the same nested type rules —
    ...
    to :func:`region_contains_committed`, and then marks the id consumed. The
    digest frames the entry under
    ``SHA-256(F(D) || F(session_id) || L(entry) || F(E))`` with domain
    ``b"zr/rcrr/v1"``, where ``L(entry)`` is the existing BoundRegionContains
    outer-leaf raw bytes (the same bytes :func:`_bound_region_contains_leaf`
    builds for :func:`verify_region_contains_bound`); the F framing and the
    E expiry encoding are reused byte for byte from the other replay guards.
    By default both the pending and the consumed state live on this guard
    instance and are never shared between instances; passing an
    :class:`SQLiteReplayStore` as ``store`` instead keeps the state in that
    ...
    As with the other single-entry guards, concurrent checks of the same id
    are decided by an atomic claim taken before the region-contains decision
    runs (the in-instance registry lock, or the store's short transaction and
    unique claim token); the decision runs without any lock or transaction
    held, so different ids are never serialized by it.
    """
    ...
    def __init__(self, *, store: SQLiteReplayStore | None = None) -> None:
        ...
        self._store = (
            None
            if store is None
            else store._view(_REGION_CONTAINS_REPLAY_DOMAIN)
        )
        self._registry = None if store is not None else _ReplayRegistry()
    ...
    def bind_once(
        ...
        *,
        ...
        :class:`RegionContainsEntry` following the same nested type rules as
        ...
        being checked or has been consumed raises :class:`ValueError`; wrong
        argument or nested field types (including ``bool`` integers) raise
        :class:`TypeError` (an empty id or an out-of-range expiry raise
        :class:`ValueError`). Inputs are never mutated.
        """
    ...
    def check(
        ...
        *,
        ...
        previously registered on this guard for ``binding.session_id``. The
        id is claimed atomically before the digest/expiry work and the
        delegated decision, so among concurrent calls for the same id at
        most one can return ``True``. The digest is recomputed over the
        presented ``entry``; for a binding with an expiry,
        ``now >= expires_at`` makes the check fail (``now`` defaults to the
        current Unix seconds and must otherwise be a non-``bool`` uint64).
        Only then is the entry handed to :func:`region_contains_committed`
        called in field order with ``entry.region``, ``entry.x_commitment``,
        ``entry.y_commitment``, ``entry.x``, ``entry.y``,
        ``entry.x_blinding`` and ``entry.y_blinding``, unchanged.

        Only a fully successful check consumes the session id; every
        rejection (unknown or consumed id, a claim lost to a concurrent
        check, unequal binding, digest mismatch, expiry or
        :func:`region_contains_committed` returning ``False``) returns
        ``False``, releases the claim and leaves the registration pending,
        so a rejected id can still succeed later. An exception escaping the

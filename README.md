...
# 整批 BoundRegionContainsBatch 与外层 Merkle 根的一次性绑定
...
- `RegionContainsBatchReplayGuard(*, store=None)` — 区域判定 `RegionContainsEntry` 批次的防重放登记册（线程安全）；无参时待用/已消费状态隔离在本实例内存中，传入 `SQLiteReplayStore` 时待用/认领/已消费状态落在该存储中（行键域为 `b"zr/rcbr/v1"`），同文件同命名空间的独立实例（含重启后、跨进程）共享状态；摘要为 `SHA-256(F(D) || F(session_id) || S(entries,L) || F(E))`，其中 `D = b"zr/rcbr/v1"`、`L` 复用既有区域判定完整批的外层叶原字节（`_bound_region_contains_leaf`，即域 `b"zkregion/rcb/v1"` 的既有外层叶编码），F/U/S/E 逐字节沿用各批守卫
  - `bind_once(entries: Sequence[RegionContainsEntry], session_id, *, expires_at=None) -> ReplayBinding` — 把非空批次（沿用 `verify_region_contains_batch` 的序列与嵌套类型规则，保序、留重）一次性绑定到 `session_id`；登记空批、空 id、uint64 越界（期限或批次长度）或待用/校验中（存储后端下含过期认领）/已消费 id 重绑抛 `ValueError`，错型（含用 `bool` 冒充整数）抛 `TypeError`
...
`RegionContainsBatchReplayGuard(*, store=None)` 为整批 :class:`RegionContainsEntry` 提供一次性会话绑定，复用同一个 `ReplayBinding` 类型；`bind_once` 的入参为 `(entries, session_id, *, expires_at=None)`，`entries` 沿用 :func:`verify_region_contains_batch` 的序列与嵌套类型规则（非 `bytes`/`bytearray`/`str` 的 `Sequence`，每项为 :class:`Region` 四个边界、两个 :class:`PedersenCommitment` 各六字段与 `x`/`y`/`x_blinding`/`y_blinding` 均嵌套类型正确的 `RegionContainsEntry`），批次保序、留重、不丢项；无参构造时待用/已消费状态隔离在本实例内存中，传入 `SQLiteReplayStore` 时待用/认领/已消费状态落在该存储中（行键域为 `b"zr/rcbr/v1"`），同文件同命名空间的独立实例（含重启后、跨进程）共享状态。绑定摘要为
...
- `F(x)` 是四字节无符号大端长度前缀加 `x`，`U(n)` 是八字节无符号大端整数编码；
- `S(a, f) = F(U(|a|)) || Σ F(f(a_i))`，即先写以 `U` 编码的元素数，再逐项成帧；
- `L(entry)` 取"Merkle 承诺的区域判定完整批验"一节定义的既有外层叶**原字节**（`_bound_region_contains_leaf`：域 `b"zkregion/rcb/v1"` 后按条目字段顺序以十进制 ASCII（负号保留）写区域四边界、两个承诺各六字段与 `x`/`y`/`x_blinding`/`y_blinding` 的叶原字节），再整体套一层 `F`；
- `E` 的过期编码与 `ReplayGuard` 逐字节相同：无期 `E = b"\x00"`，有期 `E = b"\x01" + uint64be(expires_at)`。

`bind_once` 只登记、不跑判定。类型错误（序列错型、非 `RegionContainsEntry` 项、嵌套字段错型，含用 `bool` 冒充整数）抛 `TypeError`；登记空批、`session_id` 为空、`expires_at` 越出 uint64、`U` 成帧的批次长度越界，或 id 已待用/校验中（存储后端下含过期认领）/已消费均抛 `ValueError`。外层叶以十进制 ASCII 写整数（负号保留），任何整数字段都可成帧，因此没有逐叶的可编码性拒绝。

`check(entries, binding, *, now=None) -> bool` 的校验次序为：先原子认领登记册中的等值待用绑定（存储后端下认领租约过期后可被等值 `check` 接管并换发 token），再重算摘要确认提交的整批条目（含顺序与重复项）就是绑定时的批次，再检查期限（有期绑定要求 `now < expires_at`；`now` 缺省取当前 Unix 秒，显式给出时须为非 `bool` uint64，`bool`/非整数抛 `TypeError`、越界抛 `ValueError`），随后原样委托 `verify_region_contains_batch(entries)` 逐项判定。只有整批判定通过才消费 `session_id`；未登记（含已消费）的 id、竞争失败、不等值绑定、摘要不符、空批、过期或批验返回 `False` 一律返回 `False` 且撤销认领、把 id **复原为待用**，因此被拒的绑定稍后仍可成功一次；委托验证逃出的异常同样先撤销认领再原样透传。存储后端下只有持有当前认领 token 的一方能消费，过期认领被接管后旧 token 既不能消费也不能复原；数据库错误原样透传 `sqlite3.Error`。无参构造时绑定状态不跨实例共享；`check` 的参数类型错误抛 `TypeError`，`store` 参数类型错误抛 `TypeError`。入口不改写任何输入。
...
`BoundRegionContainsReplayGuard(*, store=None)` 为整批 :class:`BoundRegionContainsBatch` 连同其外层 Merkle `root` 提供一次性会话绑定，复用同一个 `ReplayBinding` 类型，`bind_once` 的入参为 `(batch, root, session_id, *, expires_at=None)`，且只登记待用、做全量预检（不跑验证）：`batch` 为 `BoundRegionContainsBatch`，`root`、`session_id` 均为 `bytes`，id 必须非空。无参构造时待用/已消费状态隔离在本实例内存中，传入 `SQLiteReplayStore` 时待用/认领/已消费状态落在该存储中，同文件同命名空间的独立实例（含重启后、跨进程）共享状态。绑定摘要逐字节复用既有 Bound 批守卫的 F/U/S/E 与 SHA-256 公式，批次保序留重，仅两处不同：

- 域改为 `D = b"zr/brcbr/v1"`；
- 逐项成帧的 `L(entry_i)` 改为"Merkle 承诺的区域判定完整批验"一节定义的外层叶原字节（`_bound_region_contains_leaf`），即域 `b"zkregion/rcb/v1"` 的既有外层叶编码；它也是 `verify_region_contains_bound` 验根时重算的外层叶。批序与重复项原样保留，外层 `MerkleMultiProof` 的 `leaf_count`、`indices`、`siblings` 三字段均原样绑定。

`bind_once` 的错误边界与 `verify_region_contains_bound` 一致：任一层字段错型（含用 `bool` 冒充整数）抛 `TypeError`；空 id、`expires_at` 或成帧计数（外层 `leaf_count`、`proof.leaf_count`、各索引、批长度）越出 uint64、或任意状态重绑均抛 `ValueError`。

`check(batch, root, binding, *, now=None) -> bool` 的校验次序与 `BoundOpeningReplayGuard` 相同：原子认领等值待用绑定、重算摘要、检查期限后原样委托 `verify_region_contains_bound(batch, root)`；只有验根与内层 `verify_region_contains_batch` 全部通过才消费 `session_id`，未登记、已消费、绑定不等、摘要不符、过期、错根或判定不过一律返回 `False` 且撤销认领、复原待用，被拒标识稍后仍可成功一次，委托验证逃出的异常先复原认领再原样透传。存储后端下只有持有当前认领 token 的一方能消费，认领租约过期后只能由持等值绑定的检查接管；数据库错误原样透传 `sqlite3.Error`。入口不改写任何输入。

存储后端下 `BoundRegionContainsReplayGuard` 的行键域段为 `b"zr/brcbr/v1"`（与同一存储上 `RegionContainsBatchReplayGuard` 的 `b"zr/rcbr/v1"`、`BoundRegionReplayGuard` 的 `b"zr/brg/v1"` 等行互不冲突），其余机制逐字节一致。

...
class MerkleInclusionBatchEntry:
    """One independent item of an inclusion batch verification.

    Fields, in order: ``leaf`` (``bytes``), ``proof`` (:class:`MerkleProof`)
    and ``root`` (``bytes``) — exactly the arguments of
    :func:`verify_inclusion`, in the same order. All three are positional
    construction arguments; entries compare by value and are immutable.
    """
...
def verify_inclusion_batch(entries: Sequence[MerkleInclusionBatchEntry]) -> bool:
    """Check several independent inclusion proofs in one call.

    ``entries`` must be a non-empty, non-``bytes`` / ``bytearray`` /
    ``str`` sequence of :class:`MerkleInclusionBatchEntry`; an empty batch
    returns ``False`` and lists, tuples and duplicate entries are legal.
    Every entry and all of its nested fields are type-checked up front, so
    a wrong type on a later entry raises :class:`TypeError` even when an
    earlier entry would be rejected; the entries are then checked, in
    order, with :func:`verify_inclusion` against their ``leaf``, ``proof``
    and ``root`` fields, reusing its digest-length and index/path rules;
    any invalid entry returns ``False`` and short-circuiting is allowed.
    Entries are independent of one another: their leaves, roots and
    indices need not relate, and no hash encoding or cryptographic
    aggregation is added. Type errors in ``entries``, an entry or any
    field (including a ``bool`` index, a non-tuple ``siblings`` tuple or a
    non-``bytes`` sibling) raise :class:`TypeError` rather than being
    ...
    for position, entry in enumerate(items):
        if not isinstance(entry, MerkleInclusionBatchEntry):
            raise TypeError(f"entries[{position}] must be a MerkleInclusionBatchEntry")
        _check_merkle_inclusion_proof_types(entry.leaf, entry.root, entry.proof)
    if not items:
        return False
    for entry in items:
...
# pending, claimed and consumed ids in the store under b"zr/mibr/v1",
...
# checks the expiry, delegates the batch verification to
# verify_inclusion_batch and consumes the id only on full success; every
# rejection leaves the id untouched.
#
# digest = SHA-256(
#     F(D) || F(session_id) || S(entries, Q) || F(E)
...
#   id(x) = x, i.e. a sibling digest is framed raw under F, order preserved
#   S(a, f) = F(U(|a|)) || Σ F(f(a_i))
#   Q(e) is the continuous run of the single-leaf inclusion guard digest
#   transcript from F(leaf) through S(proof.siblings, id), i.e.
#     F(leaf) || F(root) || F(U(proof.index)) || S(proof.siblings, id)
#   E is the same expiry encoding as the other guards:
#     b"\x00" when expires_at is None, b"\x01" + uint64be(expires_at) otherwise


def _check_merkle_inclusion_batch_entries_types(
    entries: object,
) -> list[MerkleInclusionBatchEntry]:
    """Validate the inclusion-batch ``entries`` argument types.

    Mirrors the type checks of :func:`verify_inclusion_batch`: ``entries``
    must be a non-``bytes`` / ``bytearray`` / ``str`` sequence of
    :class:`MerkleInclusionBatchEntry` objects whose ``leaf`` / ``root``
    are ``bytes`` and whose ``proof`` is a :class:`MerkleProof` with a
    non-``bool`` integer ``index`` and a tuple-of-bytes ``siblings``. The
    entries are copied into a fresh list so the inputs are never mutated;
    an empty batch is left for
    :meth:`MerkleInclusionBatchReplayGuard.bind_once` to reject with
    :class:`ValueError`, and structural/value problems (digest lengths, a
    negative index or an index/path structure that cannot match a real
    tree) are left to :func:`verify_inclusion_batch` at check time.
    """
    if isinstance(entries, (bytes, bytearray, str)) or not isinstance(entries, Sequence):
        raise TypeError("entries must be a sequence of MerkleInclusionBatchEntry")
...
def _merkle_inclusion_batch_item_framing(entry: MerkleInclusionBatchEntry) -> bytes:
    """``Q(e) = F(leaf) || F(root) || F(U(proof.index)) ||``
    ``S(proof.siblings, id)`` for one batch entry — the continuous run of
    the single-leaf inclusion guard transcript from ``F(leaf)`` through
    the siblings sequence."""
    proof = entry.proof
    material = bytearray()
    material += _frame_length_prefixed(entry.leaf)
    material += _frame_length_prefixed(entry.root)
...
def _merkle_inclusion_batch_replay_digest(
    ...
    Writes, in order: ``F(D)``, ``F(session_id)``, ``S(entries, Q)`` and
    ``F(E)``; the entries keep their batch order (duplicates included) and
    each proof's siblings keep their leaf-to-root order.
    """
    transcript = hashlib.sha256()
    ...
    for entry in entries:
        transcript.update(
            _frame_length_prefixed(_merkle_inclusion_batch_item_framing(entry))
        )
    expiry = _replay_expiry_bytes(expires_at)
    transcript.update(_frame_length_prefixed(expiry))
    return transcript.digest()
...
class MerkleInclusionBatchReplayGuard:
    ...
    digest frames the batch under
    ``SHA-256(F(D) || F(session_id) || S(entries, Q) || F(E))`` with domain
    ``b"zr/mibr/v1"``, where each entry is framed by the continuous run of
    the single-leaf inclusion guard transcript
    ``Q = F(leaf) || F(root) || F(U(index)) || S(siblings, id)`` in the
    exact given batch order; the F / U / S framing and the E expiry
    encoding are reused byte for byte from the other guards. By default
    both the pending and the consumed state live on this guard instance
    and are never shared between instances; passing an
    :class:`SQLiteReplayStore` as ``store`` instead keeps the state in
    ...
    def bind_once(
        ...
        *,
        ...
        Returns the frozen :class:`ReplayBinding`. ``entries`` must be a
        non-string, non-empty sequence of
        :class:`MerkleInclusionBatchEntry` objects (``bytes`` leaf/root and
        a well-typed :class:`MerkleProof` each), kept in the given order
        with duplicates preserved and no item dropped; ``session_id`` must
        be non-empty ``bytes`` and ``expires_at`` must be either ``None``
        or a non-``bool`` unsigned 64-bit Unix-second timestamp. Every
        U-framed count (the proof ``index`` values and the batch /
        ``siblings`` tuple lengths) must likewise fit in uint64. A
        session id that is already pending, being checked or consumed
        raises :class:`ValueError`. Wrong argument or nested field types
        (including a ``bool`` index) raise :class:`TypeError`; an empty
        batch or empty id, an out-of-uint64 expiry or framed count, or a
        rebind raise :class:`ValueError`. Inputs are never mutated.
        """
        items = _check_merkle_inclusion_batch_entries_types(entries)
        _check_bytes(session_id, "session_id")
        if not items:
            raise ValueError("entries must not be empty")
        if not session_id:
    ...
    def check(
        ...
        *,
        ...
        defaults to the current Unix seconds and must otherwise be a
        non-``bool`` uint64). Only then is the batch handed to
        :func:`verify_inclusion_batch`, which checks every entry's leaf,
        proof and root independently.

        Only a fully successful check consumes the session id; every
        rejection (unknown or consumed id, a claim lost to a concurrent
        check, unequal binding, digest mismatch, expiry, or
        :func:`verify_inclusion_batch` returning ``False``) returns
        ``False``, releases the claim and leaves the registration pending.
        An exception escaping the delegated verification likewise releases
        the claim and then propagates unchanged, leaving the id usable.
        With a store backend, only the holder of the current claim token
        ...
        if not items or not _merkle_inclusion_batch_replay_encodable(items):
            return False  # an empty batch or negative/oversized U-framed index
        session_id = binding.session_id
        if self._store is not None:
            return self._check_with_store(items, binding, session_id, current)
        if not self._registry.claim(session_id, binding):

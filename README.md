...
# 区间证明的实例内一次性绑定：接口与 ReplayGuard 相同，条目为 RangeBatchEntry
...
assert range_guard.check(range_entry, range_binding)        # verify_range 成功并消费
...
- `ReplayBinding(session_id, digest, expires_at=None)` — 冻结的一次性防重放绑定；字段依次为非空 `bytes`、任意长度的 `bytes` 摘要（不再限定 32 字节；守卫生成的摘要恒为 32 字节）、`None` 或非 `bool` 的 uint64 Unix 秒过期时间；可位置构造、按值相等且不可变
- `ReplayGuard()` — `MultiSchnorrEntry` 的实例内防重放登记册
...
- `RangeReplayGuard()` — `RangeBatchEntry` 的实例内防重放登记册，接口与 `ReplayGuard` 完全相同
  - `bind_once(entry: RangeBatchEntry, session_id, *, expires_at=None) -> ReplayBinding` — 登记本实例的待用绑定；待用或已消费的 `session_id` 重绑抛 `ValueError`
  - `check(entry, binding, *, now=None) -> bool` — 核对登记值、摘要与期限后，按字段顺序调 `verify_range(commitment, proof, context)`；未登记、不等值、摘要错、过期或验证失败均返回 `False` 且不消费
...
`ReplayGuard` 与 `RangeReplayGuard` 在单个实例内分别为 :class:`MultiSchnorrEntry` 与 :class:`RangeBatchEntry` 提供一次性的会话绑定，两者接口与生命周期完全一致。`ReplayBinding(session_id, digest, expires_at=None)` 是两个守卫共用的冻结数据类：`session_id` 为非空 `bytes`，`digest` 为任意长度的 `bytes`（不再限定 32 字节；守卫生成的摘要恒为 SHA-256 的 32 字节输出），`expires_at` 为 `None` 或非 `bool` 的 uint64 Unix 秒；三字段均可位置构造、按值相等、不可变。绑定摘要按

```
digest = SHA-256(F(D) || F(session_id) || L(entry) || F(E))
```

计算，其中 `F(x)` 是四字节无符号大端长度前缀加 `x`，`L(entry)` **不**再套 `F`；无期绑定 `E = b"\x00"`，有期绑定 `E = b"\x01" + uint64be(expires_at)`。两种守卫只在域分隔符 `D`、叶编码 `L(entry)` 与最终验证方式上不同：

- `ReplayGuard`（Schnorr 条目）：`D = b"zr/r/v1"`，`L(entry)` 是"Merkle 承诺的 Schnorr 完整批验"一节定义的 BoundSchnorr 叶原字节；`check` 以条目的公钥与群参数构造 `SchnorrVerifier` 后调 `verify_proof`。
- `RangeReplayGuard`（区间条目）：`D = b"zr/rr/v1"`，`L(entry)` 逐字节复用"Merkle 承诺的区间证明完整批验"一节定义的 range-bound 叶编码；`check` 按字段顺序调用 `verify_range(commitment, proof, context)`。

`bind_once(entry, session_id, *, expires_at=None)` 登记本实例的待用绑定并返回它：`session_id` 为空抛 `ValueError`，`expires_at` 越界（非 uint64）抛 `ValueError`；类型错误（含 `bool`、非 `bytes` 的 `session_id`、条目种类与守卫不符或其嵌套字段类型错误）抛 `TypeError`。此外 `ReplayGuard.bind_once` 还要求 Schnorr 条目的整数字段非负（BoundSchnorr 叶无符号编码无法表示负数），否则抛 `ValueError`。`session_id` 已处于待用或已消费状态时重绑抛 `ValueError`。

`check(entry, binding, *, now=None) -> bool` 只接受本实例内仍待用且与登记值相等的绑定。校验次序为：在登记册中查到 `binding.session_id` 的待用绑定且与 `binding` 按值相等；重算摘要确认提交的 `entry` 就是绑定时的条目；有期绑定要求 `now < expires_at`（`now` 缺省取当前 Unix 秒，显式给出时须为非 `bool` uint64，越界抛 `ValueError`）；随后才做条目自身的验证——`ReplayGuard` 以 `SchnorrVerifier(public_key, prime=entry.prime, generator=entry.generator)` 配合 `entry.message`、`entry.proof`、`entry.context`（keyword `context=`）调 `verify_proof`，`RangeReplayGuard` 调 `verify_range(entry.commitment, entry.proof, entry.context)`，入参与行为完全沿用旧接口——非法群参数、承诺/证明结构错误或验证失败均返回 `False`。只有全部成功才把 `session_id` 从待用移入已消费；未登记（含已消费）的 id、不等值绑定、摘要不符、过期或验证失败一律返回 `False` 且**不消费**，因此被拒的绑定稍后仍可成功一次。绑定状态不跨实例共享（两种守卫的实例登记册也彼此独立）；`check` 的参数类型错误抛 `TypeError`。入口不改写任何输入。

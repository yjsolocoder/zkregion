...
- `prove_range_batch_bound(entries) -> tuple[BoundRangeBatch, bytes]` — 顶层构造 Merkle 承诺的区间证明完整批：`entries` 沿用 `verify_range_batch` 的全批嵌套类型规则（非 `bytes`/`bytearray`/`str` 序列）且必须非空，构造不改写输入，条目按原顺序转为元组并保留重复项；每项外层叶逐字节复用 `_bound_range_leaf` 编码，域分隔、长度帧、字段顺序及 Merkle 叶/内部哈希规则均不变；令 `n = len(entries)`，对编码叶按全索引 `tuple(range(n))` 调 `prove_multi_inclusion` 得到完整多包含证明（`indices` 覆盖每片叶、`siblings` 为空），返回批的 `leaf_count = n`、`proof` 为该证明，第二返回值为编码叶的 `merkle_root`；返回批满足 `verify_range_bound(batch, root) is True`，单条目、奇偶数与重复条目均确定，与旧手工构造逐字节兼容；全批或任一嵌套字段错型（含后项错型与 `bool` 冒充计数）抛 `TypeError`，空批、成帧计数越出 uint64 或 `verify_range_batch` 返回 `False`（含无效内层证明）抛 `ValueError`
...
- `prove_region_batch_bound(entries) -> tuple[BoundRegionBatch, bytes]` — 顶层构造 Merkle 承诺的二维区域证明完整批：`entries` 沿用 `verify_region_batch` 的全批嵌套类型规则（非 `bytes`/`bytearray`/`str` 序列）且必须非空，构造不改写输入，条目按原顺序转为元组并把重复项原样保留；每项外层叶逐字节复用 `_bound_region_leaf` 编码，域分隔、长度帧、字段顺序及 Merkle 叶/内部哈希规则均不变；令 `n = len(entries)`，对编码叶按全索引 `tuple(range(n))` 调 `prove_multi_inclusion` 得到完整多包含证明（`indices` 覆盖每片叶、`siblings` 为空），返回批的 `leaf_count = n`、`proof` 为该证明，第二返回值为编码叶的 `merkle_root`；返回批满足 `verify_region_bound(batch, root) is True`，单条目、奇偶数与整批重复均确定，与旧手工拼装逐字节等值；全批或任一嵌套字段错型（含后项错型与 `bool` 冒充计数）抛 `TypeError`，空批、成帧计数越出 uint64 或 `verify_region_batch` 返回 `False`（含无效内层证明）抛 `ValueError`

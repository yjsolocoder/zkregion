...
# 单条宽区间批验条目的一次性绑定（可选 SQLite 后端跨实例共享）
from zkregion import WideRangeReplayGuard
...
assert wrr.check(wide_entry, wrr_binding)       # 认领、重算摘要、核期限后委托 verify_range_wide 并消费：首次判真
...
assert wrr_b.check(wide_entry, wrr_binding)                     # 认领、单条宽区间验证并消费（行键域 b"zr/wrr/v1"）
...
- `WideRangeReplayGuard(*, store=None)` — 单条宽区间批验条目（`WideRangeBatchEntry`）的一次性防重放登记册（线程安全）；无参构造的状态只在本实例互不影响，传入 `SQLiteReplayStore` 时待用/认领/已消费状态落在该存储中（行键域为 `b"zr/wrr/v1"`，同一名称同时是摘要域与存储行键域，与其余各行键域互不冲突），同文件同命名空间的独立实例、跨进程与重启共享三种状态；摘要逐字节沿用既有单条守卫的成帧与摘要公式 `SHA-256(F(D) || F(session_id) || L(entry) || F(E))`，仅域改为 `b"zr/wrr/v1"`、逐项叶字节 `L(entry)` 复用宽区间完整批的外层叶编码（`_bound_wide_range_leaf`，即域 `b"zkregion/wrb/v1"` 的既有外层叶编码，原样不再加 F 帧），不改写输入
  - `bind_once(entry: WideRangeBatchEntry, session_id, *, expires_at=None) -> ReplayBinding` — 只登记待用并返回冻结的 `ReplayBinding`；`entry` 的嵌套类型规则与 `verify_range_wide_batch` 的单条条目一致（承诺六字段为非 `bool` 整数、证明三层元组均为非 `bool` 整数、`context` 为 `bytes`）；空 id、uint64 越界期限或待用/校验中（存储后端下含过期认领）/已消费 id 重绑（任意状态重绑）抛 `ValueError`，任一层字段给错类型（含用布尔值冒充整数）一律抛唯一的 `TypeError`；宽区间叶以十进制 ASCII 写整数（负号保留），任何整数字段都可成帧
  - `check(entry, binding, *, now=None) -> bool` — 返回布尔结果且次序固定：先原子认领等值的待用绑定（存储后端沿用短事务认领与租约过期接管，被接管的旧 token 无权改状态），再重算摘要、核对期限（`now` 缺省取当前 Unix 秒，显式给出时须为非 `bool` uint64，越界抛 `ValueError`），随后按条目字段次序以 `commitment`、`proof`、`context` 原样委托单条宽区间验证 `verify_range_wide`；只有宽区间验证通过才消费 `session_id`，未登记、已消费、绑定不等、摘要不符、过期或验证不过一律返回 `False` 并把标识复原待用（被拒标识稍后仍可成功一次），委托异常与数据库错误先复原认领再原样透传；并发检查同一标识至多一次成功，不同标识不被一次耗时校验全程串行阻塞；宽区间的单条生成验证、批量验证与完整批入口及全部旧守卫行为保持不变
...
`WideRangeReplayGuard(*, store=None)` 为单条 :class:`WideRangeBatchEntry` 提供与 `RangeReplayGuard` 同构的一次性会话绑定，复用同一个 `ReplayBinding` 类型，`bind_once` 的入参为 `(entry, session_id, *, expires_at=None)`、`check` 的入参为 `(entry, binding, *, now=None)`；无参构造时状态只在本实例互不影响，传入 `SQLiteReplayStore` 时待用/认领/已消费状态落在该存储中，同文件同命名空间的独立实例、跨进程与重启共享三种状态。绑定摘要按同一公式

```
digest = SHA-256(F(D) || F(session_id) || L(entry) || F(E))
```

计算，但域为独立的 `D = b"zr/wrr/v1"`，该名称同时兼作存储后端的行键域；`L(entry)` 是"Merkle 承诺的宽区间证明完整批验"一节定义的逐项外层叶原字节（`_bound_wide_range_leaf`，即域为 `b"zkregion/wrb/v1"` 的既有外层叶编码，与 `verify_range_wide_batch` / `verify_range_wide_batch_bound` 提交的同一份叶，**不**再套 `F`），`E` 的过期编码与 `ReplayGuard` 逐字节相同；`F` 成帧与字段次序逐字节沿用既有单条守卫。

`bind_once` 只登记待用并做全量类型预检（不跑验证），返回冻结的 `ReplayBinding`：`entry` 须为 `WideRangeBatchEntry`，其嵌套类型规则与 `verify_range_wide_batch` 的单条条目一致（`commitment` 为 `PedersenCommitment` 六字段非 `bool` 整数、`proof` 为 `WideRangeProof` 且 `commitments`/`challenges`/`responses` 三层元组的每一项都是非 `bool` 整数、`context` 为 `bytes`），任一层字段给错类型（含用布尔值冒充整数）一律抛唯一的 `TypeError`；`session_id` 为空、`expires_at` 越出 64 位无符号范围、或 id 已待用/校验中（存储后端下含过期认领）/已消费（任意状态重绑）均抛 `ValueError`。宽区间叶以十进制 ASCII 写整数（负号保留），任何整数字段都可成帧，因此没有额外的可编码性拒绝。

`check(entry, binding, *, now=None) -> bool` 返回布尔结果且处理次序固定：先原子认领等值的待用绑定（内存登记册锁或存储短事务加唯一认领 token），再重算摘要确认提交的 `entry` 就是绑定时的对象，再核对期限（`now` 缺省取当前 Unix 秒，显式给出时须为非 `bool` uint64，越界抛 `ValueError`），随后按条目字段次序以 `entry.commitment`、`entry.proof`、`entry.context` 原样委托单条宽区间验证 `verify_range_wide`。只有宽区间验证通过才消费 `session_id`；未登记、已消费、绑定不等、摘要不符、过期或验证不过一律返回 `False` 且把标识复原待用，被拒的标识稍后仍可成功一次；委托验证逃出的异常先复原认领再原样透传。认领发生在任何摘要/期限/证明工作之前且只是逐 id 的登记册状态而非全局锁，因此并发检查同一标识至多一次成功，不同标识不被一次耗时校验全程串行阻塞。

存储后端下 `WideRangeReplayGuard` 沿用 `SQLiteReplayStore` 的短事务认领、租约过期接管与 token 所有权口径，仅行键的域段为 `b"zr/wrr/v1"`（与同一存储上 `RangeReplayGuard` 的 `b"zr/rr/v1"`、`BoundWideRangeReplayGuard` 的 `b"zr/bwrr/v1"` 等行互不冲突）：认领、换发 token、消费与异常复原语义与 `RangeReplayGuard` 逐字节一致，被接管的旧 token 既不能消费也不能复原，数据库错误原样透传 `sqlite3.Error`。宽区间的单条生成验证、批量验证与完整批入口（`prove_range_wide`、`verify_range_wide`、`verify_range_wide_batch`、`verify_range_wide_batch_bound`）及全部旧守卫行为保持不变，本次不引入任何新的落盘口径。对照看：首次判真、二次判假、换绑判假（任意状态重绑抛 `ValueError`），并发同一标识单赢家。

...
- `prove_multi_inclusion_batch_bound(entries) -> tuple[BoundMerkleMultiBatch, bytes]` — 顶层构造 Merkle 承诺的完整多包含批：`entries` 沿用 `verify_multi_inclusion_batch` 的全批嵌套类型规则且必须非空，构造不改写输入，条目按原顺序转为元组并保留重复项；每项外层叶逐字节复用 `_bound_merkle_multi_leaf` 编码，叶摘要与内部节点沿用现有 SHA-256 Merkle 规则；令 `n = len(entries)`，按 `tuple(range(n))` 对这些叶调用 `prove_multi_inclusion` 得到完整多包含证明，返回批的 `leaf_count = n`、`proof` 为该证明，第二返回值为编码叶的 `merkle_root`；返回批满足 `verify_multi_inclusion_batch_bound(batch, root) is True`，单项、奇偶数与重复项均确定，与旧手工构造逐字节兼容；全批类型预检失败抛 `TypeError`，空批、任一 U 成帧整数（proof `index`、`siblings` 长度）越出 uint64 或 `verify_multi_inclusion_batch` 返回 `False` 抛 `ValueError`

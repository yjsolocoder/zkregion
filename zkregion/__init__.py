...
#   Q(item) = the continuous run of the single-inclusion MerkleInclusionReplayGuard
#             transcript from F(leaf) through S(proof.siblings, id):
#               F(leaf) || F(root) || F(U(proof.index)) || S(proof.siblings, id)
#             — exactly the bytes of
#             _merkle_inclusion_proof_framing(item.leaf, item.root, item.proof)
#
...
def _bound_merkle_inclusion_leaf(item: MerkleInclusionBatchEntry) -> bytes:
    ...
    through ``S(proof.siblings, id)`` —
    ``F(leaf) || F(root) || F(U(proof.index)) || S(proof.siblings, id)`` —
    exactly :func:`_merkle_inclusion_proof_framing` applied to the item's
...
def verify_inclusion_batch_bound(
    ...
    leaf exactly as specified by :func:`_bound_merkle_inclusion_leaf` and
    ...
    through :func:`verify_inclusion_batch` unchanged, which reuses its
    root length, index, sibling length and path-walk rules for every
    entry in order.
    ...
    malformed nested field types (including a ``bool`` index, a non-tuple

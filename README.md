...
# 单条同一公钥批验条目（SchnorrBatchEntry）的一次性绑定（可选 SQLite 后端跨实例共享）
from zkregion import SingleKeyEntryReplayGuard

sker_entry = SchnorrBatchEntry(b"alpha", prover.prove(b"alpha", context=b"s1"), context=b"s1")
sker = SingleKeyEntryReplayGuard(prover.public_key)        # 固定公钥与群参数
sker_binding = sker.bind_once(sker_entry, b"session-1")
assert sker.check(sker_entry, sker_binding)                # 首次判真：认领、核摘要、期限并单条验签后消费
assert not sker.check(sker_entry, sker_binding)            # 二次判假：已消费，提交被拒

sker_other = SingleKeyEntryReplayGuard(prover.public_key)  # 无参实例状态互不相通
assert not sker_other.check(sker_entry, sker_binding)      # 换绑判假：他实例的绑定在此不可用

import threading
sker_race = SingleKeyEntryReplayGuard(prover.public_key)
sker_race_binding = sker_race.bind_once(sker_entry, b"session-2")
sker_results = []
sker_threads = [
    threading.Thread(target=lambda: sker_results.append(sker_race.check(sker_entry, sker_race_binding)))
    for _ in range(8)
]
for t in sker_threads:
    t.start()
for t in sker_threads:
    t.join()
assert sker_results.count(True) == 1                       # 并发单赢家：同一标识至多一次成功
...
- `SingleKeyEntryReplayGuard(public_key, *, prime=DEFAULT_PRIME, generator=DEFAULT_GENERATOR, store=None)` — 单条同一公钥批验条目（`SchnorrBatchEntry`）的防重放登记册（线程安全）；公钥与群参数在构造时固定并据以构造一个固定的 `SchnorrVerifier`，错型（含 `bool` 整数）抛 `TypeError`，非法群值（`0 < public_key < prime`、`1 < generator < prime` 之外）抛 `ValueError`；无 `store` 时待用/已消费状态隔离在本实例内存中，传入 `SQLiteReplayStore` 时待用/认领/已消费状态落在该存储中（行键域为 `b"zr/sker/v1"`），同文件同命名空间的独立实例（含重启后、跨进程）共享状态；摘要为 `SHA-256(F(D) || F(session_id) || L(entry) || F(E))`，其中 `D = b"zr/sker/v1"`、`L(entry)` 逐字节复用以本守卫公钥和群参数提升条目后构造的 BoundSchnorr 叶原字节（`_bound_schnorr_leaf`），F/E 逐字节沿用既有单条守卫与各批守卫
  - `bind_once(entry: SchnorrBatchEntry, session_id, *, expires_at=None) -> ReplayBinding` — 把单条 `SchnorrBatchEntry`（逐层类型规则沿用 `SchnorrVerifier.verify_batch` 的逐项口径：`bytes` 消息/上下文、非 `bool` 整数承诺/响应）一次性绑定到 `session_id`；空 id、uint64 越界的期限、叶内负整数或待用/校验中（存储后端下含过期认领）/已消费 id 重绑抛 `ValueError`，entry、证明或嵌套字段错型（含 `bool` 整数）抛 `TypeError`
  - `check(entry, binding, *, now=None) -> bool` — 先原子认领等值待用绑定（存储后端可在认领租约过期后由持等值绑定的检查接管），再重算摘要、核期限，随后按条目内消息、证明、上下文的对应次序委托固定验证器的 `verify_proof(entry.message, entry.proof, context=entry.context)`；只有验签通过才消费 `session_id`，未登记、已消费、绑定不等、摘要不符、过期或签名不过一律返回 `False` 且撤销认领、复原待用（被拒标识稍后仍可成功一次），委托异常与数据库错误先复原认领再原样透传；并发检查同一标识至多一次成功，某个标识上的耗时校验不会把其他标识的登记或检查全程串行阻塞；存储后端下只有持有当前认领 token 的一方能消费，拒绝或异常时以该 token 把 id 复原为待用（token 已失效则为无操作）

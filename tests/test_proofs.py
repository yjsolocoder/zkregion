...
class MerkleConsistencyReplayGuardTest(unittest.TestCase):
    """Single-use replay bindings for one append-only consistency segment."""
    ...
    def test_bind_once_returns_spec_digest(self):
        ...
        self.assertEqual(
            binding.digest,
            self.expected_digest(old_root, new_root, proof, b"s2", 1000),
        )
    ...
    def test_domain_separator_is_distinct(self):
        ...
        # the identical segment framing under every other guard domain differs
        for other_domain in (
            b"zr/r/v1",
            b"zr/rr/v1",
            b"zr/rg/v1",
    ...
    def test_digest_binds_every_component(self):
        ...
        bind = lambda *args, **kw: MerkleConsistencyReplayGuard().bind_once(*args, **kw).digest
        # the session id is framed
        self.assertNotEqual(
            base.digest, bind(old_root, new_root, proof, b"other")
        )
        # both roots bind
        self.assertNotEqual(
            base.digest, bind(b"\x7f" * 32, new_root, proof, b"s")
        )
        self.assertNotEqual(
            base.digest, bind(old_root, b"\x7f" * 32, proof, b"s")
        )
        # the counts bind
        changed_count = MerkleConsistencyProof(
            proof.old_count, proof.new_count + 1, proof.nodes
        ...
        self.assertNotEqual(
            base.digest, bind(old_root, new_root, changed_count, b"s")
        )
        # node order binds
        reordered = MerkleConsistencyProof(
            proof.old_count, proof.new_count, proof.nodes[::-1]
        )
        self.assertNotEqual(
            base.digest, bind(old_root, new_root, reordered, b"s")
        )
        # a node binds
        changed_node = MerkleConsistencyProof(
            proof.old_count,
            proof.new_count,
            ((bytes([proof.nodes[0][0] ^ 0x01]) + proof.nodes[0][1:]),)
            + proof.nodes[1:],
        ...
        self.assertNotEqual(
            base.digest, bind(old_root, new_root, changed_node, b"s")
        )
        # the expiry framing binds
        self.assertNotEqual(
            base.digest, bind(old_root, new_root, proof, b"s", expires_at=1)
        )
        self.assertNotEqual(
            bind(old_root, new_root, proof, b"s", expires_at=1),
            bind(old_root, new_root, proof, b"s", expires_at=2),
        )

    def test_expiry_encodings_distinguish_presence_and_value(self):
        old_root, new_root, proof = self._segment()
        none_binding = MerkleConsistencyReplayGuard().bind_once(
            old_root, new_root, proof, b"a"
        )
    ...
    def test_bind_type_errors(self):
        old_root, new_root, proof = self._segment()
        for bad_root in ("root", 7, None, bytearray(old_root), True):
            with self.assertRaises(TypeError):
                MerkleConsistencyReplayGuard().bind_once(bad_root, new_root, proof, b"s")
            with self.assertRaises(TypeError):
                MerkleConsistencyReplayGuard().bind_once(old_root, bad_root, proof, b"s")
        for bad_proof in ("proof", 7, None, proof.nodes, True):
            with self.assertRaises(TypeError):
                MerkleConsistencyReplayGuard().bind_once(
                    old_root, new_root, bad_proof, b"s"
                )
        guard = MerkleConsistencyReplayGuard()
    ...
    def test_bind_value_errors(self):
        ...
        huge = 2**64
        oversized = MerkleConsistencyProof(huge, huge + 1, (b"\x00" * 32,))
        with self.assertRaises(ValueError):
            MerkleConsistencyReplayGuard().bind_once(
                old_root, new_root, oversized, b"s"
            )
        negative = MerkleConsistencyProof(-1, proof.new_count, proof.nodes)
        with self.assertRaises(ValueError):
            MerkleConsistencyReplayGuard().bind_once(
                old_root, new_root, negative, b"s"
            )
    ...
    def test_wrong_segment_rejected_without_consuming(self):
        old_root, new_root, proof = self._segment()
        other_root, other_new, other_proof = self._segment(old_count=2, size=5)
        guard = MerkleConsistencyReplayGuard()
        binding = guard.bind_once(old_root, new_root, proof, b"s")
        self.assertFalse(
            guard.check(other_root, other_new, other_proof, binding, now=1)
        )
        self.assertIn(b"s", guard._pending)
        self.assertTrue(guard.check(old_root, new_root, proof, binding, now=1))

    def test_bound_but_unverifiable_segment_rejected_and_restored(self):
        # bind_once never verifies the segment: a well-typed, U-encodable but
    ...
    def test_unknown_or_foreign_binding_rejected(self):
        ...
        self.assertFalse(
            guard.check(
                ...
                ReplayBinding(b"never", b"\x00" * 32), now=1,
            )
        )
        binding = guard.bind_once(old_root, new_root, proof, b"s")
        # a binding from another isolated instance carries no registration
        # on a fresh instance, even though the binding values are equal
        foreign = MerkleConsistencyReplayGuard().bind_once(
            old_root, new_root, proof, b"s"
        )
        self.assertEqual(foreign, binding)
        self.assertFalse(
            MerkleConsistencyReplayGuard().check(
                old_root, new_root, proof, foreign, now=1
            )
        )
        # while the instance that owns the registration accepts it
        self.assertTrue(guard.check(old_root, new_root, proof, binding, now=1))
    ...
    def test_store_domain_isolation_from_other_guards(self):
        ...
        replay_binding = replay_guard.bind_once(entry, b"same-id")
        segment_binding = consistency_guard.bind_once(
            old_root, new_root, proof, b"same-id"
        )
        self.assertTrue(replay_guard.check(entry, replay_binding))
        self.assertTrue(
            consistency_guard.check(old_root, new_root, proof, segment_binding, now=1)
        )
        domains = {row[1] for row in self.raw_rows()}
        self.assertIn(self.DOMAIN, domains)
        self.assertIn(b"zr/r/v1", domains)
    ...
    def test_store_rejection_restores_pending_for_other_instance(self):
        ...
        checker = MerkleConsistencyReplayGuard(store=store)
        self.assertFalse(
            checker.check(other_root, other_new, other_proof, binding, now=1)
        )
        self.assertIn(b"s", checker._pending)
        self.assertTrue(

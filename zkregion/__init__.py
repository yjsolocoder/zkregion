...
# verify_bound (passing the random source through unchanged) and consumes the
...
class BoundSchnorrReplayGuard:
    ...
    def bind_once(
        ...
        *,
        ...
        or consumed raises :class:`ValueError`; with a store backend an
        expired claim rejects the rebind too (only :meth:`check` may take an
        expired claim over). Wrong argument or nested field types raise
        :class:`TypeError`; an empty id, an out-of-uint64 expiry or framed
        integer, a negative leaf integer or a rebind raise
        :class:`ValueError`. Inputs are never mutated.
        """

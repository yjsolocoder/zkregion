...
def verify_consistency_chain_batch(chains: Sequence[MerkleConsistencyChain]) -> bool:
    ...
    unchanged, reusing its root, proof, chaining and append rules; any
    invalid chain returns ``False`` and short-circuiting is allowed.
    Chains are independent of one another: their checkpoints need not
    relate, and no hash encoding or cryptographic aggregation is added.
    Type errors in ``chains`` or a chain item raise :class:`TypeError`,
    as do the nested field type errors raised by the delegated
    :func:`verify_consistency_chain` (including ``bool`` counts, a
    non-tuple ``roots`` / ``proofs`` / ``nodes`` or a non-``bytes`` root
    or node), rather than being converted into a batch rejection. Inputs
    are never mutated.
    """
...
_MERKLE_CONSISTENCY_CHAIN_BATCH_REPLAY_DOMAIN = b"zr/mccbr/v1"
...
# ---------------------------------------------------------------------------
# Replay protection for Merkle consistency chain batches
#
...
# byte, the F / U / S framing and the E expiry encoding of the other replay
...
#   F(x) = four-byte unsigned big-endian length prefix of x, followed by x
#   U(n) = eight-byte unsigned big-endian encoding of n
#   L(c) = the existing BoundConsistencyChain leaf raw bytes for c
#          (_bound_consistency_chain_leaf)
#   S(a, f) = F(U(|a|)) || Σ F(f(a_i))
#   E is the same expiry encoding as the other guards:
#     b"\x00" when expires_at is None, b"\x01" + uint64be(expires_at) otherwise
#
# The batch keeps its given order, including duplicates; chains are never
# dropped or reordered.


def _check_merkle_consistency_chain_batch_types(
    chains: object,
) -> list[MerkleConsistencyChain]:
    """Validate the consistency-chain-batch ``chains`` argument types.

    Mirrors the sequence and item type rules of
    :func:`verify_consistency_chain_batch`: ``chains`` must be a
    non-``bytes`` / ``bytearray`` / ``str`` sequence of
    :class:`MerkleConsistencyChain` objects, each nestedly well-typed per
    :func:`_check_merkle_consistency_chain_types` (a tuple of ``bytes``
    roots and a tuple of well-typed :class:`MerkleConsistencyProof`
    objects). The chains are copied into a fresh list so the inputs are
    never mutated; an empty batch is left for
    ...
    chaining, digest lengths) are left to
...
def _merkle_consistency_chain_batch_replay_encodable(
    chains: Sequence[MerkleConsistencyChain],
) -> bool:
    """Every U-framed count must fit in unsigned 64 bits.

    The outer ``S`` sequence writes the batch count with ``U`` (eight-byte
    unsigned big-endian); each chain is framed as the existing
    BoundConsistencyChain leaf, whose counts are decimal ASCII and need no
    encodability rule.
    """
    return 0 <= len(chains) <= _UINT64_MAX


def _merkle_consistency_chain_batch_replay_digest(
    chains: Sequence[MerkleConsistencyChain],
    session_id: bytes,
    expires_at: int | None,
) -> bytes:
    """Compute the consistency-chain-batch replay binding digest.

    Writes, in order: ``F(D)``, ``F(session_id)``, ``S(chains, L)`` and
    ``F(E)``; ``L(chain)`` is the existing BoundConsistencyChain leaf raw
    bytes (the same bytes :func:`_bound_consistency_chain_leaf` builds for
    :func:`verify_consistency_chain_batch_bound`) and the chains keep their
    batch order (duplicates included).
    """
    transcript = hashlib.sha256()
    transcript.update(
        _frame_length_prefixed(_MERKLE_CONSISTENCY_CHAIN_BATCH_REPLAY_DOMAIN)
    )
    transcript.update(_frame_length_prefixed(session_id))
...
class MerkleConsistencyChainBatchReplayGuard:
    ...
    accepts an equal pending binding exactly once, recomputing the binding
    digest, checking the expiry and delegating to
    :func:`verify_consistency_chain_batch`, and then marks the id consumed.
    ...
    def __init__(self, *, store: SQLiteReplayStore | None = None) -> None:
        ...
        self._store = (
            None
            if store is None
            else store._view(_MERKLE_CONSISTENCY_CHAIN_BATCH_REPLAY_DOMAIN)
        )
        self._registry = None if store is not None else _ReplayRegistry()
    ...
    def bind_once(
        ...
        *,
        ...
        objects (a tuple of ``bytes`` roots and well-typed
        :class:`MerkleConsistencyProof` objects each), kept in the given
        order with duplicates preserved and no chain dropped;
        ``session_id`` must be non-empty ``bytes`` and ``expires_at`` must
        be either ``None`` or a non-``bool`` unsigned 64-bit Unix-second
        timestamp. The batch length written by the outer S sequence must
        ...
        if expires_at is not None:
            _check_uint64(expires_at, "expires_at")
        if not _merkle_consistency_chain_batch_replay_encodable(items):
            raise ValueError("the batch length must be an unsigned 64-bit integer")
        binding = ReplayBinding(
            session_id,
            _merkle_consistency_chain_batch_replay_digest(
    ...
    def check(
        ...
        *,
        ...
        defaults to the current Unix seconds and must otherwise be a
        non-``bool`` uint64). Only then is the batch handed to
        :func:`verify_consistency_chain_batch`, which checks every chain's
        roots, segment chaining and append-only proofs independently.
        ...
        if now is None:
            ...
            _check_uint64(now, "now")
            current = now
        if not items or not _merkle_consistency_chain_batch_replay_encodable(items):
            return False  # an empty batch or an oversized U-framed batch count
        session_id = binding.session_id
        if self._store is not None:
            return self._check_with_store(items, binding, session_id, current)
        if not self._registry.claim(session_id, binding):

...
- `prove_region_wide(x_commitment, y_commitment, x, y, x_blinding, y_blinding, region, context=b"", *, randbelow=secrets.randbelow) -> RegionWideProof` — 按位分解的宽区间二维矩形区域成员非交互证明：两轴承诺的声明区间须恰为区域对应轴的上下界（x 承诺为 `(region.min_x, region.max_x)`、y 承诺为 `(region.min_y, region.max_y)`），每轴区间个数须恰为 `2**k` 且位宽 `1 <= k <= 24`（两轴位宽可各自不同），违反任一约束抛 `ValueError`；生成前先逐轴以 `verify_pedersen_opening` 同一口径重算比对开合，两轴都通过才生成子证明，开合不符或坐标越出声明区间抛 `ValueError`；子证明逐轴复用 `prove_range_wide`，派生上下文沿用 `_region_sub_context` 的既有绑定（区域域分隔、轴标签、外部上下文、区域四边界与两份承诺）；随机源不可调用或返回非整数（含 `bool`）抛 `TypeError`、抽取值越界抛 `ValueError`；同一组入参在相同随机源下重复生成的证明逐字节一致，`randbelow` 原样透传，输入不被改写
- `verify_region_wide(x_commitment, y_commitment, region, proof, context=b"") -> bool` — 验证宽区间二维区域成员证明，无需坐标或盲因子：两轴承诺的声明区间须恰为区域对应轴的上下界，否则返回 `False`；逐轴以同一派生上下文委托 `verify_range_wide`，位宽越界、区间个数非 2 的幂、换区域、换上下文、换任一承诺、交换两轴或替换任一子证明一律返回 `False` 且不抛异常；任一层字段错型（含用 `bool` 冒充整数、上下文不是字节串）抛 `TypeError`，输入不被改写
- `RegionWideProof(x_proof, y_proof)` — 不可变宽区间二维区域证明对象，两字段均为 `WideRangeProof`；可位置构造、按值相等且不可变

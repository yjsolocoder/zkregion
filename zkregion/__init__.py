...
# digest, checks the expiry, delegates to verify_consistency and consumes the
# id only on full success; every rejection leaves the id untouched.
#
# digest = SHA-256(
#     F(D) || F(session_id) || F(old_root) || F(new_root)
#     || F(U(old_count)) || F(U(new_count)) || S(nodes, id) || F(E)
# )
#   D = b"zr/mcr/v1"
#   F(x) = four-byte unsigned big-endian length prefix of x, followed by x
#   U(n) = eight-byte unsigned big-endian encoding of n
#   id(x) = x, i.e. a node digest is framed raw under F
#   S(a, f) = F(U(|a|)) || Σ F(f(a_i)); nodes keep their proof order
#   E is the same expiry encoding as the other guards:
#     b"\x00" when expires_at is None, b"\x01" + uint64be(expires_at) otherwise
...
def _merkle_consistency_replay_encodable(proof: MerkleConsistencyProof) -> bool:
    ...
    The proof counts (``old_count`` / ``new_count``) and the ``nodes``
...
def _merkle_consistency_replay_digest(
    ...
    ``F(new_root)``, ``F(U(old_count))``, ``F(U(new_count))``,
    ``S(nodes, id)`` and ``F(E)``; the nodes keep their proof order.
    """
    transcript = hashlib.sha256()
...
def _check_merkle_consistency_types(old_root: object, new_root: object, proof: object) -> None:
    """Validate root/proof argument types for the replay guard.
    ...
    Structural and value problems (counts, node count, digest lengths) are
    left to :func:`verify_consistency` at check time.
    """
    _check_bytes(old_root, "old_root")
    _check_bytes(new_root, "new_root")
    if not isinstance(proof, MerkleConsistencyProof):
...
class MerkleConsistencyReplayGuard:
    """Single-use replay protection for one append-only consistency proof.

    A fresh guard has no registrations. :meth:`bind_once` registers a
    pending :class:`ReplayBinding` that commits a single
    :class:`MerkleConsistencyProof` together with the ``old_root`` and
    ``new_root`` it links to a session id; :meth:`check` accepts an equal
    pending binding exactly once, recomputing the binding digest, checking
    the expiry and delegating to :func:`verify_consistency`, and then marks
    the id consumed. The digest frames the segment under
    ...
    def bind_once(
        ...
        *,
        ...
        """Register this instance's binding of ``session_id`` to the segment.

        Returns the frozen :class:`ReplayBinding`. ``old_root`` and
        ``new_root`` must be ``bytes`` and ``proof`` a well-typed
        :class:`MerkleConsistencyProof`; ``session_id`` must be non-empty
        ``bytes`` and ``expires_at`` must be either ``None`` or a
        non-``bool`` unsigned 64-bit Unix-second timestamp; every U-framed
        count (the proof ``old_count`` / ``new_count`` and the ``nodes``
        length) must likewise fit in uint64. A session id that is already
        pending, being checked or consumed raises :class:`ValueError`.
        Wrong argument or nested field types raise :class:`TypeError`; an
        empty id, an out-of-uint64 expiry or framed count, or a rebind
        raise :class:`ValueError`. Inputs are never mutated.
        """
        _check_merkle_consistency_types(old_root, new_root, proof)
        _check_bytes(session_id, "session_id")
        if not session_id:
            raise ValueError("session_id must not be empty")
        if expires_at is not None:
        ...
        if not _merkle_consistency_replay_encodable(proof):
            raise ValueError("proof counts and nodes length must be unsigned 64-bit integers")
        binding = ReplayBinding(
            session_id,
    ...
    def check(
        ...
        *,
        ...
        """Verify and consume the pending binding for the segment.
        ...
        defaults to the current Unix seconds and must otherwise be a
        non-``bool`` uint64). Only then is the segment handed to
        :func:`verify_consistency`, which checks the counts, the node
        count, the digest lengths and both roots.
        ...
        ids are never serialized. Argument type errors raise
        :class:`TypeError`; an out-of-range ``now`` raises
        :class:`ValueError`. Inputs are never mutated.
        """
        _check_merkle_consistency_types(old_root, new_root, proof)
        ...
        try:
            if not hmac.compare_digest(
                ...
                return False  # the presented segment is not the one originally bound
            if binding.expires_at is not None and current >= binding.expires_at:
                return False  # expired: rejection does not consume the id

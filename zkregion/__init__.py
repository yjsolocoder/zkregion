...
# BoundMerkleInclusionBatch together with the outer Merkle root it is
# claimed under to a caller-chosen session id, reusing the ReplayBinding
# type and, byte for byte, the F / U / S framing, the E expiry encoding
# and the SHA-256 formula of BoundConsistencyReplayGuard; only the domain
# separator and the per-item leaves differ. Like the other bound guards
# the binding is single-use: without a store the state is local to the
# guard instance, while an SQLiteReplayStore keeps pending, claimed and
# consumed ids in the store under b"zr/bmibr/v1", shared across
# instances, processes and restarts. bind_once registers a pending
# binding, check recomputes the digest, checks the expiry, delegates the
# batch verification to verify_inclusion_batch_bound and consumes the id
...
#   U(n) = eight-byte unsigned big-endian encoding of n
#   S(a, f) = F(U(|a|)) || Σ F(f(a_i))
#   L(item) = the single-leaf bound-inclusion-batch leaf bytes
...
#             the MerkleInclusionReplayGuard framing from F(item.leaf)
#             through the siblings sequence
#   E is the same expiry encoding as the other guards:
#     b"\x00" when expires_at is None, b"\x01" + uint64be(expires_at) otherwise
#
# The items keep their batch order, duplicates included; items are never
...
def _bound_merkle_inclusion_batch_replay_encodable(
    ...
    The outer transcript frames ``batch.leaf_count``,
    ``batch.proof.leaf_count`` and every outer proof index with ``U``; each
    per-item leaf ``L(item)`` additionally U-frames that item's proof
    ``index`` and its ``siblings`` sequence length (see
    :func:`_merkle_inclusion_batch_replay_encodable`). A negative or
    larger-than-uint64 value anywhere cannot be framed. The raw root,
    sibling and leaf bytes encode for any value, so no further
    encodability rule is needed.
    """
...
def _bound_merkle_inclusion_batch_replay_digest(
    ...
    transcript.update(_frame_length_prefixed(root))
    transcript.update(_frame_length_prefixed(_uint64_be(batch.leaf_count)))
    for item in batch.entries:  # entries order, each bound inclusion leaf under F
...
class BoundMerkleInclusionBatchReplayGuard:
    ...
    A fresh guard has no registrations. :meth:`bind_once` registers a
    pending :class:`ReplayBinding` that commits a whole
    :class:`BoundMerkleInclusionBatch` together with the outer Merkle
    ``root`` it is claimed under; :meth:`check` accepts an equal pending
    binding exactly once — recomputing the binding digest, checking the
    expiry and delegating to :func:`verify_inclusion_batch_bound` — and
    then marks the id consumed. By default both the pending and the
    consumed state live on this guard instance and are never shared
    between instances; passing an :class:`SQLiteReplayStore` as ``store``
    instead keeps the state in that store under the ``b"zr/bmibr/v1"``
    key domain, so bound-inclusion-batch guards attached to the same
    store namespace share pending, claimed and consumed ids across
    independent instances, processes and process restarts.
    ...
    def bind_once(
        ...
        *,
        ...
        non-``bool`` unsigned 64-bit Unix-second timestamp; every U-framed
        integer (the outer ``leaf_count`` / ``proof.leaf_count`` / proof
        indices and, inside each per-item leaf, the item proof ``index``
        and ``siblings`` tuple length) must likewise fit in uint64. A
        session id that is already pending, being checked or consumed
        raises :class:`ValueError`. Wrong argument or nested field types

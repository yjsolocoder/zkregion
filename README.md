...
assert bmmbr.check(bound_mb, bound_mb_root, bmmbr_binding)       # 先验根与多包含批验再消费
...
assert bmmbr_b.check(bound_mb, bound_mb_root, bmmbr_binding)     # 认领、验根与多包含批验并消费
...
- `BoundMerkleMultiBatchReplayGuard(*, store=None)` — Merkle 承诺多包含批（`BoundMerkleMultiBatch`）与 bytes 根的防重放登记册（线程安全）；无参时待用/已消费状态隔离在本实例内存中，传入 `SQLiteReplayStore` 时待用/认领/已消费状态落在该存储中（行键域为 `b"zr/bmmbr/v1"`），同文件同命名空间的独立实例（含重启后、跨进程）共享状态；摘要逐字节复用 `BoundConsistencyReplayGuard` 的 F/U/S/E 与 SHA-256 公式，仅域改为 `b"zr/bmmbr/v1"`、逐项 `L(item)` 改为 README 既有外层叶原字节（`_bound_merkle_multi_leaf`，即域 `b"zkregion/multi-batch/v1"` 的 `F(D) || F(Q)` 叶编码），批次保序留重，外层 `MerkleMultiProof` 的 `leaf_count`、`indices`、`siblings` 三字段原样绑定
  - `bind_once(batch: BoundMerkleMultiBatch, root: bytes, session_id: bytes, *, expires_at=None) -> ReplayBinding` — 仅登记待用绑定并返回冻结对象；`root`、`session_id` 为 bytes 且 id 非空；空 id、uint64 越界的期限或 U 成帧整数（`leaf_count`、`proof.leaf_count`、各索引）或待用/校验中（存储后端下含过期认领）/已消费 id 重绑抛 `ValueError`，错型（含 `bool` 整数）抛 `TypeError`
...
`BoundMerkleMultiBatchReplayGuard(*, store=None)` 为整批 :class:`BoundMerkleMultiBatch` 连同其 Merkle `root` 提供一次性会话绑定，复用同一个 `ReplayBinding` 类型，`bind_once` 的入参同为 `(batch, root, session_id, *, expires_at=None)`，且只登记待用：`root`、`session_id` 均为 `bytes`，id 必须非空。无参构造时待用/已消费状态隔离在本实例内存中，传入 `SQLiteReplayStore` 时待用/认领/已消费状态落在该存储中，同文件同命名空间的独立实例（含重启后、跨进程）共享状态。绑定摘要逐字节复用 `BoundConsistencyReplayGuard` 的 F/U/S/E 与 SHA-256 公式，字段次序与批次顺序不变（保序留重），仅两处不同：

- 域改为 `D = b"zr/bmmbr/v1"`；
- 逐项成帧的 `L(item_i)` 取"Merkle 承诺的多包含证明完整批验"一节定义的既有外层叶原字节（`_bound_merkle_multi_leaf`），即 `F(b"zkregion/multi-batch/v1") || F(Q(item))`；批次的顺序与重复项、外层 `MerkleMultiProof` 的 `leaf_count`、`indices`、`siblings` 三字段均原样绑定。

`bind_once(batch, root, session_id, *, expires_at=None)` 仅登记待用绑定并返回冻结的 `ReplayBinding`。类型边界与 `verify_multi_inclusion_batch_bound` 一致（`batch` 及其嵌套条目、外层 `proof` 的字段类型同样校验），错型（含 `bool` 整数）抛 `TypeError`；`session_id` 为空、`expires_at` 非 uint64、任一 `U` 成帧整数（`leaf_count`、`proof.leaf_count` 或索引）为负或超出 uint64、或 id 已待用/校验中（存储后端下含过期认领）/已消费均抛 `ValueError`。内存模式下待用与已消费状态只存在于本实例、不跨实例共享；存储模式下在一个短事务中插入待用行，键已存在（待用、被认领——**即使租约已过期**——或已消费）即抛 `ValueError`。

`check(batch, root, binding, *, now=None) -> bool` 先原子认领：核对登记册中的待用绑定与提交绑定按值相等，再重算摘要确认提交的 `batch`/`root` 就是绑定时的对象，再核期限（`now` 缺省取当前 Unix 秒，显式给出时须为非 `bool` uint64，`bool`/非整数抛 `TypeError`、越界抛 `ValueError`），随后原样委托 `verify_multi_inclusion_batch_bound(batch, root)`。只有全部成功才消费 `session_id`；未登记（含已消费）的 id、竞争失败、被替换的绑定、摘要不符（篡改）、过期、根或兄弟长度错误、错误根、其他摘要/结构/证明无效或验批返回 `False` 一律返回 `False` 且**不消费**，撤销认领、保持待用，因此被拒的绑定稍后仍可成功一次。委托验证逃出的异常先撤销认领再原样透传。内存模式下绑定状态不跨实例共享；`check` 的参数类型错误抛 `TypeError`。入口不改写任何输入。
...
- **实例隔离或共享存储**：无参构造时，待用/校验中/已消费状态只存在于单个守卫实例内，不同实例（即便同 id）完全独立、互不阻塞；传入同一 `SQLiteReplayStore`（同文件、同命名空间）的守卫实例则经由短事务与唯一认领 token 共享三种状态，跨实例、跨进程并在重启后保持一致，认领租约过期后只能由等值 `binding` 的 `check` 接管，不同守卫域（`zr/r/v1`、`zr/sbr/v1`、`zr/skbr/v1`、`zr/rbr/v1`、`zr/rgbr/v1`、`zr/rr/v1`、`zr/rg/v1`、`zr/brg/v1`、`zr/brr/v1`、`zr/bsr/v1`、`zr/skbbr/v1`、`zr/mccr/v1`、`zr/mccbr/v1`、`zr/mcr/v1`、`zr/mir/v1`、`zr/mibr/v1`、`zr/mmr/v1`、`zr/mmb/v1`、`zr/mcbr/v1`、`zr/bcbr/v1`、`zr/bccbr/v1`、`zr/bmmbr/v1`）的行互不冲突。

...
# SingleKeyEntryReplayGuard binds one SchnorrBatchEntry — made for one fixed
# public key and group — to one session id. Without a store the
...
# under b"zr/sker/v1", shared across instances, processes and restarts.
# bind_once registers a pending binding, check claims the id atomically,
# recomputes the digest, checks the expiry, delegates the single-entry
# verification to the fixed SchnorrVerifier's verify_proof and consumes the
# id only on full success; every rejection leaves the id untouched.
#
# digest = SHA-256(
#     F(D) || F(session_id) || L(entry) || F(E)
...
def _check_schnorr_batch_entry(entry: object, name: str = "entry") -> SchnorrBatchEntry:
    """Validate one :class:`SchnorrBatchEntry` and its nested field types.

    Applies the per-entry rules of :meth:`SchnorrVerifier.verify_batch`: the
    entry must be a :class:`SchnorrBatchEntry` whose ``message`` /
    ``context`` are ``bytes`` and whose ``proof`` is a :class:`SchnorrProof`
    with non-``bool`` integer commitment / response. Structural and value
    problems (out-of-range commitments, negative responses, a proof that
    does not verify) are left to :meth:`SchnorrVerifier.verify_proof` at
    check time.
    """
    if not isinstance(entry, SchnorrBatchEntry):
        raise TypeError(f"{name} must be a SchnorrBatchEntry")
    _check_bytes(entry.message, f"{name} message")
    _check_bytes(entry.context, f"{name} context")
    proof = entry.proof
    if not isinstance(proof, SchnorrProof):
...
def _single_key_entry_replay_encodable(entry: SchnorrBatchEntry) -> bool:
    """The leaf frames the proof integers unsignedly; negatives cannot be framed.

    The BoundSchnorr leaf writes the proof commitment / response with the
    shortest unsigned big-endian encoding; the guard's own ``prime`` /
    ``generator`` / ``public_key`` are validated positive at construction,
    so only the proof integers can be negative here.
    """
...
def _single_key_entry_replay_digest(
    ...
    *,
    ...
    """Compute the single-key Schnorr entry replay binding digest.

    Writes, in order: ``F(D)``, ``F(session_id)``, ``L(entry)`` and
    ``F(E)``; ``L(entry)`` is the BoundSchnorr leaf raw bytes built with
    the guard's public key and group parameters (the same per-item leaf
    bytes :func:`_single_key_batch_leaf` produces for the batch guard).
    """
    transcript = hashlib.sha256()
...
class SingleKeyEntryReplayGuard:
    ...
    The guard is bound to one fixed ``public_key`` and group (``prime`` /
    ``generator``, defaulting to :data:`DEFAULT_PRIME` /
    :data:`DEFAULT_GENERATOR`); every entry it checks is verified by the
    :class:`SchnorrVerifier` constructed once from those parameters.
    :meth:`bind_once` registers a pending :class:`ReplayBinding` that
    commits a single :class:`SchnorrBatchEntry` — its ``message``,
    :class:`SchnorrProof` and ``context`` — to a session id;
    :meth:`check` accepts an equal pending binding exactly once,
    recomputing the binding digest, checking the expiry and delegating to
    the fixed verifier's :meth:`~SchnorrVerifier.verify_proof` with the
    entry's ``message``, ``proof`` and ``context`` in that order, and then
    marks the id consumed. The digest frames the entry under
    ``SHA-256(F(D) || F(session_id) || L(entry) || F(E))`` with domain
    ``b"zr/sker/v1"``, where ``L(entry)`` is the existing BoundSchnorr
    leaf raw bytes built with this guard's public key and group parameters
    (the same bytes :func:`_bound_schnorr_leaf` builds for
    :func:`verify_bound`, lifted through :func:`_single_key_batch_leaf`);
    the F framing and the E expiry encoding are reused byte for byte from
    the other replay guards. By default both the pending and the consumed
    state live on this guard instance and are never shared between
    instances; passing an :class:`SQLiteReplayStore` as ``store`` instead
    keeps the state in that store under the ``b"zr/sker/v1"`` key domain,
    so entry guards attached to the same store namespace share pending,
    ...
    def bind_once(
        ...
        *,
        ...
        Returns the frozen :class:`ReplayBinding`. ``entry`` must be a
        :class:`SchnorrBatchEntry` following the per-entry type rules of
        ...
        """
        _check_schnorr_batch_entry(entry)
        _check_bytes(session_id, "session_id")
        if not session_id:
            raise ValueError("session_id must not be empty")
        if expires_at is not None:
    ...
    def check(
        ...
        *,
        ...
        ``now >= expires_at`` makes the check fail (``now`` defaults to
        the current Unix seconds and must otherwise be a non-``bool``
        uint64). Only then are the entry's ``message``, ``proof`` and
        ``context`` handed to the fixed verifier's
        :meth:`SchnorrVerifier.verify_proof`, in that order, which
        recomputes the transcript challenge and checks the Schnorr
        response against the commitment.

        Only a fully successful check consumes the session id; every
        rejection (unknown or consumed id, a claim lost to a concurrent
        check, unequal binding, digest mismatch, expiry, a negative leaf
        integer, or the delegated verification returning ``False``)
        returns ``False``, releases the claim and leaves the registration
        pending, so a rejected id can still succeed once later. An
        exception escaping the delegated verification likewise releases
        the claim and then propagates unchanged, leaving the id usable.
        With a store backend, only the holder of the current claim token
        can consume the id (an expired claim may be taken over by a later
        ...
        if now is None:
            ...
            current = now
        if not _single_key_entry_replay_encodable(entry):
            return False  # negative leaf integers cannot match a framed digest
        session_id = binding.session_id
        if self._store is not None:
            return self._check_with_store(entry, binding, session_id, current)
        if not self._registry.claim(session_id, binding):

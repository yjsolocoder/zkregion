...
class SQLiteReplayStore:
    ...
    :class:`MerkleConsistencyReplayGuard` and
...
# ---------------------------------------------------------------------------
# Single-inclusion replay protection for compact multi-inclusion proofs
#
...
# other guards. Like them the binding is single-use: without a store the
# state is local to the guard instance, while an SQLiteReplayStore keeps
# pending, claimed and consumed ids in the store under b"zr/mmr/v1", shared
# across instances, processes and restarts. bind_once registers a pending
# binding, check recomputes the digest, checks the expiry, delegates the
# multi-inclusion verification to verify_multi_inclusion and consumes the id
# only on full success; every rejection leaves the id untouched.
#
# digest = SHA-256(
#     F(D) || F(session_id) || F(root) || F(U(proof.leaf_count))
#     || S(proof.indices, U)
...
def _merkle_multi_replay_encodable(
    ...
    The proof ``leaf_count``, every proof index and every entry index are
    encoded with ``U`` (the two ``S`` sequence lengths too); the raw root,
    leaf and sibling bytes need no encodability rule.
    """
    values = [proof.leaf_count, *proof.indices]
    values.extend(index for index, _leaf in entries)
    return all(0 <= value <= _UINT64_MAX for value in values)
...
def _merkle_multi_replay_digest(
    ...
    """Compute the Merkle-multi replay binding digest.

    Writes, in order: ``F(D)``, ``F(session_id)``, ``F(root)``,
    ``F(U(proof.leaf_count))``, ``S(proof.indices, U)``,
    ``S(entries, Q)``, ``S(proof.siblings, id)`` and ``F(E)``; entries keep
    their order, one ``(index, leaf)`` per ``proof.indices`` item, and the
    siblings keep their level-by-level proof order.
    """
    transcript = hashlib.sha256()
    ...
    # Q((index, leaf)) = F(U(index)) || F(leaf), itself wrapped by S's F
    transcript.update(_frame_length_prefixed(_uint64_be(len(entries))))
    for index, leaf in entries:
        transcript.update(
            _frame_length_prefixed(
                _frame_length_prefixed(_uint64_be(index))
                + _frame_length_prefixed(leaf)
            )
        )
    # S(proof.siblings, id) = F(U(|siblings|)) || Σ F(sibling)
    transcript.update(_frame_length_prefixed(_uint64_be(len(proof.siblings))))
...
def _check_merkle_multi_proof_types(
    entries: object,
    root: object,
    proof: object,
) -> None:
    """Validate multi-inclusion argument types for the replay guard.

    Mirrors the type checks of :func:`verify_multi_inclusion`: ``root`` must
    be ``bytes``, ``proof`` must be a :class:`MerkleMultiProof` with a
    non-``bool`` integer ``leaf_count``, a tuple of non-``bool`` integer
    ``indices`` and a tuple-of-bytes ``siblings``, and ``entries`` must be a
    non-string sequence of ``(index, leaf)`` pairs with a non-``bool``
    integer index and ``bytes`` leaf. Structural and value problems (digest
    lengths, ordering, range, count mismatch) are left to
...
class MerkleMultiReplayGuard:
    ...
    :meth:`check` accepts an equal pending binding exactly once, recomputing
    the binding digest, checking the expiry and delegating to
    :func:`verify_multi_inclusion`, and then marks the id consumed. The
    digest frames the proof under
    ``SHA-256(F(D) || F(session_id) || F(root) || F(U(leaf_count)) ||``
    ``S(indices, U) || S(entries, Q) || S(siblings, id) || F(E))`` with
    domain ``b"zr/mmr/v1"``, where ``Q((index, leaf))`` is
    ``F(U(index)) || F(leaf)``, reusing the F / U / S framing and the E
    expiry encoding of the Bound and consistency guards byte for byte. By
    ...
    :class:`SQLiteReplayStore` as ``store`` instead keeps the state in that
    store under the ``b"zr/mmr/v1"`` key domain, so multi guards attached to
    the same store namespace share pending, claimed and consumed ids across
    independent instances, processes and process restarts.
    ...
    def bind_once(
        ...
        *,
        ...
        Returns the frozen :class:`ReplayBinding`. ``root`` and
        ``session_id`` must be ``bytes`` (only the session id is required to
        be non-empty), ``proof`` must be a :class:`MerkleMultiProof` whose
        ``leaf_count`` and ``indices`` are non-``bool`` integers and whose
        ``siblings`` is a tuple of ``bytes``, ``entries`` must be a sequence
        of ``(index, leaf)`` pairs in the exact order of ``proof.indices``,
        and ``expires_at`` must be either ``None`` or a non-``bool`` unsigned
        64-bit Unix-second timestamp; the U-framed values
        (``proof.leaf_count`` and every proof/entry index) must likewise fit
        in uint64. A session id that is already pending, being checked or
        consumed raises :class:`ValueError`. Wrong argument or nested field
        types raise :class:`TypeError`; an empty id, an out-of-uint64 expiry
        or framed value, or a rebind raise :class:`ValueError`. Inputs are
        never mutated.
        """
        _check_merkle_multi_proof_types(entries, root, proof)
        _check_bytes(session_id, "session_id")
        if not session_id:
            raise ValueError("session_id must not be empty")
        if expires_at is not None:
            _check_uint64(expires_at, "expires_at")
        if not _merkle_multi_replay_encodable(entries, proof):
            raise ValueError(
                "leaf_count and indices must be unsigned 64-bit integers"
            )
        binding = ReplayBinding(
            session_id,
            _merkle_multi_replay_digest(
                entries, root, proof, session_id, expires_at
            ),
            expires_at,
        )
        if self._store is not None:
    ...
    def check(
        ...
        *,
        ...
        :func:`verify_multi_inclusion`, which hashes the leaves, rebuilds the
        root level by level from the compact siblings and compares it.

        Only a fully successful check consumes the session id; every
        rejection (unknown or consumed id, a claim lost to a concurrent
        check, unequal binding, digest mismatch, expiry, or
        :func:`verify_multi_inclusion` returning ``False``) returns
        ``False``, releases the claim and leaves the registration pending. An
        exception escaping the delegated verification likewise releases the
        claim and then propagates unchanged, leaving the id usable. With a
        store backend, only the holder of the current claim token can consume
        ...
        if now is None:
            ...
            current = now
        if not _merkle_multi_replay_encodable(entries, proof):
            return False  # negative or oversized U-framed indices
        session_id = binding.session_id
        if self._store is not None:
            return self._check_with_store(
                entries, root, proof, binding, session_id, current
            )
        if not self._registry.claim(session_id, binding):
            return False  # unknown id, already consumed, claimed, or unequal binding
        committed = False

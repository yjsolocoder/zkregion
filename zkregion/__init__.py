...
# SingleKeyBatchGuard binds a whole batch of single-key SchnorrBatchEntry
# items — all made for one fixed public key and group the guard is
# constructed with — to one session id, reusing the ReplayBinding type and,
# byte for byte, the F / U / S framing and the E expiry encoding of the
# other batch guards. Like them the binding is single-use: without a store
# the state is local to the guard instance, while an SQLiteReplayStore keeps
# pending, claimed and consumed ids in the store under b"zr/skbr/v1", shared
# across instances, processes and restarts. bind_once registers a pending
# binding, check claims the id atomically, recomputes the digest, checks the
# expiry, delegates the batch verification to the guard's fixed
# SchnorrVerifier.verify_batch with randbelow passed through unchanged and
# consumes the id only on full success; every rejection leaves the id
# untouched.
#
# digest = SHA-256(
...
#   F(x) = four-byte unsigned big-endian length prefix of x, followed by x
#   U(n) = eight-byte unsigned big-endian encoding of n
#   L(e) = the existing BoundSchnorr leaf raw bytes (_bound_schnorr_leaf)
#          built for the MultiSchnorrEntry that pairs e's message, proof and
#          context with the guard's own public_key, prime and generator
#   S(a, f) = F(U(|a|)) || Σ F(f(a_i))
#   E is the same expiry encoding as the other guards:
#     b"\x00" when expires_at is None, b"\x01" + uint64be(expires_at) otherwise
...
def _check_schnorr_batch_entry(entry: object, name: str = "entry") -> SchnorrBatchEntry:
    """Validate one single-key batch entry and its nested field types."""
    if not isinstance(entry, SchnorrBatchEntry):
...
def _check_single_key_batch_entries_types(
    entries: object,
) -> list[SchnorrBatchEntry]:
    """Validate the single-key batch ``entries`` argument types.
    ...
    for position, entry in enumerate(entries):
        items.append(_check_schnorr_batch_entry(entry, f"entries[{position}]"))
    return items
...
def _single_key_batch_replay_encodable(entries: Sequence[SchnorrBatchEntry]) -> bool:
    ...
    The outer ``S`` sequence writes the batch count with ``U`` (eight-byte
    unsigned big-endian), and each entry is framed as the existing
    BoundSchnorr leaf, whose integers (the guard's ``prime``, ``generator``
    and ``public_key`` — all positive by construction — and the proof
    commitment / response) use the shortest unsigned big-endian encoding; a
    count outside uint64 or a negative leaf integer cannot be framed.
    """
    if not 0 <= len(entries) <= _UINT64_MAX:
        return False
    for entry in entries:
...
def _single_key_batch_replay_digest(
    ...
    ``F(E)``; ``L(entry)`` is the existing BoundSchnorr leaf raw bytes built
    with the guard's own public key and group parameters, and the entries
    keep their batch order (duplicates included).
    """
    transcript = hashlib.sha256()
...
class SingleKeyBatchGuard:
    """Single-use replay protection for a single-key :class:`SchnorrBatchEntry` batch.

    The guard is constructed with one fixed ``public_key`` and group
    (``prime`` / ``generator``) and keeps a fixed :class:`SchnorrVerifier`
    for them. A fresh guard has no registrations. :meth:`bind_once`
    registers a pending :class:`ReplayBinding` that commits a whole
    non-empty batch of :class:`SchnorrBatchEntry` items — the same sequence
    shape accepted by :meth:`SchnorrVerifier.verify_batch`, kept in the
    given order with duplicates preserved — to a session id; :meth:`check`
    accepts an equal pending binding exactly once, recomputing the binding
    digest, checking the expiry and delegating to the fixed verifier's
    ...
    ``b"zr/skbr/v1"``, where ``L(entry)`` is the existing BoundSchnorr leaf
    raw bytes (the same bytes :func:`_bound_schnorr_leaf` builds for
    :func:`verify_bound`) of the :class:`MultiSchnorrEntry` that pairs the
    entry's ``message``, ``proof`` and ``context`` with the guard's own
    ``public_key``, ``prime`` and ``generator``; the F / U / S framing and
    the E expiry encoding are reused byte for byte from the other replay
    ...
    :class:`SQLiteReplayStore` as ``store`` instead keeps the state in that
    store under the ``b"zr/skbr/v1"`` key domain, so guards attached to the
    ...
    def bind_once(
        ...
        *,
        ...
        objects following the same sequence and nested type rules as
        :meth:`SchnorrVerifier.verify_batch`: every entry's ``message`` and
        ...
        ``U``-framed batch length must fit in uint64 and every BoundSchnorr
        leaf integer (the proof commitment / response; the guard's group
        values are positive by construction) must be non-negative. A
        ...
        Inputs are never mutated.
        """
        items = _check_single_key_batch_entries_types(entries)
        _check_bytes(session_id, "session_id")
        if not items:
            raise ValueError("entries must not be empty")
        if not session_id:
    ...
    def check(
        ...
        *,
        ...
        anything. Verification runs without any lock or transaction held,
        so other ids are never serialized. Argument type errors (including
        a non-callable ``randbelow``) raise :class:`TypeError`; an
        out-of-range ``now`` raises :class:`ValueError`. Inputs are never
        mutated.
        """
        items = _check_single_key_batch_entries_types(entries)

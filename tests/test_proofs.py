...
class MerkleInclusionReplayGuardTest(unittest.TestCase):
    ...
    def _inclusion(self, index=2, size=5):
        leaves = self._leaves(size)
        return leaves[index], merkle_root(leaves), prove_inclusion(leaves, index)
    ...
    def test_digest_binds_every_component(self):
        ...
        self.assertNotEqual(base.digest, bind(s=b"other"))
        # the leaf binds
        self.assertNotEqual(base.digest, bind(l=b"other"))
        # the root binds
        self.assertNotEqual(base.digest, bind(r=b"\x7f" * 32))
        # the index binds
        self.assertNotEqual(
            base.digest, bind(p=MerkleProof(proof.index + 1, proof.siblings))
        )
        # a sibling binds
    ...
    def test_bind_nested_type_errors(self):
        ...
        bad_proofs = [
            ...
            MerkleProof("2", proof.siblings),
            MerkleProof(2.0, proof.siblings),
    ...
    def test_bind_value_errors(self):
        ...
        huge = 2**64
        for oversized in (
            MerkleProof(huge, proof.siblings),
    ...
    def test_wrong_components_rejected_without_consuming(self):
        leaf, root, proof = self._inclusion()
        other_leaf, other_root, other_proof = self._inclusion(index=0, size=7)
        guard = MerkleInclusionReplayGuard()
        binding = guard.bind_once(leaf, root, proof, b"s")
        # a different leaf, root or proof all miss the digest
        self.assertFalse(guard.check(other_leaf, root, proof, binding, now=1))
        self.assertFalse(guard.check(leaf, other_root, proof, binding, now=1))
        self.assertFalse(guard.check(leaf, root, other_proof, binding, now=1))
        self.assertIn(b"s", guard._pending)
        self.assertTrue(guard.check(leaf, root, proof, binding, now=1))
    ...
    def test_store_domain_isolation_from_other_guards(self):
        leaf, root, proof = self._inclusion()
        leaves = self._leaves(7)
        old_root, new_root = merkle_root(leaves[:3]), merkle_root(leaves)
        ...
        inclusion_binding = inclusion_guard.bind_once(leaf, root, proof, b"same-id")
        self.assertTrue(
    ...
    def test_store_rejection_restores_pending_for_other_instance(self):
        ...
        checker = MerkleInclusionReplayGuard(store=store)
        self.assertFalse(
            checker.check(other_leaf, other_root, other_proof, binding, now=1)
        )
        self.assertIn(b"s", checker._pending)
        self.assertTrue(

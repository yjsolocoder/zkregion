...
- `OpeningReplayGuard(*, store=None)` — 单条哈希开合 `OpeningBatchEntry`（沿用批验条目形状）的防重放登记册（线程安全）；无参时待用/已消费状态隔离在本实例内存中，传入 `SQLiteReplayStore` 时待用/认领/已消费状态落在该存储中（行键域为 `b"zr/orr/v1"`），同文件同命名空间的独立实例（含重启后、跨进程）共享状态；摘要为 `SHA-256(F(D) || F(session_id) || L(entry) || F(E))`，其中 `D = b"zr/orr/v1"`、`L(entry)` 逐字节复用哈希开合完整批的外层叶原字节（`_bound_opening_leaf`，即 `F(b"zkregion/ob/v1") || F(commitment) || F(value) || F(nonce)`），F/E 逐字节沿用各单条守卫
  - `bind_once(entry: OpeningBatchEntry, session_id, *, expires_at=None) -> ReplayBinding` — 把单条条目（沿用 `verify_opening_batch` 的单条目嵌套类型规则）一次性绑定到 `session_id`；空 id、uint64 越界的期限或待用/校验中（存储后端下含过期认领）/已消费 id 重绑抛 `ValueError`，错型抛 `TypeError`
  - `check(entry, binding, *, now=None) -> bool` — 先原子认领等值待用绑定（存储后端可在认领租约过期后接管旧认领），再重算摘要、检查期限，随后原样委托 `verify_opening(entry.commitment, entry.value, entry.nonce)`；开合校验通过才消费 `session_id`，未登记、已消费、绑定不等、摘要不符、过期或开合不过一律返回 `False` 且撤销认领、复原待用（被拒标识稍后仍可成功一次）；委托异常先复原认领再原样透传；存储后端下只有持有当前认领 token 的一方能消费，`sqlite3.Error` 原样透传
...
`OpeningReplayGuard(*, store=None)` 为单条 `OpeningBatchEntry`（沿用批验条目形状，字段次序即 `verify_opening` 入参）提供与各单条守卫同构的一次性会话绑定，复用同一个 `ReplayBinding` 类型；`bind_once` 的入参为 `(entry, session_id, *, expires_at=None)`，只登记待用、不验开合。无参构造时待用/已消费状态隔离在本实例内存中，传入 `SQLiteReplayStore` 时待用/认领/已消费状态落在该存储中（行键域为 `b"zr/orr/v1"`），同文件同命名空间的独立实例（含重启后、跨进程）共享状态。绑定摘要为

```
digest = SHA-256(F(D) || F(session_id) || L(entry) || F(E))
```

其中域 `D = b"zr/orr/v1"` 同时用作存储后端的行键域段；`F(x)` 是四字节无符号大端长度前缀加 `x`；`L(entry)` 逐字节复用"Merkle 承诺的哈希开合完整批验"一节定义的外层叶原字节（`_bound_opening_leaf`），即 `F(b"zkregion/ob/v1") || F(commitment) || F(value) || F(nonce)`，原样写入、不再外套 `F`——其中 `F(commitment) || F(value) || F(nonce)` 与 `OpeningBatchReplayGuard` 摘要的 `Q(entry)` 逐字节一致；`E` 的过期编码与 `ReplayGuard` 逐字节相同：无期 `E = b"\x00"`，有期 `E = b"\x01" + uint64be(expires_at)`。

`bind_once` 只登记、不验开合。类型错误（`entry` 非 `OpeningBatchEntry`、`commitment`/`value`/`nonce` 非 `bytes`、`session_id` 非 `bytes`、期限错型）抛 `TypeError`；`session_id` 为空、`expires_at` 越出 uint64，或 id 已待用/校验中（存储后端下含过期认领）/已消费均抛 `ValueError`。

`check(entry, binding, *, now=None) -> bool` 的校验次序固定：先原子认领等值待用绑定（存储后端下认领租约过期后可由持等值绑定的检查接管并换发 token），再重算摘要确认提交的条目就是绑定时的条目，再检查期限（`now` 缺省取当前 Unix 秒，显式给出时须为非 `bool` uint64，越界抛 `ValueError`），随后以 `entry.commitment`、`entry.value`、`entry.nonce` 原样委托 `verify_opening`。只有开合校验通过才消费 `session_id`；未登记、已消费、绑定不等、摘要不符、过期或开合不过一律返回 `False` 且撤销认领、保持待用，因此被拒的标识稍后仍可成功一次；委托验证逃出的异常先复原认领再原样透传。并发检查同一标识至多一次成功；认领只是每 id 的登记册状态而非全局锁，某个 id 上的耗时校验不会把其他 id 的检查或登记全程串行阻塞。无参构造时绑定状态不跨实例共享；`check` 的参数类型错误抛 `TypeError`，`store` 参数类型错误抛 `TypeError`。

存储后端下 `OpeningReplayGuard` 沿用 `SQLiteReplayStore` 的短事务认领、租约过期接管与 token 所有权全部机制，仅行键的域段为 `b"zr/orr/v1"`（与同一存储上 `OpeningBatchReplayGuard` 的 `b"zr/obr/v1"`、`BoundOpeningReplayGuard` 的 `b"zr/bobr/v1"` 等行互不冲突）：只有持有当前认领 token 的一方能把 id 置为已消费，过期认领被接管后旧 token 既不能消费也不能复原；数据库错误原样透传 `sqlite3.Error`。守卫的摘要公式、`ReplayBinding` 类型与 `bind_once`/`check` 的参数边界在两种模式下逐字节、逐类型保持一致。入口不改写任何输入。
...
- **实例隔离或共享存储**：无参构造时，待用/校验中/已消费状态只存在于单个守卫实例内，不同实例（即便同 id）完全独立、互不阻塞；传入同一 `SQLiteReplayStore`（同文件、同命名空间）的守卫实例则经由短事务与唯一认领 token 共享三种状态，跨实例、跨进程并在重启后保持一致，认领租约过期后只能由等值 `binding` 的 `check` 接管，不同守卫域（`zr/r/v1`、`zr/sbr/v1`、`zr/skbr/v1`、`zr/rbr/v1`、`zr/rgbr/v1`、`zr/rr/v1`、`zr/rg/v1`、`zr/brg/v1`、`zr/brr/v1`、`zr/bsr/v1`、`zr/skbbr/v1`、`zr/mccr/v1`、`zr/mccbr/v1`、`zr/mcr/v1`、`zr/mir/v1`、`zr/mibr/v1`、`zr/mmr/v1`、`zr/mmb/v1`、`zr/mcbr/v1`、`zr/bccbr/v1`、`zr/bmmbr/v1`、`zr/bmibr/v1`、`zr/bobr/v1`、`zr/pbobr/v1`、`zr/rcbr/v1`、`zr/brcbr/v1`）的行互不冲突。

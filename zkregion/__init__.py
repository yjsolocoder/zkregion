...
# ---------------------------------------------------------------------------
# Replay protection for Merkle-committed complete consistency batches
#
...
# separator and the per-entry leaves differ. The bound leaf L is the
# existing consistency-bound leaf raw bytes (_bound_consistency_leaf), i.e.
# the same bytes verify_consistency_batch_bound commits under the Merkle
# root. Like the other bound guards the binding is single-use: without a
# store the state is local to the guard instance, while an
# SQLiteReplayStore keeps pending, claimed and consumed ids in the store
# under b"zr/bcbr/v1", shared across instances, processes and restarts.
# bind_once registers a pending binding, check recomputes the digest,
# checks the expiry, delegates to verify_consistency_batch_bound and
# consumes the id only on full success; every rejection leaves the id
# untouched.
#
# digest = SHA-256(
...
#   U(n) = eight-byte unsigned big-endian encoding of n
#   S(a, f) = F(U(|a|)) || Σ F(f(a_i))
#   L(entry) = the existing consistency-bound leaf bytes
#              (_bound_consistency_leaf), raw under F
#   E is the same expiry encoding as the other guards:
#     b"\x00" when expires_at is None, b"\x01" + uint64be(expires_at) otherwise
...
def _bound_consistency_replay_encodable(batch: BoundConsistencyBatch) -> bool:
    ...
    ``batch.leaf_count``, ``batch.proof.leaf_count`` and every proof index
    are written with ``U`` (eight-byte unsigned big-endian); a negative or
    larger-than-uint64 value cannot be framed. The consistency-bound leaves
    (roots, decimal-ASCII counts and proof nodes), the raw sibling bytes
    and the claimed root encode for any value, so no additional
...
def _bound_consistency_replay_digest(
    ...
    Byte-for-byte the :func:`_bound_region_replay_digest` framing with the
    BoundConsistency replay domain and the consistency-bound leaves
    (:func:`_bound_consistency_leaf`) rather than the BoundRegion leaves.
    Writes, in order: ``F(D)``, ``F(session_id)``, ``F(root)``,
    ``F(U(batch.leaf_count))``, one ``F(L(entry))`` per entry in batch
    order (entries order and duplicates preserved), then
    ``F(U(proof.leaf_count))``, ``S(proof.indices, U)``,
    ``S(proof.siblings, identity)`` and ``F(E)``.
    """
    transcript = hashlib.sha256()
    ...
    transcript.update(_frame_length_prefixed(root))
    transcript.update(_frame_length_prefixed(_uint64_be(batch.leaf_count)))
    for entry in batch.entries:  # entries order, each consistency-bound leaf under F
...
def _check_bound_consistency_batch_types(batch: object, root: object) -> None:
    ...
    batch must be a :class:`BoundConsistencyBatch` whose entries tuple
    holds nestedly well-typed :class:`MerkleConsistencyBatchEntry` objects,
    whose ``leaf_count`` is a non-``bool`` integer and whose proof is a
    well-typed :class:`MerkleMultiProof`; ``root`` must be ``bytes``.
    Structural and value problems (coverage, root commitment, leaf
    validity, the consistency proofs themselves) are left to
...
class BoundConsistencyReplayGuard:
    ...
    the id consumed. The digest reuses the F / U / S framing and the E
    expiry encoding of :class:`BoundRegionReplayGuard` under the
    ``b"zr/bcbr/v1"`` key domain, framing the existing consistency-bound
    leaf bytes (:func:`_bound_consistency_leaf`) in entries order. By
    ...
    that store under the ``b"zr/bcbr/v1"`` key domain, so bound-
    consistency guards attached to the same store namespace share
    pending, claimed and consumed ids across independent instances,
    processes and process restarts.

    Concurrent checks of the same id are decided by an atomic claim taken
    before the (potentially slow) delegated bound-batch verification (the
    in-instance registry lock, or the store's short transaction and unique
    claim token); the claim is per-id registry state rather than a global
    ...
    def bind_once(
        ...
        *,
        ...
        non-empty ``bytes`` and ``expires_at`` must be either ``None`` or a
        non-``bool`` unsigned 64-bit Unix-second timestamp; the U-framed
        integers (``leaf_count``, ``proof.leaf_count`` and the proof
        indices) must likewise fit in uint64. A session id that is already
        pending, being checked or consumed raises :class:`ValueError`.
        Wrong argument or nested field types raise :class:`TypeError`; an
        empty id, an out-of-uint64 expiry or framed integer, or a rebind
    ...
    def check(
        ...
        *,
        ...
        structure and leaf commitment contract.
        ...
        try:
            if not hmac.compare_digest(
                binding.digest,
                _bound_consistency_replay_digest(

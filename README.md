...
- `SchnorrVerifier(public_key, *, prime, generator)`
  ...
  - `prove_bound_batch(entries, *, randbelow=secrets.randbelow) -> tuple[SingleKeyBoundBatch, bytes]` — 规范构造 Merkle 承诺的同一公钥 Schnorr 完整批：公钥与群参数沿用本验证器固定值，`entries` 沿用 `verify_batch` 的全批嵌套类型规则（非 `bytes`/`bytearray`/`str` 序列的 `SchnorrBatchEntry`）且必须非空，完整预检后按原顺序转为元组并保留重复项，输入不变；每项按 `verify_bound_batch` 同一方式提升为携带本验证器公钥与群参数的 `MultiSchnorrEntry`，外层叶逐字节复用 `_bound_schnorr_leaf` 编码，既有域、长度帧、字段顺序及 Merkle 哈希规则不变；令 `n = len(entries)`，对编码叶按全索引 `tuple(range(n))` 调用 `prove_multi_inclusion` 得到完整多包含证明（`indices` 覆盖每片叶、`siblings` 为空），返回批的 `leaf_count = n`、`proof` 为该证明，第二返回值为编码叶的 `merkle_root`；返回批满足 `verify_bound_batch(batch, root) is True`，单项、奇偶批与重复项均确定，与旧手工构造逐字节兼容；`randbelow` 原样透传给 `verify_batch`，随机源自身异常按既有边界原样透传；全批或任一嵌套字段错型（含后项错型与 `bool` 计数）抛 `TypeError`，空批、U 成帧批次数越出 uint64、叶内负整数或 `verify_batch` 返回 `False`（内层批验不通过）抛 `ValueError`
...
- `prove_schnorr_batch_bound(entries, *, randbelow=secrets.randbelow) -> tuple[BoundSchnorrBatch, bytes]` — 顶层构造 Merkle 承诺的规范多公钥 Schnorr 完整批：`entries` 沿用 `verify_schnorr_batch` 的全批嵌套类型规则（非 `bytes`/`bytearray`/`str` 序列的 `MultiSchnorrEntry`）且必须非空，完整预检后按原顺序转为元组并保留重复项，输入不变；每项外层叶逐字节复用 `_bound_schnorr_leaf` 编码，既有域、长度帧、字段顺序及 Merkle 哈希规则不变；令 `n = len(entries)`，对编码叶按全索引 `tuple(range(n))` 调用 `prove_multi_inclusion` 得到完整多包含证明（`indices` 覆盖每片叶、`siblings` 为空），返回批的 `leaf_count = n`、`proof` 为该证明，第二返回值为编码叶的 `merkle_root`；返回批满足 `verify_bound(batch, root) is True`，单项、奇偶批与重复项均确定，与旧手工构造逐字节兼容；`randbelow` 原样透传给 `verify_schnorr_batch`，随机源自身异常按既有边界原样透传；全批或任一嵌套字段错型（含后项错型与 `bool` 计数）抛 `TypeError`，空批、U 成帧批次数越出 uint64、叶内负整数或 `verify_schnorr_batch` 返回 `False`（内层批验不通过）抛 `ValueError`

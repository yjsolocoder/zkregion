...
class SQLiteReplayStore:
    ...
    kinds without their ids colliding (see also
    :class:`MerkleConsistencyBatchReplayGuard` and
...
# verify_schnorr_batch) to a caller-chosen session id, reusing the
# ReplayBinding type and, byte for byte, the F / U / S framing and the E
# expiry encoding of the Bound guards. Like the other guards the binding is
# single-use: without a store the state is local to the guard instance,
# while an SQLiteReplayStore keeps pending, claimed and consumed ids in the
# store under b"zr/sbr/v1", shared across instances, processes and
...
# verification to verify_schnorr_batch with the random source passed
# through unchanged and consumes the id only on full success; every
# rejection leaves the id untouched.
#
# digest = SHA-256(
#     F(D) || F(session_id) || S(entries, Q) || F(E)
# )
#   Q(e) = the existing BoundSchnorr leaf bytes for entry e (see
#          _bound_schnorr_leaf), raw
#   D = b"zr/sbr/v1"
#   F(x) = four-byte unsigned big-endian length prefix of x, followed by x
#   U(n) = eight-byte unsigned big-endian encoding of n
#   S(a, f) = F(U(|a|)) || Σ F(f(a_i))
...
#     b"\x00" when expires_at is None, b"\x01" + uint64be(expires_at) otherwise
#
# The batch keeps its given order, including duplicates; entries are never
# dropped or reordered.


def _check_schnorr_batch_entries_types(entries: object) -> list[MultiSchnorrEntry]:
    """Validate the multi-key Schnorr batch ``entries`` argument types.

    Mirrors the type checks of :func:`verify_schnorr_batch`: ``entries``
    must be a non-``bytes`` / ``bytearray`` / ``str`` sequence of
    :class:`MultiSchnorrEntry` objects whose nested fields are well typed
    (``bytes`` messages and contexts, a :class:`SchnorrProof` with
    non-``bool`` integer commitment / response and non-``bool`` integer
    ``public_key`` / ``prime`` / ``generator``). The entries are copied
    into a fresh list so the inputs are never mutated; an empty batch is
    left for :meth:`SchnorrBatchReplayGuard.bind_once` to reject with
    :class:`ValueError`, and structural/value problems (group parameters,
    key and proof ranges) are left to :func:`verify_schnorr_batch` at
    check time.
    """
    if isinstance(entries, (bytes, bytearray, str)) or not isinstance(entries, Sequence):
        raise TypeError("entries must be a sequence of MultiSchnorrEntry")
...
def _schnorr_batch_replay_encodable(entries: Sequence[MultiSchnorrEntry]) -> bool:
    """The batch length must fit in U and every BoundSchnorr leaf integer.

    The outer ``S`` writes the batch length with ``U`` (eight-byte unsigned
    big-endian); each ``Q`` leaf encodes its five integers (``prime``,
    ``generator``, ``public_key`` and the proof commitment / response)
    with the shortest unsigned big-endian encoding, so a negative integer
    cannot be framed either. The raw message and context bytes encode for
    any value.
    """
...
def _schnorr_batch_replay_digest(
    ...
    Writes, in order: ``F(D)``, ``F(session_id)``, ``S(entries, Q)`` and
    ``F(E)``; the entries keep their batch order (duplicates included) and
    ``Q`` is the existing BoundSchnorr leaf bytes, framed raw under ``F``.
    """
    transcript = hashlib.sha256()
...
class SchnorrBatchReplayGuard:
    """Single-use replay protection for a multi-key Schnorr batch.

    A fresh guard has no registrations. :meth:`bind_once` registers a
    pending :class:`ReplayBinding` that commits a whole non-empty batch of
    :class:`MultiSchnorrEntry` items — the same sequence shape accepted by
    :func:`verify_schnorr_batch` — to a session id; :meth:`check` accepts
    an equal pending binding exactly once, claiming the id atomically
    first, then recomputing the binding digest, checking the expiry and
    delegating to :func:`verify_schnorr_batch`, and finally marks the id
    consumed. The digest frames the batch under
    ``SHA-256(F(D) || F(session_id) || S(entries, Q) || F(E))`` with domain
    ``b"zr/sbr/v1"``, where ``Q`` is the existing BoundSchnorr leaf bytes
    for each entry in the exact given batch order; the F / U / S framing
    and the E expiry encoding are reused byte for byte from the Bound and
    other batch guards. By default both the pending and the consumed state
    ...
    state in that store under the ``b"zr/sbr/v1"`` key domain, so batch
    ...
    def bind_once(
        ...
        *,
        ...
        objects with the same nested field types
        :func:`verify_schnorr_batch` requires, kept in the given order with
        duplicates preserved and no item dropped; ``session_id`` must be
        non-empty ``bytes`` and ``expires_at`` must be either ``None`` or a
        non-``bool`` unsigned 64-bit Unix-second timestamp. Every
        BoundSchnorr leaf integer (``prime``, ``generator``,
        ``public_key`` and the proof commitment / response) must likewise
        be non-negative. A session id that is already pending, being
        ...
        out-of-uint64 expiry, a negative leaf integer, or a rebind raise
        :class:`ValueError`. Inputs are never mutated.
        """
    ...
    def check(
        ...
        *,
        ...
        unchanged, under that function's structure and randomness
        contract: it is called once per structurally valid entry as
        ``randbelow(prime - 1)``.

        Only a fully successful check consumes the session id; every
        rejection (unknown or consumed id, a claim lost to a concurrent
        check, unequal binding, digest mismatch, an empty batch, a
        negative leaf integer, expiry, or
        :func:`verify_schnorr_batch` returning ``False``) returns
        ``False``, releases the claim and leaves the registration pending.
        An exception escaping the delegated verification (such as the
        :class:`TypeError` raised by a non-callable ``randbelow`` or one
        that returns a non-integer, or the :class:`ValueError` raised by a
        coefficient outside ``[0, prime - 1)``) likewise releases the claim
        and then propagates unchanged, leaving the id usable. With a store
        backend, only the holder of the current claim token can consume the
        id (an expired claim may be taken over by a later equal
        ``check``); a stale token neither consumes nor restores anything.
        Verification runs without any lock or transaction held, so other
        ids are never serialized. Argument type errors (including a
        non-callable ``randbelow`` and ``bool`` integers) raise
        :class:`TypeError`; an out-of-range ``now`` raises
        :class:`ValueError`. Inputs are never mutated.
        """
        items = _check_schnorr_batch_entries_types(entries)
        ...
        if now is None:
            ...
            _check_uint64(now, "now")
            current = now
        if not items or not _schnorr_batch_replay_encodable(items):
            return False  # an empty batch or negative leaf integers
        session_id = binding.session_id
        if self._store is not None:

...
class SQLiteReplayStore:
    ...
    :class:`BoundOpeningReplayGuard` and
...
#
# A BoundOpeningReplayGuard binds a whole BoundOpeningBatch together with the
# Merkle root it is claimed under to a caller-chosen session id, reusing the
# ReplayBinding type and, byte for byte, the F / U / S framing, the E expiry
# encoding and the SHA-256 formula of the other Bound guards; only the domain
# separator and the per-item leaves differ. Like them the binding is
# single-use: without a store the state is local to the guard instance, while
# an SQLiteReplayStore keeps pending, claimed and consumed ids in the store
# under b"zr/bobr/v1", shared across instances, processes and restarts.
# bind_once registers a pending binding, check claims the id atomically,
# recomputes the digest, checks the expiry, delegates the outer-root and inner
# batch verification to verify_opening_batch_bound and consumes the id only on
# full success; every rejection leaves the id untouched.
#
# digest = SHA-256(
#     F(D) || F(session_id) || F(root) || F(U(batch.leaf_count))
...
#   U(n) = eight-byte unsigned big-endian encoding of n
#   S(a, f) = F(U(|a|)) || Σ F(f(a_i))
#   L(entry) = the complete hash-opening batch outer-leaf bytes
#             (_bound_opening_leaf), raw under F — the existing
#             b"zkregion/ob/v1" outer-leaf encoding, i.e.
#             F(b"zkregion/ob/v1") || Q(entry) with Q(entry) the
#             OpeningBatchReplayGuard framing F(commitment) || F(value)
#             || F(nonce)
#   E is the same expiry encoding as the other guards:
#     b"\x00" when expires_at is None, b"\x01" + uint64be(expires_at) otherwise
#
# The entries keep their batch order, duplicates included; entries are never
# dropped or reordered, and the outer MerkleMultiProof's three fields are all
# bound verbatim.


def _bound_opening_batch_replay_encodable(batch: BoundOpeningBatch) -> bool:
    """Every U-framed integer must fit in unsigned 64 bits.

    The outer transcript frames ``batch.leaf_count``,
    ``batch.proof.leaf_count`` and every outer proof index with ``U``. The
    per-item leaf ``L(entry)`` frames only raw bytes (its ``commitment``,
    ``value`` and ``nonce``), so a negative or larger-than-uint64 value in
    the outer fields is the only framing failure possible.
    """
    proof = batch.proof
    values = [batch.leaf_count, proof.leaf_count, *proof.indices]
    return all(0 <= value <= _UINT64_MAX for value in values)
...
def _bound_opening_batch_replay_digest(
    batch: BoundOpeningBatch,
    root: bytes,
    session_id: bytes,
    expires_at: int | None,
    ...
    ``S(proof.siblings, identity)`` and ``F(E)``. This is byte for byte
    the :func:`_bound_consistency_replay_digest` formula with the domain
    ``b"zr/bobr/v1"`` and :func:`_bound_opening_leaf` bytes as the
    per-item leaves.
    """
    transcript = hashlib.sha256()
...
class BoundOpeningReplayGuard:
    """Single-use replay protection for a Merkle-committed opening batch.
    ...
    id consumed. By default both the pending and the consumed state live
    on this guard instance and are never shared between instances;
    passing an :class:`SQLiteReplayStore` as ``store`` instead keeps the
    state in that store under the ``b"zr/bobr/v1"`` key domain, so
    bound-opening guards attached to the same store namespace share
    pending, claimed and consumed ids across independent instances,
    processes and process restarts.
    ...
    def bind_once(
        ...
        *,
        ...
        be either ``None`` or a non-``bool`` unsigned 64-bit Unix-second
        timestamp; every U-framed integer (the outer ``leaf_count`` /
        ``proof.leaf_count`` / proof indices) must likewise fit in uint64.
        A session id that is already pending, being checked or consumed
        raises :class:`ValueError`. Wrong argument or nested field types
        raise :class:`TypeError`; an empty id, an out-of-uint64 expiry or
        framed integer, or a rebind raise :class:`ValueError`. Inputs are
        never mutated.
        """
        ...
        if expires_at is not None:
            _check_uint64(expires_at, "expires_at")
        if not _bound_opening_batch_replay_encodable(batch):
            raise ValueError("leaf_count, indices and framed counts must be unsigned 64-bit integers")
        binding = ReplayBinding(
            session_id,
            _bound_opening_batch_replay_digest(
                batch, root, session_id, expires_at
            ),
            expires_at,
        )
...
# BoundPedersenOpeningBatch together with the Merkle root it is claimed under
# to a caller-chosen session id, reusing the ReplayBinding type and, byte for
# byte, the F / U / S framing, the E expiry encoding and the SHA-256 formula
# of the other Bound guards; only the domain separator and the per-item leaves
# differ. Like them the binding is single-use: without a store the state is
# local to the guard instance, while an SQLiteReplayStore keeps pending,
# claimed and consumed ids in the store under b"zr/pbobr/v1", shared across
# instances, processes and restarts. bind_once registers a pending binding,
# check claims the id atomically, recomputes the digest, checks the expiry,
# delegates the outer-root and inner batch verification to
# verify_pedersen_opening_batch_bound and consumes the id only on full
# success; every rejection leaves the id untouched.
#
# digest = SHA-256(
#     F(D) || F(session_id) || F(root) || F(U(batch.leaf_count))
...
#   U(n) = eight-byte unsigned big-endian encoding of n
#   S(a, f) = F(U(|a|)) || Σ F(f(a_i))
#   L(entry) = the complete Pedersen-opening batch outer-leaf bytes
...
#             six integer fields and value/blinding as sign-kept decimal
#             ASCII
#   E is the same expiry encoding as the other guards:
#     b"\x00" when expires_at is None, b"\x01" + uint64be(expires_at) otherwise
#
...
def _bound_pedersen_opening_batch_replay_encodable(
    batch: BoundPedersenOpeningBatch,
) -> bool:
    """Every U-framed integer must fit in unsigned 64 bits.
    ...
    The outer transcript frames ``batch.leaf_count``,
    ``batch.proof.leaf_count`` and every outer proof index with ``U``. The
    per-item leaf ``L(entry)`` writes its integer fields as sign-kept
    decimal ASCII rather than ``U``, so every inner value frames for any
    integer and the outer fields are the only encodability gate.
    """
    proof = batch.proof
    values = [batch.leaf_count, proof.leaf_count, *proof.indices]
    return all(0 <= value <= _UINT64_MAX for value in values)
...
def _bound_pedersen_opening_batch_replay_digest(
    batch: BoundPedersenOpeningBatch,
    root: bytes,
    session_id: bytes,
    expires_at: int | None,
    ...
    transcript.update(_frame_length_prefixed(root))
    transcript.update(_frame_length_prefixed(_uint64_be(batch.leaf_count)))
    for entry in batch.entries:  # entries order, each bound Pedersen leaf under F
...
def _check_bound_pedersen_opening_batch_types(batch: object, root: object) -> None:
    ...
    tuple holds nestedly well-typed
    :class:`PedersenOpeningBatchEntry` objects, whose ``leaf_count`` is a
    non-``bool`` integer and whose proof is a well-typed
    :class:`MerkleMultiProof`; ``root`` must be ``bytes``. Structural and
    value problems (coverage, counts, digest lengths) are left to
    :func:`verify_pedersen_opening_batch_bound` at check time.
    """
    if not isinstance(batch, BoundPedersenOpeningBatch):
        raise TypeError("batch must be a BoundPedersenOpeningBatch")
    _check_bytes(root, "root")
...
class BoundPedersenOpeningReplayGuard:
    """Single-use replay protection for a Merkle-committed Pedersen batch.
    ...
    so bound-Pedersen-opening guards attached to the same store namespace
    share pending, claimed and consumed ids across independent instances,
    processes and process restarts.
    ...
    def bind_once(
        ...
        *,
        ...
        raises :class:`ValueError`. Wrong argument or nested field types
        (including ``bool`` integers) raise :class:`TypeError`; an empty
        id, an out-of-uint64 expiry or framed integer, or a rebind raise
        :class:`ValueError`. Inputs are never mutated.
        """
        ...
        if expires_at is not None:
            _check_uint64(expires_at, "expires_at")
        if not _bound_pedersen_opening_batch_replay_encodable(batch):
            raise ValueError("leaf_count, indices and framed counts must be unsigned 64-bit integers")
        binding = ReplayBinding(
            session_id,
            _bound_pedersen_opening_batch_replay_digest(
                batch, root, session_id, expires_at
            ),
            expires_at,
        )
    ...
    def check(
        ...
        *,
        ...
        rejection (unknown or consumed id, a claim lost to a concurrent
        check, unequal binding, digest mismatch, expiry, a non-32-byte root
        or sibling, or :func:`verify_pedersen_opening_batch_bound`
        returning ``False``) returns ``False``, releases the claim and
        leaves the registration pending. An exception escaping the
        delegated verification likewise releases the claim and then
        propagates unchanged, leaving the id usable. With a store backend,
        only the holder of the current claim token can consume the id (an
        expired claim may be taken over by a later equal ``check``); a
        stale token neither consumes nor restores anything. Verification

...
# 独立一致性批次（MerkleConsistencyBatchEntry 序列）与 session 的一次性绑定（可选 SQLite 后端跨实例共享）
...
mcbr_binding = mcbr.bind_once(consistency_batch, b"session-1")
assert mcbr.check(consistency_batch, mcbr_binding)             # 核摘要、期限并批验后消费
assert not mcbr.check(consistency_batch, mcbr_binding)         # 二次提交被拒

store = SQLiteReplayStore(tempfile.mktemp(suffix=".db"))
mcbr_a = MerkleConsistencyBatchReplayGuard(store=store)
mcbr_binding = mcbr_a.bind_once(consistency_batch, b"session-3")
mcbr_b = MerkleConsistencyBatchReplayGuard(store=store)                     # 另一个独立实例
assert mcbr_b.check(consistency_batch, mcbr_binding)                        # 认领、批验并消费
assert not mcbr_a.check(consistency_batch, mcbr_binding)                    # 已消费，二次提交被拒
store.close()
...
- `SQLiteReplayStore(path: str, namespace: bytes = b"default", *, lease_seconds: int = 30, clock=None)` — `ReplayGuard`、`RangeReplayGuard`、`RegionReplayGuard`、`BoundRegionReplayGuard`、`BoundRangeReplayGuard`、`BoundSchnorrReplayGuard`、`MerkleConsistencyBatchReplayGuard`、`MerkleConsistencyChainReplayGuard` 与 `MerkleConsistencyReplayGuard`、`MerkleInclusionReplayGuard`、`MerkleMultiReplayGuard` 的可选 SQLite 后端：同一文件同一命名空间的独立实例（含重启后、跨进程）共享待用、认领与已消费状态；行键为 `namespace`、守卫域（`ReplayGuard` 为 `b"zr/r/v1"`、`RangeReplayGuard` 为 `b"zr/rr/v1"`、`RegionReplayGuard` 为 `b"zr/rg/v1"`、`BoundRegionReplayGuard` 为 `b"zr/brg/v1"`、`BoundRangeReplayGuard` 为 `b"zr/brr/v1"`、`BoundSchnorrReplayGuard` 为 `b"zr/bsr/v1"`、`MerkleConsistencyBatchReplayGuard` 为 `b"zr/mcbr/v1"`、`MerkleConsistencyChainReplayGuard` 为 `b"zr/mccr/v1"`、`MerkleConsistencyReplayGuard` 为 `b"zr/mcr/v1"`、`MerkleInclusionReplayGuard` 为 `b"zr/mir/v1"`、`MerkleMultiReplayGuard` 为 `b"zr/mmr/v1"`）、`session_id` 三段 `bytes`，值保存等值 `ReplayBinding`（摘要与 `E` 期限编码逐字节沿用绑定摘要的编码）及状态、认领 token 与租约截止；`clock` 缺省取整数 Unix 秒，返回值须为非 `bool` uint64，否则 `ValueError`/`TypeError`；`lease_seconds` 须为正的非 `bool` 整数否则 `ValueError`；`store` 参数类型错误抛 `TypeError`；数据库错误原样透传 `sqlite3.Error`
...
- `MerkleConsistencyBatchReplayGuard(*, store=None)` — 独立 Merkle 一致性证明批次（`MerkleConsistencyBatchEntry` 非空序列，沿用 `verify_consistency_batch` 的条目类型与逐对规则，批次及 `nodes` 保序、保留重复项）的防重放登记册（线程安全）；无参时待用/已消费状态隔离在本实例内存中，传入 `SQLiteReplayStore` 时待用/认领/已消费状态落在该存储中（行键域为 `b"zr/mcbr/v1"`），同文件同命名空间的独立实例（含重启后、跨进程）共享状态
  - `bind_once(entries, session_id: bytes, *, expires_at=None) -> ReplayBinding` — 把整批一致性对一次性绑定到 `session_id`；摘要为 `SHA-256(F(D) || F(session_id) || S(entries, Q) || F(E))`，其中 `D = b"zr/mcbr/v1"`、`Q(entry) = F(entry.old_root) || F(entry.new_root) || F(U(proof.old_count)) || F(U(proof.new_count)) || S(proof.nodes, id)`，F/U/S/E 逐字节沿用各 Bound 与其他 Merkle 守卫，批次与各 `nodes` 保持原序、重复项逐对保留；空批、空 id、uint64 越界的期限或 U 值（各 proof 的 `old_count`/`new_count`、各 `nodes` 长度、批长度）或待用/校验中（存储后端下含过期认领）/已消费 id 重绑抛 `ValueError`，entries、条目或嵌套字段错型（含 `bool` 计数）抛 `TypeError`
  - `check(entries, binding, *, now=None) -> bool` — 原子认领、重算绑定摘要、检查期限后委托 `verify_consistency_batch` 按序批验各对；仅完全成功才消费 `session_id`，其余无效（未知/已消费 id、竞争失败、摘要不符、过期、空批或批验返回 `False`）一律返回 `False` 且撤销认领、保持待用，批验抛异常时同样撤销认领并原样透传；存储后端下只有持有当前认领 token 的一方能消费，拒绝或异常时以该 token 把 id 复原为待用（token 已失效则为无操作）
...
`ReplayGuard`、`RangeReplayGuard`、`RegionReplayGuard` 三个单条守卫与 `BoundRegionReplayGuard`、`BoundRangeReplayGuard`、`BoundSchnorrReplayGuard` 三个 Bound 批守卫以及 `MerkleConsistencyBatchReplayGuard`、`MerkleConsistencyChainReplayGuard`、`MerkleConsistencyReplayGuard`、`MerkleInclusionReplayGuard`、`MerkleMultiReplayGuard` 五个 Merkle 守卫的登记册都是线程安全的，且并发语义一致。每个 `session_id` 在一个实例内依次经历三种状态：**待用**（`bind_once` 登记后尚无校验在进行）、**校验中**（某个 `check` 已原子认领，正在执行可能耗时的证明/批验）、**已消费**（校验成功）。
...
- **实例隔离或共享存储**：无参构造时，待用/校验中/已消费状态只存在于单个守卫实例内，不同实例（即便同 id）完全独立、互不阻塞；传入同一 `SQLiteReplayStore`（同文件、同命名空间）的守卫实例则经由短事务与唯一认领 token 共享三种状态，跨实例、跨进程并在重启后保持一致，认领租约过期后只能由等值 `binding` 的 `check` 接管，不同守卫域（`zr/r/v1`、`zr/rr/v1`、`zr/rg/v1`、`zr/brg/v1`、`zr/brr/v1`、`zr/bsr/v1`、`zr/mcbr/v1`、`zr/mccr/v1`、`zr/mcr/v1`、`zr/mir/v1`、`zr/mmr/v1`）的行互不冲突。

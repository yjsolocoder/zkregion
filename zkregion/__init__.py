...
def prove_consistency_batch_bound(
    ...
    :func:`verify_consistency_batch` and must be non-empty; every item is
    copied into a tuple in its original order with duplicates preserved,
    and the inputs are never mutated. Each entry is encoded to its leaf
    byte for byte with :func:`_bound_consistency_leaf`; the leaf digests,
    internal nodes and odd-last-node duplication follow the existing
    SHA-256 Merkle protocol — no domain separator, framing or field order
    changes. With ``n = len(entries)``, the complete multi-inclusion proof
    is built with :func:`prove_multi_inclusion` over the encoded leaves
    and the full indices ``tuple(range(n))`` — so its ``indices`` cover
    every leaf and its ``siblings`` are empty — and the returned batch
    ...
    ``verify_consistency_batch_bound(batch, root)`` returns ``True``.

    A type preflight over the whole batch (including later items and
    ``bool`` counts) raises :class:`TypeError` before anything is built;
    an empty batch or :func:`verify_consistency_batch` returning ``False``
    raises :class:`ValueError`.
    """
...
def prove_consistency_chain_batch_bound(
    ...
    its leaf byte for byte with :func:`_bound_consistency_chain_leaf`; the
    leaf digests, internal nodes and odd-last-node duplication follow the
    existing SHA-256 Merkle protocol — no domain separator, length frame
    or field order changes. With ``n = len(chains)``, the complete
    multi-inclusion proof is built with :func:`prove_multi_inclusion`
    over the encoded leaves and the full indices ``tuple(range(n))`` — so
    its ``indices`` cover every leaf and its ``siblings`` are empty — and
    the returned batch carries ``leaf_count = n`` alongside that proof.
    The second return value is the outer tree's :func:`merkle_root` of
    the encoded leaves, which is exactly the root the batch verifies
    under: ``verify_consistency_chain_batch_bound(batch, root)`` returns
    ``True``.

    A type preflight over the whole batch (including later chains and any
    nested field, with ``bool`` counts rejected) raises
    :class:`TypeError` before anything is built; an empty batch or
    :func:`verify_consistency_chain_batch` returning ``False`` raises
    :class:`ValueError`.
    """

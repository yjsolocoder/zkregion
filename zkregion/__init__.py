...
# ---------------------------------------------------------------------------
# Replay protection for one multi-inclusion batch entry
#
...
# Like them the binding is single-use: without a store the state is local to
# the guard instance, while an SQLiteReplayStore keeps pending, claimed and
# consumed ids in the store under b"zr/mmer/v1", shared across instances,
# processes and restarts. bind_once registers a pending binding, check claims
# the id atomically, recomputes the digest, checks the expiry, delegates the
# single multi-inclusion check to verify_multi_inclusion and consumes the id
...
#   with Q(entry) the continuous run of the multi-inclusion guard transcript
#   from F(root) through S(proof.siblings, id):
#     F(root) || F(U(proof.leaf_count)) || S(proof.indices, U)
...
def _check_merkle_multi_entry(entry: object, name: str = "entry"):
    """Validate one :class:`MerkleMultiBatchEntry` and its nested types.

    Applies the per-item rules of
    :func:`verify_multi_inclusion_batch`: the entry must be a
    :class:`MerkleMultiBatchEntry` whose ``entries`` is a tuple of
    two-item ``(index, leaf)`` pairs with a non-``bool`` integer index
    and ``bytes`` leaf, whose ``root`` is ``bytes`` and whose
    ``proof`` is a :class:`MerkleMultiProof` with a non-``bool``
    integer ``leaf_count``, a tuple of non-``bool`` integer
    ``indices`` and a tuple-of-bytes ``siblings``. Structural and
    value problems (a non-positive ``leaf_count``, empty or
    out-of-order indices, malformed digest lengths, an index out of
    range, mismatched pair/indices counts) are left to
    :func:`verify_multi_inclusion` at check time.
    """
    ...
    for sibling_position, sibling in enumerate(proof.siblings):
        _check_bytes(
            sibling, f"{name} proof siblings[{sibling_position}]"
        )
    return entry


def _merkle_multi_entry_replay_encodable(entry: MerkleMultiBatchEntry) -> bool:
    """Every U-framed count must fit in unsigned 64 bits.

    The ``proof.leaf_count``, the proof ``indices`` / entry
    ``entries`` / ``siblings`` tuple lengths and every proof and entry
    index written inside the outer leaf's ``Q`` run are encoded with
    ``U``; the raw root, sibling and leaf bytes need no encodability
    rule.
    """
    proof = entry.proof
    values = [
...
def _merkle_multi_entry_replay_digest(
    ...
    (:func:`_bound_merkle_multi_leaf`), the same per-item leaf bytes
    that :func:`verify_multi_inclusion_batch_bound` recomputes when
    checking the outer root. The entry's pairs keep their
    proof-indices order and the proof's siblings their
    level-by-level leaf-to-root order.
    """
    transcript = hashlib.sha256()
    transcript.update(
...
class MerkleMultiEntryReplayGuard:
    ...
    A fresh guard has no registrations. :meth:`bind_once` registers a
    pending :class:`ReplayBinding` that commits a single
    :class:`MerkleMultiBatchEntry` — the ``(index, leaf)`` entries it
    places, its compact :class:`MerkleMultiProof` and the Merkle
    ``root`` it claims inclusion under — to a session id;
    :meth:`check` accepts an equal pending binding exactly once,
    recomputing the binding digest, checking the expiry and delegating
    to :func:`verify_multi_inclusion`, and then marks the id consumed.
    The digest frames the entry under
    ``SHA-256(F(D) || F(session_id) || L(entry) || F(E))`` with domain
    ...
    the continuous run of the :class:`MerkleMultiReplayGuard`
    transcript from ``F(root)`` through ``S(proof.siblings, id)``; the
    F / U / S framing and the E expiry encoding are reused byte for
    byte from the other guards. By default both the pending and the
    consumed state live on this guard instance and are never shared
    between instances; passing an :class:`SQLiteReplayStore` as
    ``store`` instead keeps the state in that store under the
    ``b"zr/mmer/v1"`` key domain, so entry guards attached to the same
    store namespace share pending, claimed and consumed ids across
    independent instances, processes and process restarts.

    Concurrent checks of the same id are decided by an atomic claim
    taken before the digest/expiry work and the delegated verification
    (the in-instance registry lock, or the store's short transaction
    and unique claim token); the claim is per-id registry state rather
    than a global lock, so a proof being verified under one id never
    serializes checks or binds of other ids.
    """
    ...
    def bind_once(
        ...
        *,
        ...
        Returns the frozen :class:`ReplayBinding`. ``entry`` must be a
        :class:`MerkleMultiBatchEntry` whose ``entries`` is a tuple of
        ``(index, leaf)`` pairs (a non-``bool`` integer index and
        ``bytes`` leaf each), whose ``root`` is ``bytes`` and whose
        ``proof`` is a well-typed :class:`MerkleMultiProof`; the
        nested field shape follows the per-item rules of
        :func:`verify_multi_inclusion_batch`. ``session_id`` must be
        non-empty ``bytes`` and ``expires_at`` must be either ``None``
        or a non-``bool`` unsigned 64-bit Unix-second timestamp. The
        U-framed values (``proof.leaf_count``, every proof and entry
        index and the three sequence lengths) must likewise fit in
        uint64. A session id that is already pending, being checked or
        consumed raises :class:`ValueError`. Wrong argument or nested
        field types (including a ``bool`` count or index, a
        non-tuple ``entries`` / ``indices`` / ``siblings``) raise
        :class:`TypeError`; an empty id, an out-of-uint64 expiry or
        framed value, or a rebind raise :class:`ValueError`. Inputs
        are never mutated.
        """
        ...
        if not _merkle_multi_entry_replay_encodable(entry):
            raise ValueError(
                "proof counts and indices must be unsigned 64-bit integers"
            )
        binding = ReplayBinding(
            session_id,
    ...
    def check(
        ...
        *,
        ...
        :func:`verify_multi_inclusion` returning ``False``) returns
        ``False``, releases the claim and leaves the registration
        pending. An exception escaping the delegated verification or a
        database error likewise releases the claim and then propagates
        unchanged, leaving the id usable. With a store backend, only the
        holder of the current claim token can consume the id (an expired

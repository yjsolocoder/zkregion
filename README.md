...
面向区域成员关系的承诺与交互式证明原语。提供哈希承诺、素域乘法群上的 Schnorr 交互证明、确定性 SHA-256 Merkle 包含证明、量化区间的 Pedersen 陷门承诺及其非交互区间证明，以及量化整数坐标下的矩形区域判定。
...
# Pedersen 非交互区间证明：不泄露开合，证明承诺值落在声明区间内
...
assert not verify_range(commitment, proof)  # context 不符
...
- `prove_range(commitment, value, blinding, context=b"", *, randbelow=secrets.randbelow) -> RangeProof` — 对承诺值落在声明区间内的非交互 Schnorr OR 证明；生成前先验证开合
- `verify_range(commitment, proof, context=b"") -> bool` — 复用承诺对象内参数验证区间证明
- `RangeProof(t, e, s)` — 不可变区间证明对象，三个字段均为 `tuple[int, ...]`，长度等于区间整数个数 `n`
...
约束：必须有 `lower <= value <= upper`，且区间宽度 `upper - lower < prime - 1`；盲因子须在 `[1, prime - 1)`；`prime > 3`，`g` 与 `h` 均须位于 `(1, prime)`——缺省 `h = g**2 mod prime` 计算后同样校验 `1 < h < prime`，不满足（例如 `g**2 ≡ 1 (mod prime)`）抛 `ValueError`。所有数值只接受非 `bool` 整数（`True`/`False` 不算整数）：`value`、边界、`prime`、`g`、`h`、`blinding` 类型错误抛 `TypeError`；`randbelow` 不可调用或其返回值不是整数抛 `TypeError`。非法区间、越界 `value`、非法群参数或盲因子越界抛 `ValueError`；`randbelow` 返回值超出 `[0, prime - 2)` 也抛 `ValueError`。
...
> **警告：默认 `h = g**2 mod prime` 的离散对数（`log_g(h) = 2`）是公开已知的，因此默认配置下的承诺不具备绑定性**——知道陷门即可对同一 `element` 给出多个开合（例如 `(value, r)` 与 `(value + 2, r - 1)`），演示程序会展示这一点。生产用途必须传入离散对数未知（无可信设置陷门）的 `h`。此外承诺对象本身只是带区间声明的承诺，**不是范围证明**；范围证明由下面的 `prove_range` / `verify_range` 提供，但其安全性同样继承这里的陷门警告。

### Pedersen 非交互区间证明

`prove_range(commitment, value, blinding, context=b"")` 在不泄露开合的前提下，证明 `commitment` 的承诺值落在其声明区间 `[lower, upper]` 内。设 `n = upper - lower + 1`（**最多 256 个整数**），对每个偏移 `i ∈ [0, n)` 定义

```
D_i = element * g**(-i) mod prime
```

真实偏移 `m = value - lower` 处 `D_m = h**blinding`，即证明方知道 `D_m` 以 `h` 为底的离散对数。证明采用 Cramer-Damgård-Schoenmakers 的 Schnorr OR 构造：真实分支取随机 `k`、`t_m = h**k mod prime`，其余分支随机取挑战份额 `e_i ∈ [0, prime)` 与响应 `s_i ≥ 0` 并反解 `t_i = h**s_i * D_i**(-e_i) mod prime`；随后用 Fiat-Shamir 挑战 `c` 补齐真实分支 `e_m = (c - Σe_i) mod prime`、`s_m = k + e_m * blinding`。证明为冻结的 `RangeProof(t, e, s)`，三个字段均为长度 `n` 的整数元组。

挑战 `c` 为 SHA-256 摘要的大端整数模 `prime`。转录依次写入固定域 `b"zkregion/pedersen-range/v1"`、承诺六字段（`element`、`lower`、`upper`、`prime`、`generator`、`h`）、`context`、`n` 和全部 `t`；每项前置四字节无符号大端长度，**整数编码为十进制 ASCII**（负值含 `-` 号）。

`verify_range(commitment, proof, context=b"")` 只复用承诺对象内的群参数与区间：检查 `n ≤ 256`、三个元组长度均为 `n`、各 `t_i ∈ [1, prime)`、各 `e_i ∈ [0, prime)`、各 `s_i ≥ 0`、`sum(e) % prime == c`，并逐分支验证 Schnorr 等式 `h**s_i == t_i * D_i**e_i (mod prime)`。

`prove_range` 生成前先验证开合，开合不符抛 `ValueError`；区间超过 256 个整数抛 `ValueError`；`commitment`、`value`、`blinding`、`context` 或 `randbelow` 的类型错误抛 `TypeError`（沿用非 `bool` 整数与 `bytes` 规则），`randbelow` 返回值越界抛 `ValueError`。`verify_range` 的类型错误抛 `TypeError`；其他非法结构、篡改或绑定不符（错误 context、错配承诺、改动任一 `t`/`e`/`s`）一律返回 `False`。两个入口都不改写输入。

> **警告（演示级安全声明）**：区间证明继承 Pedersen 承诺的全部演示级限制——默认 `h` 带公开陷门、默认群不是素数阶群、响应不针对群阶取模，且 256 分支的 OR 证明未做生产级安全分析。它只用于协议演示，不足以用于真实部署。
...
`DEFAULT_PRIME` 是梅森素数而非安全素数，`2**127 - 2` 的因子分解不干净，因此这里没有可用的素数阶子群，应答按普通整数计算、不针对群阶取模；安全性只够做协议演示，不足以用于真实部署。哈希承诺只支持单点开合，区域判定也只是朴素的坐标比较，不检查坐标是否经过承诺绑定；批量验证所用的随机线性组合与默认群一样仅供演示。Pedersen 承诺默认的 `h = g**2 mod prime` 带有公开陷门、破坏绑定性；区间证明（`prove_range` / `verify_range`）是 Schnorr OR 构造，但同样继承默认群与陷门 `h` 的全部演示级限制，未做生产级安全分析。

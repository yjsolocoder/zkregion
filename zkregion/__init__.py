...
# b"zr/rr/v1" and a RegionReplayGuard under b"zr/rg/v1"), shared across
...
class SQLiteReplayStore:
    ...
    own domain segment via :meth:`_view`, so one store file and namespace
    can serve several guard kinds without their ids colliding.
    ...
    def _view(self, domain: bytes) -> SQLiteReplayStore:
        ...
        (for example ``b"zr/rr/v1"`` for :class:`RangeReplayGuard` or
        ``b"zr/rg/v1"`` for :class:`RegionReplayGuard`).
        """
...
class RegionReplayGuard:
    ...
    As with :class:`ReplayGuard`, concurrent checks of the same id are
    decided by an atomic claim taken before the region proof is verified (the
    in-instance registry lock, or the store's short transaction and unique
    claim token); the verification runs without any lock or transaction
    held, so different ids are never serialized.
    """

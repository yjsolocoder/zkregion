...
class BoundSchnorrReplayGuardTest(unittest.TestCase):
    ...
    def test_domain_separator_is_distinct(self):
        ...
        self.assertEqual(binding.digest, self.expected_digest(batch, root, b"s"))
        # the single-entry ReplayGuard domain must produce another digest
        material = (
        ...
        self.assertNotEqual(binding.digest, hashlib.sha256(material).digest())
        # the same framing with the bound-range / bound-region domains differs
        proof = batch.proof
        ...
        for other_domain in (b"zr/brr/v1", b"zr/brg/v1"):
            other_material = F(other_domain) + F(b"s") + F(root) + F(U(3))
            other_material += b"".join(
            ...
            self.assertNotEqual(
                binding.digest, hashlib.sha256(other_material).digest()
            )
    ...
    def test_bind_nested_type_errors(self):
        ...
        entry = entries[0]
        bad_entry_cases = [
            dataclasses.replace(entry, public_key="k"),
            dataclasses.replace(entry, public_key=True),
            dataclasses.replace(entry, message=7),
            dataclasses.replace(entry, proof="p"),
            dataclasses.replace(
                entry,
            ...
            dataclasses.replace(
                entry,
                proof=SchnorrProof(entry.proof.commitment, False),
            ),
            dataclasses.replace(entry, context="ctx"),
    ...
    def test_negative_leaf_integer_is_a_bind_value_error(self):
        batch, root = self.honest()
        guard = BoundSchnorrReplayGuard()
        entry = batch.entries[0]
        negative_cases = [
            ...
            dataclasses.replace(entry, prime=-1),
            dataclasses.replace(entry, generator=-1),
            dataclasses.replace(entry, public_key=-1),
            dataclasses.replace(
                entry, proof=SchnorrProof(-1, entry.proof.response)
            ),
        ]
        for bad_entry in negative_cases:
            bad_batch = BoundSchnorrBatch(
                (bad_entry,) + batch.entries[1:], batch.leaf_count, batch.proof
            )
            with self.assertRaises(ValueError):
                guard.bind_once(bad_batch, root, b"s")
    ...
    def test_randbelow_is_passed_through_to_delegation(self):
        ...
        binding = guard.bind_once(batch, root, b"s")
        self.assertTrue(guard.check(batch, root, binding, now=1, randbelow=recording))
        # one draw per structurally valid entry, at that entry's prime - 1
        self.assertEqual(calls, [self.G_PRIME - 1, self.G_PRIME - 1, self.G2_PRIME - 1])
    ...
    def test_bad_signature_rejection_does_not_consume(self):
        entries = [
            self.entry(self.alice, b"alpha", prime=self.G_PRIME, generator=3),
            self.entry(self.bob, b"beta", prime=self.G_PRIME, generator=3),
            self.entry(self.carol, b"gamma", prime=self.G2_PRIME, generator=3),
        ]
        forged_proof = SchnorrProof(
            entries[0].proof.commitment, entries[0].proof.response + 1
        )
        forged_entries = [
            dataclasses.replace(entries[0], proof=forged_proof),
            entries[1],
            entries[2],
        ]
        forged_batch, forged_root = self.build(forged_entries)
        ...
        self.assertFalse(
            guard.check(
                forged_batch, forged_root, binding, now=1,
                randbelow=counter_randbelow(),
            )
        )
        with self.assertRaises(ValueError):
    ...
    def test_negative_leaf_integer_check_returns_false_without_consuming(self):
        batch, root = self.honest()
        guard = BoundSchnorrReplayGuard()
        binding = guard.bind_once(batch, root, b"s")
        entry = batch.entries[0]
        ...
        negative_batch = BoundSchnorrBatch(
            (negative_entry,) + batch.entries[1:], batch.leaf_count, batch.proof
        )
        # no OverflowError from the unsigned leaf encoding: check returns False

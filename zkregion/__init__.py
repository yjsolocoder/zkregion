...
def _check_opening_batch_entry(entry: object, name: str = "entry") -> OpeningBatchEntry:
    """Validate one :class:`OpeningBatchEntry` and its nested field types.

    Mirrors the per-item checks of :func:`_check_opening_batch_entries_types`
    for a single entry: it must be an :class:`OpeningBatchEntry` whose
    ``commitment``, ``value`` and ``nonce`` are all ``bytes``. Structural
    and value problems (a mismatching commitment or nonce) are left to
    :func:`verify_opening` at check time.
    """
    if not isinstance(entry, OpeningBatchEntry):
        raise TypeError(f"{name} must be an OpeningBatchEntry")
    _check_bytes(entry.commitment, f"{name} commitment")
    _check_bytes(entry.value, f"{name} value")
    _check_bytes(entry.nonce, f"{name} nonce")
    return entry
...
def _check_pedersen_opening_batch_entry(
    entry: object, name: str = "entry"
) -> PedersenOpeningBatchEntry:
    """Validate one :class:`PedersenOpeningBatchEntry` and its nested field types.

    Mirrors the per-item checks of
    :func:`_check_pedersen_opening_batch_entries_types` for a single entry:
    it must be a :class:`PedersenOpeningBatchEntry` whose ``commitment`` is a
    :class:`PedersenCommitment` with non-``bool`` integer
    ``element`` / ``lower`` / ``upper`` / ``prime`` / ``generator`` / ``h``
    fields and whose ``value`` / ``blinding`` are non-``bool`` integers; a
    ``bool`` masquerading as an integer raises :class:`TypeError`.
    Structural and value problems (out-of-range parameters, a value outside
    the declared range or an incorrect opening) are left to
    :func:`verify_pedersen_opening` at check time.
    """
    if not isinstance(entry, PedersenOpeningBatchEntry):
        raise TypeError(f"{name} must be a PedersenOpeningBatchEntry")
    commitment = entry.commitment
    if not isinstance(commitment, PedersenCommitment):
        raise TypeError(f"{name} commitment must be a PedersenCommitment")
    for field_name in ("element", "lower", "upper", "prime", "generator", "h"):
        _check_int(
            getattr(commitment, field_name), f"{name} commitment {field_name}"
        )
    _check_int(entry.value, f"{name} value")
    _check_int(entry.blinding, f"{name} blinding")
    return entry
...
# ---------------------------------------------------------------------------
# Non-interactive Pedersen wide range proofs (bit decomposition)
#
# A declared range of exactly 2**k integers (1 <= k <= 24) is proven by
# decomposing the offset m = value - lower into bits b_0 .. b_{k-1}. The
# prover commits to each bit as C_i = g**b_i * h**r_i mod prime with
# sum(2**i * r_i) == blinding (mod prime - 1), so the bit commitments
# multiply back to the commitment: product(C_i**(2**i)) == element.
# Each bit is then proven to be 0 or 1 with a Schnorr OR over the two
# statements D_{i,0} = C_i and D_{i,1} = C_i * generator**(-1) mod prime;
# the per-bit challenge shares sum to the transcript challenge modulo
# prime. The announcements are recomputed by the verifier from the
# challenge shares and responses, so each bit branch stores exactly its
# commitment, its two challenge shares and its two responses.
...
class WideRangeProof:
    ...
    ``commitments`` holds the per-bit Pedersen commitments ``C_i`` in bit
    order (least significant bit first); ``challenges`` and ``responses``
    hold, per bit, the two challenge shares and the two responses of the
    Schnorr OR that proves the committed bit to be 0 or 1. All three are
    tuples of ``k`` entries, where ``k`` is the base-2 bit width of the
    declared range (the range must contain exactly ``2**k`` integers).
    """
...
def _wide_range_challenge(
    ...
    The transcript is the domain separator, the six commitment fields, the
    context, the bit width ``k`` and, per bit in bit order, the bit
    commitment ``C_i`` and the two recomputed announcements; each item is
...
def _wide_range_bit_width(commitment: PedersenCommitment) -> int:
    """Bit width of the declared range, or 0 if it is not a power of two."""
...
def prove_range_wide(
    ...
    *,
    ...
    The declared range must contain exactly ``2**k`` integers with the
    base-2 bit width ``k`` between 1 and 24; any other size raises
    :class:`ValueError`. ``value`` and ``blinding`` must be a valid opening
    of ``commitment``; the opening is verified with
    ...
    :class:`ValueError`, mirroring :func:`prove_range`. Randomness is drawn
    from ``randbelow`` (default :func:`secrets.randbelow`); a non-callable
    source or a draw that is not a non-bool integer raises
    :class:`TypeError`, an out-of-range draw raises :class:`ValueError`.
    The same inputs under the same random source produce a byte-identical
    ...
    # Bit blindings: r_1 .. r_{k-1} are random, r_0 closes the weighted sum
    # so sum(2**i * r_i) == blinding (mod prime - 1); its weight is 1, so
    # the correction is always an integer.
    ...
    challenge = _wide_range_challenge(
        ...
        context,
        width,
        tuple(bit_commitments),
        tuple(tuple(pair) for pair in announcements),
    )
    for i in range(width):
        bit = (offset >> i) & 1
    ...
    return WideRangeProof(
        commitments=tuple(bit_commitments),
        challenges=tuple(tuple(pair) for pair in challenges),
        responses=tuple(tuple(pair) for pair in responses),
    )
...
def verify_range_wide(
    ...
    The declared range must contain exactly ``2**k`` integers with
    ``1 <= k <= 24``; any other size returns ``False``. The bit commitments
    must multiply back to the commitment
    (``product(C_i**(2**i)) == element mod prime``) and, per bit, the two
    announcements are recomputed as ``h**s == t * D**e (mod prime)`` with
    ``D`` the bit commitment or the bit commitment divided by
    ``generator``; the two challenge shares of every bit must sum to the
    transcript challenge modulo ``prime``. Type errors (wrong object,
    ...
    any other invalid structure, tampering, branch reordering or binding
    mismatch returns ``False``. Inputs are never mutated.
    """
    if not isinstance(commitment, PedersenCommitment):
        raise TypeError("commitment must be a PedersenCommitment")
    _check_commitment_fields(commitment)
...
class WideRangeBatchEntry:
    ...
    :func:`verify_range_wide`, in the same order.
    """
...
def _check_wide_range_batch_entries_types(
    ...
    Mirrors the type expectations of :func:`verify_range_wide` for
    *every* entry before any verification runs: ``entries`` must be a
    non-``bytes`` / ``bytearray`` / ``str`` sequence of
    :class:`WideRangeBatchEntry` objects whose ``commitment`` is a
    :class:`PedersenCommitment` with non-``bool`` integer
    ``element`` / ``lower`` / ``upper`` / ``prime`` / ``generator`` /
    ``h`` fields, whose ``proof`` is a :class:`WideRangeProof` with
    ...
    is left to :func:`verify_range_wide_batch` to reject with
...
def verify_range_wide_batch(
    ...
    *,
    ...
    ``bool`` passed as an integer or a non-tuple proof field — raises
    :class:`TypeError`; only then are the entries checked one by one and
    the first invalid entry short-circuits the batch.

    Every entry reuses the established :class:`WideRangeProof`
    transcript byte for byte: the group parameters and the declared
    range must be valid, the declared range must contain exactly
    ``2**k`` integers with ``1 <= k <= 24``, the proof tuple sizes and
    the bit-commitment / challenge-share / response bounds must match,
    the bit commitments weighted by ``2**i`` must multiply back to the
    commitment element, and the two challenge shares of every bit must
    sum to the transcript challenge. Those checks stay per entry and
    ...
    including ``bool`` integers and a non-callable ``randbelow`` or
    one that returns a non-integer — raise :class:`TypeError`; a
    coefficient outside ``[0, prime - 1)`` raises :class:`ValueError`.
    Every other invalid structure, tampering, commitment swap, context
    swap, declared-range change, branch swap or bit-position move
...
# context and the commitments / challenges / responses fields of the proof:
# commitments as its decimal element count followed by every bit commitment,
# then each of the two pair sequences as its decimal pair count followed by
# every pair item flattened in pair order. Verification first checks every
# leaf against the Merkle root, then runs the unchanged wide range batch
# verification.
...
def _bound_wide_range_leaf(entry: WideRangeBatchEntry) -> bytes:
    ...
    (dataclass field order), the context, then the proof's ``commitments``
    sequence framed as its decimal element count followed by every bit
    commitment, followed by each of the ``challenges`` / ``responses`` pair
    sequences framed as its decimal pair count followed by every pair item
    flattened in pair order. Every item is prefixed with its four-byte
...
def verify_range_wide_batch_bound(
    ...
    *,
    ...
    exactly as specified by :func:`_bound_wide_range_leaf` and the whole
    ...
    valid OR sub-branch as ``randbelow(prime - 1)``.
    ...
    malformed nested field types (a non-:class:`WideRangeProof` proof or
    non-tuple integer fields at any nesting level), or a non-callable
    ``randbelow`` — raise :class:`TypeError`; a non-integer randomness
    return raises :class:`TypeError` and an out-of-range one raises
    :class:`ValueError`, both surfaced by
    :func:`verify_range_wide_batch` per its own contract. Every other
    invalidity (an empty batch, a count or index mismatch, a wrong root,
    tampered leaf bytes or an inner batch rejection) returns ``False``.
    Inputs are never mutated.
    """
...
def prove_range_wide_batch_bound(
    ...
    *,
    ...
    :func:`verify_range_wide_batch` and must be non-empty; every entry is
    ...
    separator ``b"zkregion/wrb/v1"``, the length framing, decimal integer
    encoding and field order stay unchanged, and the leaf digests and
    ...
    zero and its ``siblings`` are empty — and the returned batch carries
    ``leaf_count = n`` alongside that proof. The second return value is the
    outer tree's :func:`merkle_root` of the encoded leaves, which is exactly
    the root the batch verifies under:
    ``verify_range_wide_batch_bound(batch, root)`` returns ``True``.
    Single-item, odd- and even-sized batches and duplicate entries are all
    deterministic, and the same inputs under the same random source rebuild
    a byte-identical batch, root and proof.
...
# ---------------------------------------------------------------------------
# Non-interactive 2-D region membership proofs over wide ranges
#
# The wide variant of a region proof: a pair of bit-decomposition wide range
# proofs, one per axis, reusing prove_range_wide / verify_range_wide so each
# axis may span up to 2**24 integers instead of 256. The declared ranges and
# the derived per-axis contexts follow exactly the prove_region binding: the
# region domain separator, the axis label, the external context, the four
# region bounds and both commitments.


@dataclass(frozen=True)
class RegionWideProof:
    """A non-interactive 2-D region membership proof over wide ranges.
...
def prove_region_wide(
    ...
    *,
    ...
    """Prove that the committed ``(x, y)`` lies inside ``region``, wide axes.

    ``x_commitment`` must be declared over exactly
    ``[region.min_x, region.max_x]`` and ``y_commitment`` over exactly
    ``[region.min_y, region.max_y]``; a mismatch raises :class:`ValueError`.
    Before any proving work happens, each axis opening is recomputed and
    compared with :func:`verify_pedersen_opening` — both axes must pass
    before either sub-proof is produced, and a mismatch (including a
    coordinate outside the declared range) raises :class:`ValueError`. Each
    axis range must contain exactly ``2**k`` integers with ``1 <= k <= 24``
    (the two axes may use different widths); any other size raises
    :class:`ValueError`, as does an out-of-range draw from ``randbelow``.
    Type errors (wrong objects, non-integer or ``bool`` numbers,
    non-``bytes`` context, non-callable ``randbelow`` or a non-integer draw)
    raise :class:`TypeError`. The per-axis sub-proofs reuse
    :func:`prove_range_wide` unchanged under the same derived contexts as
    :func:`prove_region`; the same inputs under the same random source
...
def verify_region_wide(
    ...
    non-integer or ``bool`` fields, non-``bytes`` context) raise
    :class:`TypeError`; a commitment range that does not equal the region
    bounds, a bit-width violation, any invalid structure, tampering, a
    swapped region, context or commitment, or swapped axes returns ``False``
    without raising. Inputs are never mutated.
    """
    if not isinstance(x_commitment, PedersenCommitment):
        raise TypeError("x_commitment must be a PedersenCommitment")
    if not isinstance(y_commitment, PedersenCommitment):
...
#
# A RegionWideBatchEntry freezes the five arguments of verify_region_wide
# (both commitments, the region, the RegionWideProof and the external
# context); verify_region_wide_batch preflights the nested types of the
# whole batch, then keeps every per-entry wide-range check (the
# power-of-two weighted bit-commitment binding and the per-bit challenge
# sums) while aggregating the two OR sub-branches of every bit of both
# axis sub-proofs into one random linear equation per (prime, generator, h)
# group, exactly as verify_range_wide_batch does for a single axis.
...
class RegionWideBatchEntry:
    ...
    :func:`verify_region_wide`, in the same order. Construction performs
    no validation; entries are positional, compare by value and are
    immutable.
    """
...
def _check_region_wide_entry_fields(
    entry: object, position: int
) -> None:
    """Validate every nested type of one :class:`RegionWideBatchEntry`.

    Mirrors the type expectations of :func:`verify_region_wide` and
    :func:`verify_range_wide` for the two axis sub-proofs: both
    commitments must be :class:`PedersenCommitment` objects with
    non-``bool`` integer fields, the region must be a :class:`Region`
    with non-``bool`` integer bounds, the proof must be a
    :class:`RegionWideProof` whose two :class:`WideRangeProof` objects
    carry tuple fields of non-``bool`` integers at every nesting level,
    and the context must be ``bytes``.
    """
    label = f"entries[{position}]"
    if not isinstance(entry, RegionWideBatchEntry):
        raise TypeError(f"{label} must be a RegionWideBatchEntry")
    ...
    _check_region_fields(region)
    if not isinstance(proof, RegionWideProof):
        raise TypeError(f"{label} proof must be a RegionWideProof")
    for axis_name, sub_proof in (("x", proof.x_proof), ("y", proof.y_proof)):
        if not isinstance(sub_proof, WideRangeProof):
            raise TypeError(
                f"{label} proof {axis_name}_proof must be a WideRangeProof"
            )
        if not isinstance(sub_proof.commitments, tuple):
            raise TypeError(
                f"{label} proof {axis_name}_proof commitments must be a tuple "
                "of integers"
            )
        for item in sub_proof.commitments:
            _check_int(
                item, f"{label} proof {axis_name}_proof commitments entry"
            )
        for field_name in ("challenges", "responses"):
            ...
            for pair in field:
                ...
                for item in pair:
                    _check_int(
                        item,
                        f"{label} proof {axis_name}_proof {field_name} entry item",
                    )
...
def _check_region_wide_batch_entries_types(
    ...
    *every* entry before any verification runs: ``entries`` must be a
    non-``bytes`` / ``bytearray`` / ``str`` sequence of
    :class:`RegionWideBatchEntry` objects with valid nested types at
    ...
    for position, entry in enumerate(entries):
        _check_region_wide_entry_fields(entry, position)
        items.append(entry)
    return items
...
def verify_region_wide_batch(
    ...
    *,
    ...
    duplicate entries are legal (each draws its own coefficients). The
    nested types of the *whole* batch are preflighted first, so a wrong
    type at any nesting level of any entry — including the last one,
    including a ``bool`` passed as an integer and including a
    non-``bytes`` context — raises :class:`TypeError`; only then are the
    entries checked one by one and the first invalid entry short-
    circuits the batch.

    Every entry reuses the established :class:`RegionWideProof` /
    :class:`WideRangeProof` transcripts byte for byte: each commitment's
    declared range must equal the corresponding region bounds, the group
    parameters and declared ranges must be valid, each declared range
    must contain exactly ``2**k`` integers with ``1 <= k <= 24``, the
    sub-proof tuple sizes and the bit-commitment / challenge-share /
    response bounds must match, and the two challenge shares of every
    bit must sum to the transcript challenge under the per-axis derived
    context from :func:`_region_sub_context`. The bit commitments
    weighted by ``2**i`` must multiply back to that axis commitment
    element; this weighted-sum binding, like the challenge-share sums,
    stays a per-entry, per-axis check and is never aggregated.

    Each of the two OR sub-branches of every bit of both axis
    sub-proofs then draws exactly one random coefficient
    ``a = r + 1`` with ``r = randbelow(prime - 1)``. Sub-branches
    sharing the same ``(prime, generator, h)`` group — across axes and
    entries alike — are checked together with a single aggregate
    equation
    ...
    invalid structure, tampering, a commitment, region or context swap,
    a swapped axis assignment or a replaced sub-proof returns
    ``False``. Entries are independent and may appear in any order,
    including repeats; the same batch under the same random source
    gives the same result. Missing entries cannot be detected: the
    caller guarantees the batch is complete. Inputs are never mutated.
    """
    ...
    for entry in items:
        ...
        for commitment, sub_proof, sub_context in axes:
            ...
            if not 1 <= width <= _MAX_WIDE_RANGE_BITS:
                return False
            if not (
                len(sub_proof.commitments)
                == len(sub_proof.challenges)
                == len(sub_proof.responses)
                == width
            ):
                return False
            if any(len(pair) != 2 for pair in sub_proof.challenges):
                return False
            if any(len(pair) != 2 for pair in sub_proof.responses):
                return False
            if any(not 1 <= c_i < prime for c_i in sub_proof.commitments):
                return False
            if any(
                not 0 <= share < prime for pair in sub_proof.challenges for share in pair
            ):
                return False
            if any(
                not 0 <= response < prime - 1
                for pair in sub_proof.responses
                for response in pair
            ):
                return False
            # The power-of-two weighted binding stays a per-entry check.
            product = 1
            for i, bit_commitment in enumerate(sub_proof.commitments):
                product = product * pow(bit_commitment, 1 << i, prime) % prime
            if product != commitment.element:
                return False
            generator = commitment.generator
            ...
            for i in range(width):
                ...
                for branch in (0, 1):
                    ...
                    try:
                        announcement = (
                            pow(h, sub_proof.responses[i][branch], prime)
                            * pow(statement, -sub_proof.challenges[i][branch], prime)
                            % prime
                        )
                    except ValueError:
                        return False  # statement not invertible modulo prime
            ...
            challenge = _wide_range_challenge(
                ...
                sub_context,
                width,
                sub_proof.commitments,
                tuple(announcements),
            )
            for pair_e in sub_proof.challenges:
                if (pair_e[0] + pair_e[1]) % prime != challenge:
                    return False
            group_key = (prime, generator, h)
            for i in range(width):
...
# commits to every entry. Each Merkle leaf starts from the domain separator
# b"zkregion/rwb/v1" and frames, in entry field order, the six fields of
# the x then the y commitment, the four region bounds, the context and the
# commitments / challenges / responses fields of each axis sub-proof:
# commitments as its decimal element count followed by every bit
# commitment, then each of the two pair sequences as its decimal pair count
# followed by every pair item flattened in pair order, x axis before y.
# Verification first checks every leaf against the Merkle root, then runs
# the unchanged wide region batch verification.
...
class BoundRegionWideBatch:
    ...
    All three are positional construction arguments; batches compare by
...
def verify_region_wide_batch_bound(
    ...
    *,
    ...
    structurally valid OR sub-branch as ``randbelow(prime - 1)``.

    Type errors — a batch that is not a
    :class:`BoundRegionWideBatch`, non-tuple entries,
    non-:class:`RegionWideBatchEntry` items, a non-integer or ``bool``
    ``leaf_count``, a wrong proof/root object, malformed nested field
    types at any nesting level (including a ``bool`` masquerading as an
    integer or a non-``bytes`` context), or a non-callable
    ``randbelow`` — raise :class:`TypeError`; a non-integer randomness
    return raises :class:`TypeError` and an out-of-range one raises
    :class:`ValueError`, both surfaced by
...
def prove_region_wide_batch_bound(
    ...
    *,
    ...
    all deterministic, and the same inputs rebuild a byte-identical
    batch, root and proof.

    A type preflight over the whole batch — every entry and every
    nested field, including ``bool`` fields and later entries — raises
    :class:`TypeError` before anything is built; an empty batch, a
    batch length outside uint64, or
    :func:`verify_region_wide_batch` returning ``False`` (an invalid
    inner wide region proof) raises :class:`ValueError`. The
    ``randbelow`` argument is passed through to
    :func:`verify_region_wide_batch` unchanged under its randomness
    contract.
    """
    items = _check_region_wide_batch_entries_types(entries)
    if not callable(randbelow):
...
_SINGLE_KEY_ENTRY_REPLAY_DOMAIN = b"zr/sker/v1"
...
_OPENING_REPLAY_DOMAIN = b"zr/orr/v1"
_PEDERSEN_OPENING_BATCH_REPLAY_DOMAIN = b"zr/pobr/v1"
_PEDERSEN_OPENING_REPLAY_DOMAIN = b"zr/porr/v1"
...
_BOUND_WIDE_RANGE_REPLAY_DOMAIN = b"zr/bwrr/v1"
_MERKLE_INCLUSION_ENTRY_REPLAY_DOMAIN = b"zr/mirr/v1"
_WIDE_RANGE_REPLAY_DOMAIN = b"zr/wrr/v1"
_WIDE_RANGE_BATCH_REPLAY_DOMAIN = b"zr/wrbr/v1"
_UINT64_MAX = (1 << 64) - 1
...
class SQLiteReplayStore:
    ...
    :class:`SingleKeyEntryReplayGuard`,
    ...
    :class:`MerkleMultiEntryReplayGuard`,
    :class:`SchnorrBatchReplayGuard`,
    :class:`OpeningBatchReplayGuard`,
    :class:`OpeningReplayGuard` and
...
# ---------------------------------------------------------------------------
# Replay protection for single compact Merkle multi-inclusion batch entries
#
# A MerkleMultiEntryReplayGuard binds one MerkleMultiBatchEntry — the same
# (entries, proof, root) item shape accepted by verify_multi_inclusion_batch
# — to a caller-chosen session id, reusing the ReplayBinding type and, byte
# for byte, the F / U / S framing and the E expiry encoding of the other
# guards. Like them the binding is single-use: without a store the state is
# local to the guard instance, while an SQLiteReplayStore keeps pending,
# claimed and consumed ids in the store under b"zr/mmer/v1", shared across
# instances, processes and restarts. bind_once registers a pending binding,
# check claims the id atomically, recomputes the digest, checks the expiry,
# delegates the single multi-inclusion item to verify_multi_inclusion and
# consumes the id only on full success; every rejection leaves the id
# untouched.
#
# digest = SHA-256(
...
#   S(a, f) = F(U(|a|)) || Σ F(f(a_i))
#   L(entry) is the raw outer leaf of the Merkle-committed complete
#   multi-inclusion batch (_bound_merkle_multi_leaf), no F framing:
#     F(b"zkregion/multi-batch/v1") || F(Q(entry))
#   with Q(entry) the continuous run of the multi-inclusion guard
#   transcript from F(root) through S(proof.siblings, id):
#     F(root) || F(U(proof.leaf_count)) || S(proof.indices, U)
#     || S(entry.entries, Q_e) || S(proof.siblings, id)
#   where Q_e((index, leaf)) = F(U(index)) || F(leaf), i.e. the bytes of
#   _merkle_multi_proof_framing
#   E is the same expiry encoding as the other guards:
#     b"\x00" when expires_at is None, b"\x01" + uint64be(expires_at) otherwise


def _check_merkle_multi_entry(
    entry: object, name: str = "entry"
) -> MerkleMultiBatchEntry:
    """Validate one :class:`MerkleMultiBatchEntry` and its nested types.

    Applies the per-item rules of :func:`verify_multi_inclusion_batch`: the
    entry must be a :class:`MerkleMultiBatchEntry` whose ``entries`` is a
    tuple of two-item ``(index, leaf)`` pairs with a non-``bool`` integer
    index and ``bytes`` leaf, whose ``root`` is ``bytes`` and whose
    ``proof`` is a :class:`MerkleMultiProof` with a non-``bool`` integer
    ``leaf_count``, a tuple of non-``bool`` integer ``indices`` and a
    tuple-of-bytes ``siblings``. Structural and value problems (a
    non-positive ``leaf_count``, empty or out-of-order indices, malformed
    digest lengths, an index out of range or a pair/indices count
    mismatch) are left to :func:`verify_multi_inclusion` at check time.
    """
    if not isinstance(entry, MerkleMultiBatchEntry):
        raise TypeError(f"{name} must be a MerkleMultiBatchEntry")
    if not isinstance(entry.entries, tuple):
        raise TypeError(
            f"{name} entries must be a tuple of (index, leaf) pairs"
        )
    for pair_position, pair in enumerate(entry.entries):
        if not isinstance(pair, tuple) or len(pair) != 2:
            raise TypeError(
                f"{name} entries[{pair_position}] must be an (index, leaf) pair"
            )
        index, leaf = pair
        if not isinstance(index, int) or isinstance(index, bool):
            raise TypeError(
                f"{name} entries[{pair_position}] index must be an integer"
            )
        if not isinstance(leaf, bytes):
            raise TypeError(
                f"{name} entries[{pair_position}] leaf must be bytes"
            )
    _check_bytes(entry.root, f"{name} root")
    proof = entry.proof
    if not isinstance(proof, MerkleMultiProof):
    ...
    if not isinstance(proof.leaf_count, int) or isinstance(proof.leaf_count, bool):
        raise TypeError(f"{name} proof leaf_count must be an integer")
    if not isinstance(proof.indices, tuple):
        raise TypeError(f"{name} proof indices must be a tuple of integers")
    for index_position, index in enumerate(proof.indices):
        if not isinstance(index, int) or isinstance(index, bool):
            raise TypeError(
                f"{name} proof indices[{index_position}] must be an integer"
            )
    if not isinstance(proof.siblings, tuple):
        raise TypeError(f"{name} proof siblings must be a tuple of bytes")
    for sibling_position, sibling in enumerate(proof.siblings):
        _check_bytes(sibling, f"{name} proof siblings[{sibling_position}]")
    return entry


def _merkle_multi_entry_replay_encodable(
    entry: MerkleMultiBatchEntry,
) -> bool:
    """Every U-framed count must fit in unsigned 64 bits.

    The proof ``leaf_count``, every proof and entry index and the
    ``indices`` / ``entries`` / ``siblings`` tuple lengths written inside
    the outer leaf's ``Q`` run are encoded with ``U``; the raw root,
    sibling and leaf bytes need no encodability rule.
    """
    proof = entry.proof
    values = [
        proof.leaf_count,
        ...
        len(proof.siblings),
        *proof.indices,
        *(index for index, _leaf in entry.entries),
    ]
    return all(0 <= value <= _UINT64_MAX for value in values)


def _merkle_multi_entry_replay_digest(
    entry: MerkleMultiBatchEntry,
    session_id: bytes,
    expires_at: int | None,
) -> bytes:
    """Compute the Merkle-multi-entry replay binding digest.

    Writes, in order: ``F(D)``, ``F(session_id)``, ``L(entry)`` and
    ``F(E)``, where ``L(entry)`` is the raw outer leaf of the
    Merkle-committed complete multi-inclusion batch
    (:func:`_bound_merkle_multi_leaf`), the same per-item leaf bytes that
    :func:`verify_multi_inclusion_batch_bound` recomputes when checking the
    outer root. Each entry pair keeps its proof-indices order and the
    proof siblings their level-by-level leaf-to-root order.
    """
    transcript = hashlib.sha256()
    transcript.update(
        _frame_length_prefixed(_MERKLE_MULTI_ENTRY_REPLAY_DOMAIN)
    )
    transcript.update(_frame_length_prefixed(session_id))
    transcript.update(_bound_merkle_multi_leaf(entry))  # raw leaf, no F framing
    expiry = _replay_expiry_bytes(expires_at)
    transcript.update(_frame_length_prefixed(expiry))
    return transcript.digest()
...
class MerkleMultiEntryReplayGuard:
    """Single-use replay protection for one multi-inclusion batch entry.

    A fresh guard has no registrations. :meth:`bind_once` registers a
    pending :class:`ReplayBinding` that commits a single
    :class:`MerkleMultiBatchEntry` — its ``(index, leaf)`` ``entries``,
    its :class:`MerkleMultiProof` and the Merkle ``root`` it claims
    inclusion under — to a session id; :meth:`check` accepts an equal
    pending binding exactly once, recomputing the binding digest, checking
    the expiry and delegating to :func:`verify_multi_inclusion`, and then
    marks the id consumed. The digest frames the entry under
    ``SHA-256(F(D) || F(session_id) || L(entry) || F(E))`` with domain
    ``b"zr/mmer/v1"``, where ``L(entry)`` is the raw outer leaf of the
    Merkle-committed complete multi-inclusion batch
    (:func:`_bound_merkle_multi_leaf`), i.e.
    ``F(b"zkregion/multi-batch/v1") || F(Q(entry))`` with ``Q(entry)``
    the continuous run of the :class:`MerkleMultiReplayGuard` transcript
    from ``F(root)`` through ``S(proof.siblings, id)``; the F / U / S
    framing and the E expiry encoding are reused byte for byte from the
    other guards. By default both the pending and the consumed state live
    on this guard instance and are never shared between instances;
    passing an :class:`SQLiteReplayStore` as ``store`` instead keeps the
    state in that store under the ``b"zr/mmer/v1"`` key domain, so
    multi-entry guards attached to the same store namespace share
    pending, claimed and consumed ids across independent instances,
    processes and process restarts.
    ...
    def __init__(self, *, store: SQLiteReplayStore | None = None) -> None:
        ...
        self._store = (
            None
            if store is None
            else store._view(_MERKLE_MULTI_ENTRY_REPLAY_DOMAIN)
        )
        self._registry = None if store is not None else _ReplayRegistry()
    ...
    def bind_once(
        ...
        *,
        ...
        Returns the frozen :class:`ReplayBinding`. ``entry`` must be a
        :class:`MerkleMultiBatchEntry` (a tuple of ``(index, leaf)``
        pairs, a well-typed :class:`MerkleMultiProof` and ``bytes``
        root), ``session_id`` must be non-empty ``bytes`` and
        ``expires_at`` must be either ``None`` or a non-``bool`` unsigned
        64-bit Unix-second timestamp. The U-framed values (the proof
        ``leaf_count``, every proof and entry index and the
        ``indices`` / ``entries`` / ``siblings`` tuple lengths) must
        likewise fit in uint64. The pairs are expected in the exact
        order of ``proof.indices``; order, range and count problems are
        left to :func:`verify_multi_inclusion` at check time. A session
        id that is already pending, being checked or consumed raises
        :class:`ValueError`. Wrong argument or nested field types
        (including a ``bool`` count or index) raise :class:`TypeError`;
        an empty id, an out-of-uint64 expiry or framed value, or a
        rebind raise :class:`ValueError`. Inputs are never mutated.
        """
        _check_merkle_multi_entry(entry)
        _check_bytes(session_id, "session_id")
        if not session_id:
            raise ValueError("session_id must not be empty")
        if expires_at is not None:
            _check_uint64(expires_at, "expires_at")
        if not _merkle_multi_entry_replay_encodable(entry):
            raise ValueError(
        ...
        binding = ReplayBinding(
            session_id,
            _merkle_multi_entry_replay_digest(entry, session_id, expires_at),
            expires_at,
        )
        if self._store is not None:
            self._store.register(session_id, binding)
    ...
    def check(
        ...
        *,
        ...
        uint64). Only then are the entry's ``entries``, ``proof`` and
        ``root`` handed to :func:`verify_multi_inclusion`, in that order,
        which hashes the leaves, rebuilds the missing siblings level by
        level and compares the recomputed root.
        ...
        try:
            if not hmac.compare_digest(
                binding.digest,
                _merkle_multi_entry_replay_digest(
                    entry, session_id, binding.expires_at
                ),
            ):
                return False  # the presented entry is not the one originally bound
            if binding.expires_at is not None and current >= binding.expires_at:
                return False  # expired: rejection does not consume the id
            if not verify_multi_inclusion(entry.entries, entry.proof, entry.root):
                return False
            self._registry.commit(session_id)
            committed = True
            return True
...
# ---------------------------------------------------------------------------
# Replay protection for one same-key Schnorr entry
#
# A SingleKeyEntryReplayGuard binds a single SchnorrBatchEntry — one
# message, proof and context made for one fixed public key and group — to
# one caller-chosen session id. It reuses the ReplayBinding type and, byte
# for byte, the F framing and the E expiry encoding of the other
# single-entry guards (ReplayGuard / OpeningReplayGuard / ...): only the
# domain separator and the leaf differ. The guard is constructed with one
# fixed public_key and group (prime / generator), and the leaf is the
# BoundSchnorr leaf raw bytes (_single_key_batch_leaf, i.e.
# _bound_schnorr_leaf over the entry lifted with the guard's fixed key and
# group) — the same per-item outer leaf byte the same-key batch guards use.
# Without a store the pending/claimed/consumed states are local to the
# guard instance, while an SQLiteReplayStore keeps them in the store under
# b"zr/sker/v1", shared across instances, processes and restarts.
# bind_once registers a pending binding, check claims the id atomically,
# recomputes the digest, checks the expiry, delegates the single proof to
# the fixed SchnorrVerifier's verify_proof with the entry's message, proof
# and context in field order and consumes the id only on full success;
# every rejection leaves the id untouched.
#
# digest = SHA-256(
#     F(D) || F(session_id) || L(entry) || F(E)
...
#   D = b"zr/sker/v1"
#   F(x) = four-byte unsigned big-endian length prefix of x, followed by x
#   L(entry) = the BoundSchnorr leaf raw bytes (_bound_schnorr_leaf) built
#       from a MultiSchnorrEntry carrying this guard's fixed public_key,
#       prime and generator together with the entry's message, proof and
#       context, raw, i.e. without an F framing
#   E is the same expiry encoding as the other guards:
#     b"\x00" when expires_at is None, b"\x01" + uint64be(expires_at) otherwise


def _check_single_key_entry_type(entry: object) -> SchnorrBatchEntry:
    """Validate the single same-key ``entry`` argument types.

    Mirrors the nested type rules of :meth:`SchnorrVerifier.verify_proof`
    and of one item of :meth:`SchnorrVerifier.verify_batch`: ``entry`` must
    be a :class:`SchnorrBatchEntry` whose ``message`` / ``context`` are
    ``bytes`` and whose ``proof`` is a :class:`SchnorrProof` with
    non-``bool`` integer commitment / response. Structural and value
    problems (commitment range, negative response) are left to
    :meth:`~SchnorrVerifier.verify_proof` at check time.
    """
    if not isinstance(entry, SchnorrBatchEntry):
        raise TypeError("entry must be a SchnorrBatchEntry")
    _check_bytes(entry.message, "entry message")
    proof = entry.proof
    if not isinstance(proof, SchnorrProof):
        raise TypeError("entry proof must be a SchnorrProof")
    if (
        not isinstance(proof.commitment, int)
        or isinstance(proof.commitment, bool)
        or not isinstance(proof.response, int)
        ...
        raise TypeError("entry proof commitment and response must be integers")
    _check_bytes(entry.context, "entry context")
    return entry


def _single_key_entry_replay_encodable(entry: SchnorrBatchEntry) -> bool:
    """Every leaf integer must be non-negative.

    The BoundSchnorr leaf frames the proof commitment / response with the
    shortest unsigned big-endian encoding, so a negative integer cannot be
    framed. The guard's own ``prime`` / ``generator`` / ``public_key`` are
    validated at construction.
    """
    return min(entry.proof.commitment, entry.proof.response) >= 0


def _single_key_entry_replay_digest(
    entry: SchnorrBatchEntry,
    session_id: bytes,
    expires_at: int | None,
    *,
    ...
    """Compute the single same-key Schnorr entry replay binding digest.

    Byte for byte the :class:`ReplayGuard` single-entry framing with the
    same-key entry replay domain and the BoundSchnorr leaf raw bytes built
    from the entry under the fixed public key and group. Writes, in order:
    ``F(D)``, ``F(session_id)``, ``L(entry)`` (raw, without an F framing)
    and ``F(E)``.
    """
    transcript = hashlib.sha256()
    transcript.update(_frame_length_prefixed(_SINGLE_KEY_ENTRY_REPLAY_DOMAIN))
    transcript.update(_frame_length_prefixed(session_id))
    transcript.update(
        _single_key_batch_leaf(entry, public_key, prime, generator)
    )  # raw leaf, no F framing
    expiry = _replay_expiry_bytes(expires_at)
    transcript.update(_frame_length_prefixed(expiry))
    return transcript.digest()
...
class SingleKeyEntryReplayGuard:
    """Single-use replay protection for one same-key :class:`SchnorrBatchEntry`.

    The guard is bound to one fixed ``public_key`` and group (``prime`` /
    ``generator``, defaulting to :data:`DEFAULT_PRIME` /
    :data:`DEFAULT_GENERATOR`); the entry it protects is a single
    :class:`SchnorrBatchEntry` whose ``message``, ``proof`` and ``context``
    are verified under that fixed key and group by the
    :class:`SchnorrVerifier` constructed once from those parameters. A
    fresh guard has no registrations. :meth:`bind_once` registers a
    pending :class:`ReplayBinding` that commits one entry to a session id;
    :meth:`check` accepts an equal pending binding exactly once — claiming
    the id atomically, recomputing the binding digest, checking the expiry
    and delegating to the fixed verifier's
    :meth:`~SchnorrVerifier.verify_proof` with the entry's ``message``,
    ``proof`` and ``context`` in field order — and then marks the id
    consumed. The digest frames the entry under
    ``SHA-256(F(D) || F(session_id) || L(entry) || F(E))`` with domain
    ``b"zr/sker/v1"``, where ``L(entry)`` is byte for byte the per-item
    BoundSchnorr leaf built with this guard's fixed public key and group
    parameters (the same raw bytes :class:`SingleKeyBatchGuard` frames per
    batch item); the F framing and the E expiry encoding are reused byte
    for byte from the other single-entry replay guards. By default both
    the pending and the consumed state live on this guard instance and are
    never shared between instances; passing an :class:`SQLiteReplayStore`
    as ``store`` instead keeps the state in that store under the
    ``b"zr/sker/v1"`` key domain, so same-key entry guards attached to the
    same store namespace share pending, claimed and consumed ids across
    independent instances, processes and process restarts.

    Concurrent checks of the same id are decided by an atomic claim taken
    before the digest/expiry work and the delegated proof verification
    (the in-instance registry lock, or the store's short transaction and
    unique claim token); the claim is per-id registry state rather than a
    global lock, so an entry being verified under one id never serializes
    checks or binds of other ids.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._store = (
            None
            if store is None
            else store._view(_SINGLE_KEY_ENTRY_REPLAY_DOMAIN)
        )
        self._registry = None if store is not None else _ReplayRegistry()
    ...
    def _digest(
        ...
        """The binding digest over ``entry`` under this guard's key/group."""
        return _single_key_entry_replay_digest(
            entry,
            session_id,
            expires_at,
            public_key=self._public_key,
    ...
    def bind_once(
        ...
        *,
        ...
        Returns the frozen :class:`ReplayBinding`. ``entry`` follows the
        same nested type rules as one item of
        :meth:`SchnorrVerifier.verify_batch`: its ``message`` and
        ``context`` are ``bytes`` and its ``proof`` is a
        :class:`SchnorrProof` with non-``bool`` integer commitment /
        response. ``session_id`` must be non-empty ``bytes`` and
        ``expires_at`` must be either ``None`` or a non-``bool`` unsigned
        64-bit Unix-second timestamp; every leaf integer (the proof
        commitment / response) must be non-negative. A session id that is
        already pending, being checked or consumed raises
        :class:`ValueError`. Wrong argument or nested field types
        (including ``bool`` integers) raise :class:`TypeError`; an empty
        id, an out-of-uint64 expiry, a negative leaf integer or a rebind
        raise :class:`ValueError`. Inputs are never mutated.
        """
        _check_single_key_entry_type(entry)
        _check_bytes(session_id, "session_id")
        if not session_id:
            raise ValueError("session_id must not be empty")
        if expires_at is not None:
            _check_uint64(expires_at, "expires_at")
        if not _single_key_entry_replay_encodable(entry):
            raise ValueError("leaf integers must be non-negative unsigned integers")
        binding = ReplayBinding(
            session_id,
    ...
    def check(
        ...
        *,
        ...
        delegated verification, so among concurrent calls for the same id
        at most one can return ``True``. The digest is recomputed over the
        presented ``entry`` under this guard's fixed public key and group;
        for a binding with an expiry, ``now >= expires_at`` makes the check
        fail (``now`` defaults to the current Unix seconds and must
        otherwise be a non-``bool`` uint64). Only then is the entry's
        ``message``, ``proof`` and ``context`` handed to the fixed
        verifier's :meth:`~SchnorrVerifier.verify_proof` in field order.

        Only a fully successful check consumes the session id; every
        rejection (unknown or consumed id, a claim lost to a concurrent
        check, unequal binding, digest mismatch, expiry, a negative leaf
        integer, or ``verify_proof`` returning ``False``) returns
        ``False``, releases the claim and leaves the registration pending.
        An exception escaping the delegated verification likewise releases
        the claim and then propagates unchanged, leaving the id usable.
        With a store backend, only the holder of the current claim token
        ...
        if now is None:
            ...
            current = now
        if not _single_key_entry_replay_encodable(entry):
            return False  # negative leaf integers
        session_id = binding.session_id
        if self._store is not None:
            return self._check_with_store(entry, binding, session_id, current)
        if not self._registry.claim(session_id, binding):
        ...
        try:
            ...
            if binding.expires_at is not None and current >= binding.expires_at:
                return False  # expired: rejection does not consume the id
            if not self._verifier.verify_proof(
                entry.message, entry.proof, context=entry.context
            ):
                return False
            self._registry.commit(session_id)
...
# ---------------------------------------------------------------------------
# Replay protection for a single hash-commitment opening entry
#
# An OpeningReplayGuard binds one OpeningBatchEntry — the same entry shape
# accepted item by item by verify_opening_batch and by verify_opening — to a
# caller-chosen session id, reusing the ReplayBinding type and, byte for byte,
# the F framing and the E expiry encoding of the other single-entry guards
# (RegionContainsReplayGuard / RegionReplayGuard); only the domain separator
# and the entry leaf differ. Like them the binding is single-use: without a
# store the pending and consumed state lives on this guard instance and is
# never shared between instances, while an SQLiteReplayStore keeps pending,
# claimed and consumed ids in the store under b"zr/orr/v1", shared across
# instances, processes and restarts. bind_once registers a pending binding,
# check claims the id atomically, recomputes the digest, checks the expiry,
# delegates the opening decision to verify_opening and consumes the id only
# on full success; every rejection leaves the id untouched.
#
# digest = SHA-256(F(D) || F(session_id) || L(entry) || F(E))
#   D = b"zr/orr/v1"
#   F(x) = four-byte unsigned big-endian length prefix of x, followed by x
#   L(entry) = the BoundOpeningBatch outer-leaf bytes for one entry
#              (_bound_opening_leaf), i.e. F(b"zkregion/ob/v1") followed by
#              the entry's commitment, value and nonce each length-framed —
#              the same per-item leaf bytes reused raw, without an F framing
#   E = b"\x00"                          when expires_at is None
#     = b"\x01" + uint64be(expires_at)   otherwise


def _opening_replay_digest(
    entry: OpeningBatchEntry,
    session_id: bytes,
    expires_at: int | None,
) -> bytes:
    """Compute ``SHA-256(F(D) || F(session_id) || L(entry) || F(E))``.

    Byte for byte the :func:`_region_contains_replay_digest` single-entry
    framing with the domain ``b"zr/orr/v1"`` and
    :func:`_bound_opening_leaf` bytes as the raw per-entry leaf.
    """
    transcript = hashlib.sha256()
    transcript.update(_frame_length_prefixed(_OPENING_REPLAY_DOMAIN))
    transcript.update(_frame_length_prefixed(session_id))
    transcript.update(_bound_opening_leaf(entry))  # raw leaf, no F framing
    expiry = _replay_expiry_bytes(expires_at)
    transcript.update(_frame_length_prefixed(expiry))
    return transcript.digest()


class OpeningReplayGuard:
    """Single-use replay protection for one hash-commitment opening entry.

    A fresh guard has no registrations. :meth:`bind_once` registers a pending
    :class:`ReplayBinding` that commits one :class:`OpeningBatchEntry` — the
    same entry shape accepted item by item by :func:`verify_opening_batch`
    and by :func:`verify_opening` — to a session id; :meth:`check` accepts
    an equal pending binding exactly once, recomputing the binding digest,
    checking the expiry and delegating to :func:`verify_opening`, and then
    marks the id consumed. The digest frames the entry under
    ``SHA-256(F(D) || F(session_id) || L(entry) || F(E))`` with domain
    ``b"zr/orr/v1"``, where ``L(entry)`` is byte for byte the per-item outer
    leaf of a Merkle-committed opening batch (:func:`_bound_opening_leaf`,
    i.e. ``F(b"zkregion/ob/v1")`` followed by the entry's ``commitment`` /
    ``value`` / ``nonce``); the F framing and the E expiry encoding are
    reused byte for byte from the other single-entry replay guards. By
    default both the pending and the consumed state live on this guard
    instance and are never shared between instances; passing an
    :class:`SQLiteReplayStore` as ``store`` instead keeps the state in that
    store under the ``b"zr/orr/v1"`` key domain, so opening guards attached
    to the same store namespace share pending, claimed and consumed ids
    across independent instances, processes and process restarts.

    Concurrent checks of the same id are decided by an atomic claim taken
    before the digest/expiry work and the delegated opening check (the
    in-instance registry lock, or the store's short transaction and unique
    claim token); the claim is per-id registry state rather than a global
    lock, so an opening being checked under one id never serializes checks
    or binds of other ids.
    """
    ...
    def __init__(self, *, store: SQLiteReplayStore | None = None) -> None:
        if store is not None and not isinstance(store, SQLiteReplayStore):
            raise TypeError("store must be an SQLiteReplayStore")
        self._store = (
            None if store is None else store._view(_OPENING_REPLAY_DOMAIN)
        )
        self._registry = None if store is not None else _ReplayRegistry()
    ...
    def bind_once(
        ...
        *,
        ...
        Returns the frozen :class:`ReplayBinding`. ``entry`` follows the
        same nested type rules as one item of
        :func:`verify_opening_batch`: it must be an
        :class:`OpeningBatchEntry` whose ``commitment``, ``value`` and
        ``nonce`` are all ``bytes``. ``session_id`` must be non-empty
        ``bytes`` and ``expires_at`` must be either ``None`` or a
        non-``bool`` unsigned 64-bit Unix-second timestamp. A session id
        that is already pending, being checked or has been consumed raises
        :class:`ValueError`. Wrong argument or nested field types raise
        :class:`TypeError`; an empty id, an out-of-uint64 expiry, or a
        rebind in any state raise :class:`ValueError`. Inputs are never
        mutated.
        """
        _check_opening_batch_entry(entry)
        _check_bytes(session_id, "session_id")
        if not session_id:
            raise ValueError("session_id must not be empty")
        if expires_at is not None:
        ...
        binding = ReplayBinding(
            session_id,
            _opening_replay_digest(entry, session_id, expires_at),
            expires_at,
        )
        if self._store is not None:
            self._store.register(session_id, binding)
    ...
    def check(
        ...
        *,
        ...
        previously registered on this guard for ``binding.session_id``. The
        id is claimed atomically before the digest/expiry work and the
        delegated check, so among concurrent calls for the same id at most
        one can return ``True``. The digest is recomputed over the
        presented ``entry``; for a binding with an expiry,
        ``now >= expires_at`` makes the check fail (``now`` defaults to the
        current Unix seconds and must otherwise be a non-``bool`` uint64).
        Only then is the opening checked with :func:`verify_opening` called
        in argument order with ``entry.commitment``, ``entry.value`` and
        ``entry.nonce``, unchanged.

        Only a fully successful check consumes the session id; every
        rejection (unknown or consumed id, a claim lost to a concurrent
        check, unequal binding, digest mismatch, expiry, or
        :func:`verify_opening` returning ``False``) returns ``False``,
        releases the claim and leaves the registration pending, so a
        rejected id can still succeed once later. An exception escaping
        the delegated check likewise releases the claim and then
        propagates unchanged, leaving the id usable. With a store backend,
        only the holder of the current claim token can consume the id (an
        expired claim may be taken over by a later equal ``check``); a
        stale token neither consumes nor restores anything. Verification
        ...
        try:
            if not hmac.compare_digest(
                binding.digest,
                _opening_replay_digest(entry, session_id, binding.expires_at),
            ):
                return False  # the presented entry is not the one originally bound
            if binding.expires_at is not None and current >= binding.expires_at:
                return False  # expired: rejection does not consume the id
...
# ---------------------------------------------------------------------------
# Replay protection for a single Pedersen opening entry
#
# A PedersenOpeningReplayGuard binds one PedersenOpeningBatchEntry — the same
# entry shape accepted item by item by verify_pedersen_opening_batch and by
# verify_pedersen_opening — to a caller-chosen session id, reusing the
# ReplayBinding type and, byte for byte, the F framing and the E expiry
# encoding of the other single-entry guards (OpeningReplayGuard /
# RegionContainsReplayGuard); only the domain separator and the entry leaf
# differ. Like them the binding is single-use: without a store the pending
# and consumed state lives on this guard instance and is never shared between
# instances, while an SQLiteReplayStore keeps pending, claimed and consumed
# ids in the store under b"zr/porr/v1", shared across instances, processes
# and restarts. bind_once registers a pending binding, check claims the id
# atomically, recomputes the digest, checks the expiry, delegates the opening
# decision to verify_pedersen_opening and consumes the id only on full
# success; every rejection leaves the id untouched.
#
# digest = SHA-256(F(D) || F(session_id) || L(entry) || F(E))
#   D = b"zr/porr/v1"
#   F(x) = four-byte unsigned big-endian length prefix of x, followed by x
#   L(entry) = the BoundPedersenOpeningBatch outer-leaf bytes for one entry
#              (_bound_pedersen_opening_leaf), i.e. F(b"zkregion/pob/v1")
#              followed by the commitment's six integer fields and the
#              entry's value and blinding, each integer as decimal ASCII
#              with the sign kept and length-framed — the same per-item leaf
#              bytes reused raw, without an F framing
#   E = b"\x00"                          when expires_at is None
#     = b"\x01" + uint64be(expires_at)   otherwise


def _pedersen_opening_replay_digest(
    entry: PedersenOpeningBatchEntry,
    session_id: bytes,
    expires_at: int | None,
) -> bytes:
    """Compute ``SHA-256(F(D) || F(session_id) || L(entry) || F(E))``.

    Byte for byte the :func:`_opening_replay_digest` single-entry framing
    with the domain ``b"zr/porr/v1"`` and
    :func:`_bound_pedersen_opening_leaf` bytes as the raw per-entry leaf.
    """
    transcript = hashlib.sha256()
    transcript.update(_frame_length_prefixed(_PEDERSEN_OPENING_REPLAY_DOMAIN))
    transcript.update(_frame_length_prefixed(session_id))
    transcript.update(_bound_pedersen_opening_leaf(entry))  # raw leaf, no F framing
    expiry = _replay_expiry_bytes(expires_at)
    transcript.update(_frame_length_prefixed(expiry))
    return transcript.digest()


class PedersenOpeningReplayGuard:
    """Single-use replay protection for one Pedersen opening entry.

    A fresh guard has no registrations. :meth:`bind_once` registers a pending
    :class:`ReplayBinding` that commits one
    :class:`PedersenOpeningBatchEntry` — the same entry shape accepted item
    by item by :func:`verify_pedersen_opening_batch` and by
    :func:`verify_pedersen_opening` — to a session id; :meth:`check` accepts
    an equal pending binding exactly once, recomputing the binding digest,
    checking the expiry and delegating to :func:`verify_pedersen_opening`,
    and then marks the id consumed. The digest frames the entry under
    ``SHA-256(F(D) || F(session_id) || L(entry) || F(E))`` with domain
    ``b"zr/porr/v1"``, where ``L(entry)`` is byte for byte the per-item outer
    leaf of a Merkle-committed Pedersen opening batch
    (:func:`_bound_pedersen_opening_leaf`, i.e. ``F(b"zkregion/pob/v1")``
    followed by the commitment's six integer fields and the entry's
    ``value`` / ``blinding``, each integer as decimal ASCII with the sign
    kept); the F framing and the E expiry encoding are reused byte for byte
    from the other single-entry replay guards. By default both the pending
    and the consumed state live on this guard instance and are never shared
    between instances; passing an :class:`SQLiteReplayStore` as ``store``
    instead keeps the state in that store under the ``b"zr/porr/v1"`` key
    domain, so opening guards attached to the same store namespace share
    pending, claimed and consumed ids across independent instances, processes
    ...
    def __init__(self, *, store: SQLiteReplayStore | None = None) -> None:
        if store is not None and not isinstance(store, SQLiteReplayStore):
            raise TypeError("store must be an SQLiteReplayStore")
        self._store = (
            None if store is None else store._view(_PEDERSEN_OPENING_REPLAY_DOMAIN)
        )
        self._registry = None if store is not None else _ReplayRegistry()
    ...
    def bind_once(
        ...
        *,
        ...
        Returns the frozen :class:`ReplayBinding`. ``entry`` follows the
        same nested type rules as one item of
        :func:`verify_pedersen_opening_batch`: it must be a
        :class:`PedersenOpeningBatchEntry` whose ``commitment`` is a
        :class:`PedersenCommitment` with non-``bool`` integer
        ``element`` / ``lower`` / ``upper`` / ``prime`` / ``generator`` /
        ``h`` fields and whose ``value`` / ``blinding`` are non-``bool``
        integers. ``session_id`` must be non-empty ``bytes`` and
        ``expires_at`` must be either ``None`` or a non-``bool`` unsigned
        64-bit Unix-second timestamp. A session id that is already pending,
        being checked or has been consumed raises :class:`ValueError`.
        Wrong argument or nested field types raise :class:`TypeError`; an
        empty id, an out-of-uint64 expiry, or a rebind in any state raise
        :class:`ValueError`. Inputs are never mutated.
        """
        _check_pedersen_opening_batch_entry(entry)
        _check_bytes(session_id, "session_id")
        if not session_id:
            raise ValueError("session_id must not be empty")
        if expires_at is not None:
        ...
        binding = ReplayBinding(
            session_id,
            _pedersen_opening_replay_digest(entry, session_id, expires_at),
            expires_at,
        )
        if self._store is not None:
            self._store.register(session_id, binding)
    ...
    def check(
        ...
        *,
        ...
        ``now >= expires_at`` makes the check fail (``now`` defaults to the
        current Unix seconds and must otherwise be a non-``bool`` uint64).
        Only then is the opening checked with
        :func:`verify_pedersen_opening` called in argument order with
        ``entry.commitment``, ``entry.value`` and ``entry.blinding``,
        unchanged.

        Only a fully successful check consumes the session id; every
        rejection (unknown or consumed id, a claim lost to a concurrent
        check, unequal binding, digest mismatch, expiry, or
        :func:`verify_pedersen_opening` returning ``False``) returns
        ``False``, releases the claim and leaves the registration pending,
        so a rejected id can still succeed once later. An exception escaping
        the delegated check likewise releases the claim and then
        propagates unchanged, leaving the id usable. With a store backend,
        only the holder of the current claim token can consume the id (an
        expired claim may be taken over by a later equal ``check``); a
        ...
        try:
            if not hmac.compare_digest(
                binding.digest,
                _pedersen_opening_replay_digest(
                    entry, session_id, binding.expires_at
                ),
            ):
                return False  # the presented entry is not the one originally bound
...
# ---------------------------------------------------------------------------
# Replay protection for region-contains decision batches
#
...
# other batch guards; only the domain separator and the per-entry leaves
# differ. The binding is single-use: without a store the pending and consumed
# state lives on this guard instance and is never shared between instances,
# while an SQLiteReplayStore keeps pending, claimed and consumed ids in the
# store under b"zr/rcbr/v1", shared across instances, processes and restarts.
# bind_once registers a pending binding, check claims the id atomically,
...
#   S(a, f) = F(U(|a|)) || Σ F(f(a_i))
#   L(entry) = the BoundRegionContains outer-leaf bytes
#              (_bound_region_contains_leaf), raw under F
#   E is the same expiry encoding as the other guards:
#     b"\x00" when expires_at is None, b"\x01" + uint64be(expires_at) otherwise
...
def _region_contains_batch_replay_digest(
    ...
    Writes, in order: ``F(D)``, ``F(session_id)``, ``S(entries, L)`` and
    ``F(E)``; ``L(entry)`` is the existing BoundRegionContains outer-leaf
    raw bytes (:func:`_bound_region_contains_leaf`) and the entries keep
    their batch order (duplicates included).
    """
    transcript = hashlib.sha256()
...
class RegionContainsBatchReplayGuard:
    ...
    ``SHA-256(F(D) || F(session_id) || S(entries, L) || F(E))`` with domain
    ``b"zr/rcbr/v1"``, where ``L(entry)`` is the existing BoundRegionContains
    outer-leaf raw bytes (the same bytes :func:`_bound_region_contains_leaf`
    builds for :func:`verify_region_contains_bound`); the F / U / S framing
    and the E expiry encoding are reused byte for byte from the other replay
    guards. By default both the pending and the consumed state live on this
    guard instance and are never shared between instances; passing an
    :class:`SQLiteReplayStore` as ``store`` instead keeps the state in that
...
# ---------------------------------------------------------------------------
# Replay protection for Merkle-committed complete region-contains batches
#
# A BoundRegionContainsReplayGuard binds a whole BoundRegionContainsBatch
# together with the Merkle root it is claimed under to a caller-chosen
# session id, reusing the ReplayBinding type and, byte for byte, the
# F / U / S framing, the E expiry encoding and the SHA-256 formula of
# BoundRegionReplayGuard; only the domain separator and the per-entry
# leaves differ. Like the other bound guards the binding is single-use:
# without a store the state is local to the guard instance, while an
# SQLiteReplayStore keeps pending, claimed and consumed ids in the store
# under b"zr/brcbr/v1", shared across instances, processes and restarts.
# bind_once registers a pending binding, check claims the id atomically,
# recomputes the digest, checks the expiry, delegates the root and batch
# verification to verify_region_contains_bound and consumes the id only on
# full success; every rejection leaves the id untouched.
#
# digest = SHA-256(
#     F(D) || F(session_id) || F(root) || F(U(batch.leaf_count))
...
#
# The entries keep their batch order, duplicates included, and the outer
# MerkleMultiProof's leaf_count, indices and siblings are all bound verbatim.


def _bound_region_contains_replay_encodable(
    batch: BoundRegionContainsBatch,
) -> bool:
    """Every U-framed integer must fit in unsigned 64 bits.
    ...
    ``batch.leaf_count``, ``batch.proof.leaf_count`` and every proof index
    are written with ``U`` (eight-byte unsigned big-endian); a negative or
    larger-than-uint64 value cannot be framed. Each entry's integers are
...
def _bound_region_contains_replay_digest(
    ...
    """Compute the BoundRegionContains replay binding digest.

    Writes, in order: ``F(D)``, ``F(session_id)``, ``F(root)``,
    ``F(U(batch.leaf_count))``, one ``F(L(entry))`` per entry in batch
    order, then ``F(U(proof.leaf_count))``, ``S(proof.indices, U)``,
    ``S(proof.siblings, identity)`` and ``F(E)``. This is byte for byte
    the :func:`_bound_region_replay_digest` formula with the domain
    ``b"zr/brcbr/v1"`` and :func:`_bound_region_contains_leaf` bytes as
    the per-entry leaves.
    """
    transcript = hashlib.sha256()
    ...
    transcript.update(_frame_length_prefixed(root))
    transcript.update(_frame_length_prefixed(_uint64_be(batch.leaf_count)))
    for entry in batch.entries:  # entries order, each BoundRegionContains leaf under F
        transcript.update(_frame_length_prefixed(_bound_region_contains_leaf(entry)))
    proof = batch.proof
    transcript.update(_frame_length_prefixed(_uint64_be(proof.leaf_count)))
    # S(proof.indices, U) = F(U(|indices|)) || Σ F(U(index))
...
class BoundRegionContainsReplayGuard:
    ...
    bound-region-contains guards attached to the same store namespace
    share pending, claimed and consumed ids across independent instances,
    processes and process restarts.
    ...
    def bind_once(
        ...
        *,
        ...
        be either ``None`` or a non-``bool`` unsigned 64-bit Unix-second
        timestamp; every U-framed integer (the outer ``leaf_count`` /
        ``proof.leaf_count`` and the proof indices) must likewise fit in
        uint64. A session id that is already pending, being checked or
        consumed raises :class:`ValueError`. Wrong argument or nested
    ...
    def check(
        ...
        *,
        ...
        ``now >= expires_at`` makes the check fail (``now`` defaults to
        the current Unix seconds and must otherwise be a non-``bool``
        uint64). Only then is the batch/root handed to
        :func:`verify_region_contains_bound` under that function's root,
        structure and batch contract.

        Only a fully successful check consumes the session id; every
        rejection (unknown or consumed id, a claim lost to a concurrent
        check, unequal binding, digest mismatch, expiry, a non-32-byte
        root or sibling, or :func:`verify_region_contains_bound` returning
        ``False``) returns ``False``, releases the claim and leaves the
        registration pending. An exception escaping the delegated
        verification likewise releases the claim and then propagates
        unchanged, leaving the id usable. With a store backend, only the
...
# ---------------------------------------------------------------------------
# Replay protection for Merkle-committed complete wide range batches
#
# A BoundWideRangeReplayGuard binds a whole BoundWideRangeBatch together
# with the outer Merkle root it is claimed under to a caller-chosen session
# id, reusing the ReplayBinding type and, byte for byte, the F / U / S
# framing and the E expiry encoding of BoundRangeReplayGuard; only the
# domain separator and the per-entry leaves differ. Like the other bound
# guards the binding is single-use: without a store the state is local to
# the guard instance, while an SQLiteReplayStore keeps pending, claimed and
# consumed ids in the store under b"zr/bwrr/v1", shared across instances,
# processes and restarts. bind_once registers a pending binding, check
# atomically claims the id, recomputes the digest, checks the expiry,
# delegates the root verification and inner batch verification to
# verify_range_wide_batch_bound (passing the random source through
# unchanged) and consumes the id only on full success; every rejection
# leaves the id pending and reusable.
#
# digest = SHA-256(
#     F(D) || F(session_id) || F(root) || F(U(batch.leaf_count))
#     || Σ_i F(L(entry_i))
...
#   U(n) = eight-byte unsigned big-endian encoding of n
#   S(a, f) = F(U(|a|)) || Σ F(f(a_i))
#   L(entry) = the complete wide range batch outer-leaf bytes
#              (_bound_wide_range_leaf), raw under F — the existing
#              b"zkregion/wrb/v1" outer-leaf encoding
#   E is the same expiry encoding as the other guards:
#     b"\x00" when expires_at is None, b"\x01" + uint64be(expires_at) otherwise
#
# The entries keep their batch order, duplicates included; entries are
# never dropped or reordered, and the outer MerkleMultiProof's three
# fields (leaf_count, indices, siblings) are all bound verbatim.


def _bound_wide_range_replay_encodable(batch: BoundWideRangeBatch) -> bool:
    """Every U-framed integer must fit in unsigned 64 bits.

    ``batch.leaf_count``, ``batch.proof.leaf_count`` and every proof index
    are written with ``U`` (eight-byte unsigned big-endian); a negative or
    larger-than-uint64 value cannot be framed. The decimal-ASCII wide range
    leaves (the negative sign is kept), the raw sibling bytes and the root
    encode for any value.
    """
    proof = batch.proof
    values = [batch.leaf_count, proof.leaf_count, *proof.indices]
    return all(0 <= value <= _UINT64_MAX for value in values)
...
def _bound_wide_range_replay_digest(
    batch: BoundWideRangeBatch,
    root: bytes,
    session_id: bytes,
    expires_at: int | None,
) -> bytes:
    """Compute the BoundWideRange replay binding digest.

    Byte-for-byte the :func:`_bound_range_replay_digest` framing with the
    BoundWideRange replay domain and the wide range complete-batch
    (:func:`_bound_wide_range_leaf`) leaves. Writes, in order: ``F(D)``,
    ``F(session_id)``, ``F(root)``, ``F(U(batch.leaf_count))``, one
    ``F(L(entry))`` per entry in batch order, then
    ``F(U(proof.leaf_count))``, ``S(proof.indices, U)``,
    ``S(proof.siblings, identity)`` and ``F(E)``.
    """
    transcript = hashlib.sha256()
    transcript.update(_frame_length_prefixed(_BOUND_WIDE_RANGE_REPLAY_DOMAIN))
    transcript.update(_frame_length_prefixed(session_id))
    transcript.update(_frame_length_prefixed(root))
    transcript.update(_frame_length_prefixed(_uint64_be(batch.leaf_count)))
    for entry in batch.entries:  # entries order, each wide range leaf under F
        transcript.update(_frame_length_prefixed(_bound_wide_range_leaf(entry)))
    proof = batch.proof
    transcript.update(_frame_length_prefixed(_uint64_be(proof.leaf_count)))
    # S(proof.indices, U) = F(U(|indices|)) || Σ F(U(index))
    transcript.update(_frame_length_prefixed(_uint64_be(len(proof.indices))))
...
def _check_bound_wide_range_batch_types(batch: object, root: object) -> None:
    """Validate BoundWideRangeBatch argument types for the replay guard.

    Mirrors the type checks of :func:`verify_range_wide_batch_bound`: the
    batch must be a :class:`BoundWideRangeBatch` whose entries tuple holds
    nestedly well-typed :class:`WideRangeBatchEntry` objects, whose
    ``leaf_count`` is a non-``bool`` integer and whose proof is a
    well-typed :class:`MerkleMultiProof`; ``root`` must be ``bytes``.
    Structural and value problems (coverage, counts, ranges) are left to
    :func:`verify_range_wide_batch_bound` at check time.
    """
    if not isinstance(batch, BoundWideRangeBatch):
        raise TypeError("batch must be a BoundWideRangeBatch")
    _check_bytes(root, "root")
...
class BoundWideRangeReplayGuard:
    """Single-use replay protection for a Merkle-committed wide range batch.

    A fresh guard has no registrations. :meth:`bind_once` registers a
    pending :class:`ReplayBinding` that commits a whole
    :class:`BoundWideRangeBatch` together with the outer Merkle ``root``
    it is claimed under; :meth:`check` accepts an equal pending binding
    exactly once — first atomically claiming the id, then recomputing the
    binding digest, checking the expiry and handing the same random source
    unchanged to :func:`verify_range_wide_batch_bound`, which checks the
    outer root and the inner wide range batch in that fixed order — and
    then marks the id consumed. The digest reuses the
    :class:`BoundRangeReplayGuard` framing byte for byte, with only the
    domain separator (``b"zr/bwrr/v1"``) and the per-entry leaves changed
    to the wide range complete-batch leaves
    (:func:`_bound_wide_range_leaf`). By default both the pending and the
    consumed state live on this guard instance and are never shared
    between instances; passing an :class:`SQLiteReplayStore` as ``store``
    instead keeps the state in that store under the ``b"zr/bwrr/v1"``
    key domain, so bound-wide-range guards attached to the same store
    namespace share pending, claimed and consumed ids across independent
    instances, processes and process restarts.

    Concurrent checks of the same id are decided by an atomic claim taken
    before the digest/expiry work and the (potentially slow) delegated
    root and batch verification (the in-instance registry lock, or the
    store's short transaction and unique claim token); the claim is
    ...
    def __init__(self, *, store: SQLiteReplayStore | None = None) -> None:
        ...
        self._store = (
            None
            if store is None
            else store._view(_BOUND_WIDE_RANGE_REPLAY_DOMAIN)
        )
        self._registry = None if store is not None else _ReplayRegistry()
    ...
    def bind_once(
        ...
        *,
        ...
        non-empty ``bytes`` and ``expires_at`` must be either ``None`` or a
        non-``bool`` unsigned 64-bit Unix-second timestamp; the U-framed
        integers (``leaf_count`` and the outer proof indices) must likewise
        fit in uint64. A session id that is already pending, being checked
        or consumed raises :class:`ValueError`. Wrong argument or nested
        field types (including ``bool`` integers) raise
        :class:`TypeError`; an empty id, an out-of-uint64 expiry or framed
        ...
        mutated.
        """
        _check_bound_wide_range_batch_types(batch, root)
        _check_bytes(session_id, "session_id")
        if not session_id:
            raise ValueError("session_id must not be empty")
        if expires_at is not None:
            _check_uint64(expires_at, "expires_at")
        if not _bound_wide_range_replay_encodable(batch):
            raise ValueError("leaf_count and proof indices must be unsigned 64-bit integers")
        binding = ReplayBinding(
            session_id,
            _bound_wide_range_replay_digest(batch, root, session_id, expires_at),
            expires_at,
        )
        if self._store is not None:
            self._store.register(session_id, binding)
    ...
    def check(
        ...
        *,
        ...
        the current Unix seconds and must otherwise be a non-``bool``
        uint64). Only then is the batch/root handed to
        :func:`verify_range_wide_batch_bound` with ``randbelow`` passed
        through unchanged, under that function's root, structure and
        randomness contract (outer root first, then the inner wide range
        batch).

        Only a fully successful check consumes the session id; every
        rejection (unknown or consumed id, a claim lost to a concurrent
        check, unequal binding, digest mismatch, expiry, a wrong root or
        :func:`verify_range_wide_batch_bound` returning ``False`` for the
        root or the inner batch) returns ``False``, releases the claim
        and leaves the registration pending, so a rejected id can
        ...
        without any lock or transaction held, so other ids are never
        serialized. Argument type errors (including a non-callable
        ``randbelow`` and ``bool`` integers) raise :class:`TypeError`; an
        out-of-range ``now`` raises :class:`ValueError`. The random
        source's own exceptions propagate at the existing boundary.
        Inputs are never mutated.
        """
        _check_bound_wide_range_batch_types(batch, root)
        if not isinstance(binding, ReplayBinding):
        ...
        try:
            if not hmac.compare_digest(
                binding.digest,
                _bound_wide_range_replay_digest(batch, root, session_id, binding.expires_at),
            ):
                return False  # the presented batch/root is not the one originally bound
            if binding.expires_at is not None and current >= binding.expires_at:
                return False  # expired: rejection does not consume the id
            if not verify_range_wide_batch_bound(batch, root, randbelow=randbelow):
                return False
            self._registry.commit(session_id)
            committed = True
            return True
    ...
    def _check_with_store(
        ...
        taken over and issued a fresh random token); verification runs with
        no transaction held; only the token returned by the winning claim
        can consume the id, and every rejection, escaped verification
        error or database error restores the id to pending with that same
        token before propagating. A stale token (a claim taken over while
...
def _wide_range_replay_digest(
    entry: WideRangeBatchEntry,
    session_id: bytes,
    expires_at: int | None,
) -> bytes:
    """Compute ``SHA-256(F(D) || F(session_id) || L(entry) || F(E))``."""
    transcript = hashlib.sha256()
    transcript.update(len(_WIDE_RANGE_REPLAY_DOMAIN).to_bytes(4, "big"))
    transcript.update(_WIDE_RANGE_REPLAY_DOMAIN)
    transcript.update(len(session_id).to_bytes(4, "big"))
    transcript.update(session_id)
    transcript.update(_bound_wide_range_leaf(entry))  # raw leaf, no F framing
    expiry = _replay_expiry_bytes(expires_at)
    transcript.update(len(expiry).to_bytes(4, "big"))
    transcript.update(expiry)
    return transcript.digest()
...
def _check_wide_range_batch_entry(
    ...
    """Validate one WideRangeBatchEntry and its nested field types."""
    if not isinstance(entry, WideRangeBatchEntry):
    ...
    if not isinstance(proof.commitments, tuple):
        raise TypeError(
            f"{name} proof commitments must be a tuple of integers"
        )
    for item in proof.commitments:
...
class WideRangeReplayGuard:
    """Single-use replay protection for one wide range batch entry.

    A fresh guard has no registrations. :meth:`bind_once` registers a pending
    :class:`ReplayBinding` for a single :class:`WideRangeBatchEntry`;
    :meth:`check` accepts an equal pending binding exactly once and then
    marks the id consumed. By default both the pending and the consumed
    state live on this guard instance and are never shared between
    instances; passing an :class:`SQLiteReplayStore` as ``store`` instead
    keeps the state in that store under the ``b"zr/wrr/v1"`` key domain, so
    guards attached to the same store namespace share pending, claimed and
    consumed ids across independent instances, processes and restarts.
    ...
    (``b"zr/wrr/v1"``, which doubles as the store row-key domain) and in
    ``L(entry)``: the raw per-item outer leaf bytes of a complete wide
    range batch (:func:`_bound_wide_range_leaf`, the existing outer leaf
    encoding under ``b"zkregion/wrb/v1"``).

    As with :class:`RangeReplayGuard`, concurrent checks of the same id are
    decided by an atomic claim taken before the digest/expiry work and the
    potentially slow wide range verification (the in-instance registry
    lock, or the store's short transaction and unique claim token); the
    verification runs without any lock or transaction held, so different
    ...
    def __init__(self, *, store: SQLiteReplayStore | None = None) -> None:
        if store is not None and not isinstance(store, SQLiteReplayStore):
            raise TypeError("store must be an SQLiteReplayStore")
        self._store = (
            None if store is None else store._view(_WIDE_RANGE_REPLAY_DOMAIN)
        )
        self._registry = None if store is not None else _ReplayRegistry()
    ...
    def bind_once(
        ...
        *,
        ...
        that is already pending, being checked or has been consumed raises
        :class:`ValueError`; wrong argument or nested field types
        (including ``bool`` integers) raise :class:`TypeError` (an empty id
        or an out-of-range expiry raises :class:`ValueError`). Inputs are
        never mutated.
        """
        _check_wide_range_batch_entry(entry)
        _check_bytes(session_id, "session_id")
        if not session_id:
            raise ValueError("session_id must not be empty")
        if expires_at is not None:
        ...
        binding = ReplayBinding(
            session_id,
            _wide_range_replay_digest(entry, session_id, expires_at),
            expires_at,
        )
        if self._store is not None:
            self._store.register(session_id, binding)
    ...
    def check(
        ...
        *,
        ...
        previously registered on this guard for ``binding.session_id``. The
        id is claimed atomically before verification, so among concurrent
        calls for the same id at most one can return ``True``. The digest
        is then recomputed over the presented entry, the expiry is checked
        (``now >= expires_at`` makes the check fail; ``now`` defaults to
        the current Unix seconds and must otherwise be a non-``bool``
        unsigned 64-bit integer), and only then is the entry's wide range
        proof checked with :func:`verify_range_wide` called with
        ``entry.commitment``, ``entry.proof`` and ``entry.context`` in
        field order, unchanged.

        Only a fully successful check consumes the session id; every
        rejection (unknown or consumed id, a claim lost to a concurrent
        check, unequal binding, digest mismatch, expiry or a failing wide
        range proof) returns ``False``, releases any claim and leaves the
        registration pending, so a rejected id can succeed on a later
        attempt. An exception escaping the delegated verification likewise
        releases the claim and then propagates unchanged, leaving the id
        usable; a database error restores the claim before it propagates.
        With a store backend, only the holder of the current claim token
        can consume the id (an expired claim may be taken over by a later
        equal ``check``); a stale token neither consumes nor restores
        anything. Verification runs without a lock, so other ids are never
        ...
        try:
            if not hmac.compare_digest(
                binding.digest,
                _wide_range_replay_digest(entry, session_id, binding.expires_at),
            ):
                return False  # the presented entry is not the one originally bound
            if binding.expires_at is not None and current >= binding.expires_at:
                return False  # expired: rejection does not consume the id
            if not verify_range_wide(entry.commitment, entry.proof, entry.context):
                return False
            self._registry.commit(session_id)
            committed = True
            return True
    ...
    def _check_with_store(
        ...
        taken over); verification runs with no transaction held; only the
        token returned by the winning claim can consume the id, and every
        rejection or escaped error restores the id to pending with that
        same token. A stale token (a claim taken over while verification
        ran) neither consumes nor restores anything.
        """
        token = self._store.claim(session_id, binding)
        if token is None:
            return False  # unknown id, already consumed, live claim, or unequal binding
...
# A WideRangeBatchReplayGuard binds a whole non-empty batch of
# WideRangeBatchEntry items to one session id. Without a store the
# pending/claimed/consumed states are local to the guard instance, while an
# SQLiteReplayStore keeps pending, claimed and consumed ids in the store
# under b"zr/wrbr/v1", shared across instances, processes and restarts.
# bind_once registers a pending binding, check claims the id atomically,
# recomputes the digest, checks the expiry, delegates the batch verification
# to verify_range_wide_batch with randbelow passed through unchanged and
# consumes the id only on full success; every rejection leaves the id
# untouched.
#
# digest = SHA-256(
...
#   F(x) = four-byte unsigned big-endian length prefix of x, followed by x
#   U(n) = eight-byte unsigned big-endian encoding of n
#   L(e) = the existing BoundWideRange outer leaf raw bytes for e
#          (_bound_wide_range_leaf, the outer leaf encoding under
#          b"zkregion/wrb/v1")
#   S(a, f) = F(U(|a|)) || Σ F(f(a_i))
#   E is the same expiry encoding as the other guards:
#     b"\x00" when expires_at is None, b"\x01" + uint64be(expires_at) otherwise
...
def _wide_range_batch_replay_encodable(
    entries: Sequence[WideRangeBatchEntry],
) -> bool:
    """The S-framed batch length must fit in an unsigned 64-bit integer.

    The outer ``S`` sequence writes the batch count with ``U`` (eight-byte
    unsigned big-endian). Each entry is framed as the existing
    BoundWideRange outer leaf, whose integers are encoded as decimal ASCII
    with the sign kept, so every integer field frames for any value and no
    per-entry encodability rule is needed.
    """
    return 0 <= len(entries) <= _UINT64_MAX


def _wide_range_batch_replay_digest(
    entries: Sequence[WideRangeBatchEntry],
    session_id: bytes,
    expires_at: int | None,
) -> bytes:
    """Compute the wide-range-batch replay binding digest.

    Writes, in order: ``F(D)``, ``F(session_id)``, ``S(entries, L)`` and
    ``F(E)``; ``L(entry)`` is the existing BoundWideRange outer leaf raw
    bytes (the same bytes :func:`_bound_wide_range_leaf` builds for
    :func:`verify_range_wide_batch_bound`) and the entries keep their batch
    order (duplicates included).
    """
    transcript = hashlib.sha256()
    transcript.update(_frame_length_prefixed(_WIDE_RANGE_BATCH_REPLAY_DOMAIN))
    transcript.update(_frame_length_prefixed(session_id))
    # S(entries, L) = F(U(|entries|)) || Σ F(L(entry))
    transcript.update(_frame_length_prefixed(_uint64_be(len(entries))))
    for entry in entries:
...
class WideRangeBatchReplayGuard:
    """Single-use replay protection for a batch of :class:`WideRangeBatchEntry`.

    A fresh guard has no registrations. :meth:`bind_once` registers a
    pending :class:`ReplayBinding` that commits a whole non-empty batch of
    :class:`WideRangeBatchEntry` items — the same sequence shape accepted by
    :func:`verify_range_wide_batch`, kept in the given order with duplicates
    preserved — to a session id; :meth:`check` accepts an equal pending
    binding exactly once, recomputing the binding digest, checking the
    expiry and delegating to :func:`verify_range_wide_batch` with the random
    source passed through unchanged, and then marks the id consumed. The
    digest frames the batch under
    ``SHA-256(F(D) || F(session_id) || S(entries, L) || F(E))`` with domain
    ``b"zr/wrbr/v1"``, where ``L(entry)`` is the raw per-item outer leaf of
    a complete wide range batch (the same bytes
    :func:`_bound_wide_range_leaf` builds under ``b"zkregion/wrb/v1"`` for
    :func:`verify_range_wide_batch_bound`); the F / U / S framing and the E
    expiry encoding are reused byte for byte from the other batch replay
    guards. By default both the pending and the consumed state live on this
    guard instance and are never shared between instances; passing an
    :class:`SQLiteReplayStore` as ``store`` instead keeps the state in that
    store under the ``b"zr/wrbr/v1"`` key domain, so batch guards attached
    to the same store namespace share pending, claimed and consumed ids
    across independent instances, processes and process restarts.
    ...
    def __init__(self, *, store: SQLiteReplayStore | None = None) -> None:
        ...
        self._store = (
            None
            if store is None
            else store._view(_WIDE_RANGE_BATCH_REPLAY_DOMAIN)
        )
        self._registry = None if store is not None else _ReplayRegistry()
    ...
    def bind_once(
        ...
        *,
        ...
        Returns the frozen :class:`ReplayBinding`. ``entries`` must be a
        non-string, non-empty sequence of :class:`WideRangeBatchEntry`
        objects following the same sequence and nested type rules as
        :func:`verify_range_wide_batch`: every entry's
        :class:`PedersenCommitment` six integer fields, its
        :class:`WideRangeProof` tuple fields at every nesting level and its
        ``context`` are type-checked; entries are kept in the given order
        with duplicates preserved and no item dropped. ``session_id`` must
        be non-empty ``bytes`` and ``expires_at`` must be either ``None`` or
        a non-``bool`` unsigned 64-bit Unix-second timestamp; the
        ``U``-framed batch length must fit in uint64. A session id that is
        already pending, being checked or consumed raises
        :class:`ValueError`. Wrong argument or nested field types
        (including ``bool`` integers) raise :class:`TypeError`; an empty
        batch or empty id, an out-of-uint64 expiry or batch length or a
        ...
        if expires_at is not None:
            _check_uint64(expires_at, "expires_at")
        if not _wide_range_batch_replay_encodable(items):
            raise ValueError("batch length must be an unsigned 64-bit integer")
        binding = ReplayBinding(
            session_id,
            _wide_range_batch_replay_digest(items, session_id, expires_at),
            expires_at,
        )
        if self._store is not None:
            self._store.register(session_id, binding)
    ...
    def check(
        ...
        *,
        ...
        delegated verification, so among concurrent calls for the same id
        at most one can return ``True``. The digest is recomputed over the
        presented ``entries`` in their given order (duplicates included);
        for a binding with an expiry, ``now >= expires_at`` makes the check
        fail (``now`` defaults to the current Unix seconds and must
        otherwise be a non-``bool`` uint64). Only then is the batch handed
        to :func:`verify_range_wide_batch` with ``randbelow`` passed
        through unchanged, under that function's sequence, structure and
        randomness contract: it is called once per structurally valid
        wide-range OR sub-branch as ``randbelow(prime - 1)``, a
        non-callable source or a non-integer result raises
        :class:`TypeError` and a coefficient outside
        ``[0, prime - 1)`` raises :class:`ValueError`.

        Only a fully successful check consumes the session id; every
        rejection (unknown or consumed id, a claim lost to a concurrent
        check, unequal binding, digest mismatch, expiry, an empty/oversized
        batch or :func:`verify_range_wide_batch` returning ``False``)
        returns ``False``, releases the claim and leaves the registration
        pending. An exception escaping the delegated verification (such as
        the :class:`TypeError` / :class:`ValueError` raised by a bad
        ``randbelow``) likewise releases the claim and then propagates
        ...
        try:
            if not hmac.compare_digest(
                binding.digest,
                _wide_range_batch_replay_digest(items, session_id, binding.expires_at),
            ):
                return False  # the presented batch is not the one originally bound
            if binding.expires_at is not None and current >= binding.expires_at:
                return False  # expired: rejection does not consume the id
    ...
    def _check_with_store(
        ...
        try:
            if not hmac.compare_digest(
                binding.digest,
                _wide_range_batch_replay_digest(entries, session_id, binding.expires_at),
            ):
                return False  # the presented batch is not the one originally bound
            if binding.expires_at is not None and current >= binding.expires_at:
                return False  # expired: rejection does not consume the id
            if not verify_range_wide_batch(entries, randbelow=randbelow):
                return False
            if not self._store.commit(session_id, token):
                return False  # the lease expired and another check took over
            committed = True

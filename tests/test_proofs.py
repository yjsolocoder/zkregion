...
class BoundConsistencyChainReplayGuardTest(unittest.TestCase):
    ...
    def _leaves(self, count=9):
        return [f"bccbr-{i}".encode() for i in range(count)]

    def _chains(self, leaves=None):
        leaves = self._leaves(9) if leaves is None else leaves
        return [
    ...
    def test_domain_separator_is_distinct(self):
        ...
        binding = self.guard().bind_once(batch, root, b"s")
        self.assertEqual(binding.digest, self.expected_digest(batch, root, b"s"))
        # identical framing under the non-chain bound-consistency domain differs
        proof = batch.proof
    ...
    def test_digest_binds_every_component(self):
        ...
        # chain order and duplicates are part of the binding
        reordered, reordered_root = self.build([chains[1], chains[0], chains[2]])
        self.assertNotEqual(
            base.digest, self.expected_digest(reordered, reordered_root, b"s")
        )
        duplicated, duplicated_root = self.build([chains[0], chains[0], chains[2]])
        self.assertNotEqual(
            base.digest, self.expected_digest(duplicated, duplicated_root, b"s")
        )
        # proof siblings participate: a partial multi proof carries siblings
        partial = prove_multi_inclusion(cleaves, (0, 1))
        partial_batch = BoundConsistencyChainBatch(tuple(chains[:2]), 2, partial)
        self.assertNotEqual(
            base.digest, self.expected_digest(partial_batch, root, b"s")
        )
        # the proof indices participate: reordered indices are structurally
    ...
    def test_bind_nested_type_errors(self):
        ...
        proof = batch.proof
        with self.assertRaises(TypeError):
            guard.bind_once(BoundConsistencyChainBatch(list(chains), 3, proof), root, b"s")
        with self.assertRaises(TypeError):
            guard.bind_once(
                BoundConsistencyChainBatch(("x",) * 3, 3, proof), root, b"s"
            )
        with self.assertRaises(TypeError):
            guard.bind_once(BoundConsistencyChainBatch(chains, True, proof), root, b"s")
        with self.assertRaises(TypeError):
            guard.bind_once(BoundConsistencyChainBatch(chains, 3.0, proof), root, b"s")
        with self.assertRaises(TypeError):
            guard.bind_once(BoundConsistencyChainBatch(chains, 3, "proof"), root, b"s")
        with self.assertRaises(TypeError):
            guard.bind_once(
                BoundConsistencyChainBatch(
        ...
        bad_chain_cases = [
            ...
            MerkleConsistencyChain(chain.roots + (7,), chain.proofs),
            MerkleConsistencyChain(chain.roots, "p"),
            MerkleConsistencyChain(chain.roots, chain.proofs + (object(),)),
            MerkleConsistencyChain(
                chain.roots,
                (MerkleConsistencyProof(True, chain.proofs[0].new_count, chain.proofs[0].nodes),),
            ),
            MerkleConsistencyChain(
                chain.roots,
                (MerkleConsistencyProof(
                    chain.proofs[0].old_count, chain.proofs[0].new_count, (7,)
                ),),
            ),
        ]
        for bad_chain in bad_chain_cases:
            with self.assertRaises(TypeError):
                guard.bind_once(
                    BoundConsistencyChainBatch((bad_chain,) + chains[1:], 3, proof),
                    root, b"s",
                )
    ...
    def test_delegated_batch_rejection_does_not_consume(self):
        # leaves commit under the multi-inclusion root, but the first chain's
        # own consistency segment is truncated: the root passes and the chain fails
        chains = self._chains()
        segment0 = chains[0].proofs[0]
        broken_segment = MerkleConsistencyProof(
            segment0.old_count, segment0.new_count, segment0.nodes[:-1]
        )
        broken_chain = MerkleConsistencyChain(
            chains[0].roots, (broken_segment,) + chains[0].proofs[1:]
        )
    ...
    def test_oversized_framed_integer_check_returns_false(self):
        ...
        guard = self.guard()
        binding = guard.bind_once(batch, root, b"s")
        oversized = BoundConsistencyChainBatch(batch.chains, 2**64, batch.proof)
        self.assertFalse(guard.check(oversized, root, binding, now=1))
        self.assertTrue(guard.check(batch, root, binding, now=1))
    ...
    def test_store_domain_isolation_from_other_guards(self):
        ...
        store = self.make_store()
        guard = self.guard(store=store)
        bcbr_guard = BoundConsistencyReplayGuard(store=store)
        binding = guard.bind_once(batch, root, b"same-id")
        # a structurally different batch type under the neighboring b"zr/bcbr/v1"
        # domain must neither collide nor share state
        other_batch = BoundConsistencyBatch((), 0, MerkleMultiProof(0, (), ()))
        other_root = bytes(32)
        bcbr_binding = bcbr_guard.bind_once(other_batch, other_root, b"same-id")
        self.assertNotEqual(binding.digest, bcbr_binding.digest)
        self.assertTrue(guard.check(batch, root, binding, now=1))
        self.assertIn(b"same-id", bcbr_guard._pending)
        domains = {row[1] for row in self.raw_rows()}
        self.assertIn(self.DOMAIN, domains)
        self.assertIn(b"zr/bcbr/v1", domains)
        store.close()

...
# used by verify_opening_batch, i.e. the commitment / value / nonce arguments
# of verify_opening in that order — to a caller-chosen session id, reusing
# the ReplayBinding type and, byte for byte, the F framing and the E expiry
# encoding of the other single-entry guards. The binding is single-use:
# without a store the state is local to the guard instance, while an
# SQLiteReplayStore keeps pending, claimed and consumed ids in the store
# under b"zr/orr/v1", shared across instances, processes and restarts.
# bind_once registers a pending binding, check claims the id atomically,
# recomputes the digest, checks the expiry, delegates the opening check to
# verify_opening and consumes the id only on full success; every rejection
# leaves the id untouched.
#
# digest = SHA-256(
...
#   D = b"zr/orr/v1"
#   F(x) = four-byte unsigned big-endian length prefix of x, followed by x
#   L(entry) = the BoundOpeningBatch outer-leaf bytes
#              (_bound_opening_leaf), raw, i.e. without an F framing:
#              F(b"zkregion/ob/v1") || F(commitment) || F(value) || F(nonce)
#   E = b"\x00"                          when expires_at is None
#     = b"\x01" + uint64be(expires_at)   otherwise


def _check_opening_batch_entry(
    entry: object, name: str = "entry"
) -> OpeningBatchEntry:
    """Validate one :class:`OpeningBatchEntry` and its field types.

    Mirrors the per-item checks of
    :func:`_check_opening_batch_entries_types` for a single entry: the
    object must be an :class:`OpeningBatchEntry` whose ``commitment``,
    ``value`` and ``nonce`` are all ``bytes``.
    """
    if not isinstance(entry, OpeningBatchEntry):
...
def _opening_replay_digest(
    ...
    Byte for byte the single-entry guard framing with the domain
    ``b"zr/orr/v1"`` and the :class:`BoundOpeningBatch` outer-leaf bytes
    (:func:`_bound_opening_leaf`) as the raw per-entry leaf — that leaf's
    field run is itself the ``Q(entry)`` framing reused from
    :class:`OpeningBatchReplayGuard`.
    """
    transcript = hashlib.sha256()
...
class OpeningReplayGuard:
    ...
    :class:`OpeningBatchEntry` — the same entry shape accepted by
    :func:`verify_opening_batch`, whose ``commitment`` / ``value`` /
    ``nonce`` fields are exactly the arguments of :func:`verify_opening` —
    to a session id; :meth:`check` accepts an equal pending binding
    exactly once, recomputing the binding digest, checking the expiry,
    delegating to :func:`verify_opening`, and then marks the id consumed.
    The digest frames the entry under
    ``SHA-256(F(D) || F(session_id) || L(entry) || F(E))`` with domain
    ``b"zr/orr/v1"``, where ``L(entry)`` is the
    :class:`BoundOpeningBatch` outer-leaf encoding
    (:func:`_bound_opening_leaf`,
    ``F(b"zkregion/ob/v1") || F(commitment) || F(value) || F(nonce)``),
    raw under no extra framing; the F framing and the E expiry encoding
    are reused byte for byte from the other single-entry guards. By
    default both the pending and the consumed state live on this guard
    instance and are never shared between instances; passing an
    :class:`SQLiteReplayStore` as ``store`` instead keeps the state in
    that store under the ``b"zr/orr/v1"`` key domain, so opening guards
    attached to the same store namespace share pending, claimed and
    consumed ids across independent instances, processes and process
    restarts.

    Concurrent checks of the same id are decided by an atomic claim
    taken before the digest/expiry work and the delegated opening check
    (the in-instance registry lock, or the store's short transaction and
    unique claim token); the claim is per-id registry state rather than a
    global lock, so an opening being checked under one id never
    ...
    def bind_once(
        ...
        *,
        ...
        Returns the frozen :class:`ReplayBinding`. ``entry`` must be an
        :class:`OpeningBatchEntry` whose ``commitment``, ``value`` and
        ``nonce`` are all ``bytes`` — the same nested type rules as one
        item of :func:`verify_opening_batch`. ``session_id`` must be
    ...
    def check(
        ...
        *,
        ...
        previously registered on this guard for ``binding.session_id``. The
        id is claimed atomically before the digest/expiry work and the
        delegated opening check, so among concurrent calls for the same id
        at most one can return ``True``. The digest is recomputed over the
        presented ``entry``; for a binding with an expiry,
        ``now >= expires_at`` makes the check fail (``now`` defaults to the
        current Unix seconds and must otherwise be a non-``bool`` uint64).
        Only then is the entry handed unchanged to :func:`verify_opening`
        with ``entry.commitment``, ``entry.value`` and ``entry.nonce``.
        ...
        the delegated opening check likewise releases the claim and then
        propagates unchanged, leaving the id usable. With a store
        backend, only the holder of the current claim token can consume
        the id (an expired claim may be taken over by a later equal
        ``check``); a stale token neither consumes nor restores

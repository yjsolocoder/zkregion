...
# 整批 BoundMerkleInclusionBatch 与外层 bytes 根的一次性绑定（可选 SQLite 后端跨实例共享）
from zkregion import BoundMerkleInclusionBatchReplayGuard

bmibr = BoundMerkleInclusionBatchReplayGuard()
bmibr_binding = bmibr.bind_once(bound_ib, bound_ib_root, b"session-1")
assert bmibr.check(bound_ib, bound_ib_root, bmibr_binding)       # 先验外层根与整批单叶包含证明再消费
assert not bmibr.check(bound_ib, bound_ib_root, bmibr_binding)   # 二次提交被拒

store = SQLiteReplayStore(tempfile.mktemp(suffix=".db"))
bmibr_a = BoundMerkleInclusionBatchReplayGuard(store=store)
bmibr_binding = bmibr_a.bind_once(bound_ib, bound_ib_root, b"session-3")
bmibr_b = BoundMerkleInclusionBatchReplayGuard(store=store)             # 另一个独立实例
assert bmibr_b.check(bound_ib, bound_ib_root, bmibr_binding)     # 认领、验外层根与整批单叶包含证明并消费
assert not bmibr_a.check(bound_ib, bound_ib_root, bmibr_binding) # 已消费，二次提交被拒
store.close()
...
- `BoundMerkleInclusionBatchReplayGuard(*, store=None)` — Merkle 承诺单叶包含证明完整批（`BoundMerkleInclusionBatch`）与外层 bytes 根的防重放登记册（线程安全）；无参时待用/已消费状态隔离在本实例内存中，传入 `SQLiteReplayStore` 时待用/认领/已消费状态落在该存储中（行键域为 `b"zr/bmibr/v1"`），同文件同命名空间的独立实例（含重启后、跨进程）共享状态；摘要逐字节复用 `BoundConsistencyReplayGuard` 的 F/U/S/E 与 SHA-256 公式，仅域改为 `b"zr/bmibr/v1"`、逐项 `L(item)` 改为既有单叶包含 Bound 批外层叶原字节（`_bound_merkle_inclusion_leaf`，即 `F(b"zkregion/inclusion-batch/v1") || F(Q(item))`、域 `b"zkregion/inclusion-batch/v1"` 的既有外层叶编码），批次保序留重、外层 `MerkleMultiProof` 三字段均原样绑定
  - `bind_once(batch: BoundMerkleInclusionBatch, root: bytes, session_id: bytes, *, expires_at=None) -> ReplayBinding` — 只登记待用并做全量预检，返回冻结对象；`root`、`session_id` 为 bytes 且 id 非空；空 id、uint64 越界的期限或 U 成帧整数（外层 `leaf_count`、`proof.leaf_count`、各索引，以及各项叶内的 `proof.index` 与 `siblings` 序列长度）或待用/校验中（存储后端下含过期认领）/已消费 id 重绑抛 `ValueError`，错型（含 `bool` 整数）抛 `TypeError`
  - `check(batch, root, binding, *, now=None) -> bool` — 原子认领、重算绑定摘要、核期限后原样调用 `verify_inclusion_batch_bound`；仅成功消费 `session_id`，其余无效（未知/已消费 id、竞争失败、绑定不等、摘要不符、过期、错误根或验批返回 `False`）一律返回 `False` 且撤销认领、保持待用，委托验证抛出的异常先复原再原样透传，`sqlite3.Error` 原样透传；存储后端下只有持有当前认领 token 的一方能消费，拒绝或异常时以该 token 把 id 复原为待用（token 已失效则为无操作）
...
### Merkle 承诺单叶包含证明批的一次性绑定

`BoundMerkleInclusionBatchReplayGuard(*, store=None)` 为整批 :class:`BoundMerkleInclusionBatch` 连同其外层 Merkle `root` 提供一次会话绑定，复用同一个 `ReplayBinding` 类型，`bind_once` 的入参同为 `(batch, root, session_id, *, expires_at=None)`，且只登记待用并做全量预检：`batch` 为 `BoundMerkleInclusionBatch`，`root`、`session_id` 均为 `bytes`，id 必须非空。无参构造时待用/已消费状态隔离在本实例内存中，传入 `SQLiteReplayStore` 时待用/认领/已消费三态落在该存储中，同文件同命名空间的独立实例（含重启后、跨进程）共享状态。绑定摘要逐字节复用 `BoundConsistencyReplayGuard` 的 F/U/S/E 与 SHA-256 公式，批次保序留重，仅两处不同：

- 域改为 `D = b"zr/bmibr/v1"`；
- 逐项成帧的 `L(item_i)` 改为"Merkle 承诺的单叶包含证明完整批验"一节定义的单叶包含 Bound 批外层叶原字节（`_bound_merkle_inclusion_leaf`），即 `F(b"zkregion/inclusion-batch/v1") || F(Q(item))`，其中 `Q(item)` 逐字节复用 `MerkleInclusionReplayGuard` 摘要从 `F(leaf)` 到 `S(proof.siblings, id)` 的连续字节（`_merkle_inclusion_proof_framing`）；它也是 `verify_inclusion_batch_bound` 验外层根时重算的叶。批序、重复项及外层 `MerkleMultiProof` 的 `leaf_count`、`indices`、`siblings` 三字段均原样绑定。

`bind_once(batch, root, session_id, *, expires_at=None)` 登记本实例的待用绑定并返回冻结的 :class:`ReplayBinding`。类型边界与 `verify_inclusion_batch_bound` 一致（`batch` 及其嵌套 `MerkleInclusionBatchEntry` 条目、内层 `MerkleProof` 与外层 `MerkleMultiProof` 的字段类型同样预检），错型（含 `bool` 整数）抛 `TypeError`；`session_id` 为空、`expires_at` 非 uint64、任一 `U` 成帧整数（外层 `leaf_count`、`proof.leaf_count`、各索引，以及每项叶内的 `proof.index` 与 `siblings` 序列长度）为负或超出 uint64、或 id 已待用/校验中（存储后端下含过期认领）/已消费均抛 `ValueError`。内存模式下待用与已消费状态只存在于本实例、不跨实例共享；存储模式下在一个短事务中插入待用行，键已存在（待用、被认领——**即使租约已过期**——或已消费）即抛 `ValueError`。

`check(batch, root, binding, *, now=None) -> bool` 先原子认领：核对登记册中的待用绑定与提交的 `binding: ReplayBinding` 按值相等，再重算摘要确认提交的 `batch`/`root` 就是绑定时的对象，再核期限（`now` 缺省取当前 Unix 秒，显式给出时须为非 `bool` uint64，越界抛 `ValueError`），随后调用 `verify_inclusion_batch_bound(batch, root)`。只有全部成功才消费 `session_id`；未登记（含已消费）的 id、竞争失败、被替换的绑定、摘要不符（篡改）、过期、错误根、其他摘要/结构/证明无效或验批返回 `False` 一律返回 `False` 且**不消费**，撤销认领、复原为待用，因此被拒的绑定稍后仍可成功一次。委托验证逃出的异常先撤销认领再原样透传；存储后端的数据库错误原样透传 `sqlite3.Error`。内存模式下绑定状态不跨实例共享；`check` 的参数类型错误抛 `TypeError`。入口不改写任何输入。

...
- `MerkleInclusionEntryReplayGuard(*, store=None)` — 单条包含批验条目（`MerkleInclusionBatchEntry`）的防重放登记册（线程安全）；无参时待用/已消费状态隔离在本实例内存中，传入 `SQLiteReplayStore` 时待用/认领/已消费状态落在该存储中（行键域为 `b"zr/mirr/v1"`，该名称同时是摘要域与存储行键域），同文件同命名空间的独立实例（含重启后、跨进程）共享状态；摘要逐字节复用 `OpeningReplayGuard` 等单条守卫的 F/E 与 SHA-256 公式，仅域改为 `b"zr/mirr/v1"`、`L(entry)` 改为单叶包含完整批的逐项外层叶原字节（`_bound_merkle_inclusion_leaf`，即 `F(b"zkregion/inclusion-batch/v1") || F(Q(entry))`、域 `b"zkregion/inclusion-batch/v1"` 的既有外层叶编码，其中 `Q(entry)` 为 `F(leaf) || F(root) || F(U(proof.index)) || S(proof.siblings,id)`）
  - `bind_once(entry: MerkleInclusionBatchEntry, session_id: bytes, *, expires_at=None) -> ReplayBinding` — 把单条 `MerkleInclusionBatchEntry`（嵌套类型规则沿用 `verify_inclusion_batch` 的逐项检查）一次性绑定到 `session_id`；空 id、uint64 越界的期限或 U 值（`proof.index`）或待用/校验中（存储后端下含过期认领）/已消费 id 重绑抛 `ValueError`，entry、leaf/root、证明或嵌套字段错型（含 `bool` 冒充整数索引）抛唯一的 `TypeError`，输入不改写
  - `check(entry, binding, *, now=None) -> bool` — 先原子认领等值待用绑定（存储后端可在认领租约过期后接管旧认领），再重算摘要、检查期限，随后按叶、证明、根的次序原样委托 `verify_inclusion(entry.leaf, entry.proof, entry.root)`；只有包含校验通过才消费 `session_id`，未登记、已消费、绑定不等、摘要不符、过期或包含不过一律返回 `False` 且撤销认领、把标识复原待用，被拒标识稍后仍可成功一次；委托异常与数据库错误先复原认领再原样透传；存储后端下只有持有当前认领 token 的一方能消费（token 已失效则复原为无操作），验证期间不持锁/事务，不同标识不被一次耗时校验全程串行阻塞
...
`MerkleInclusionEntryReplayGuard(*, store=None)` 为单条 :class:`MerkleInclusionBatchEntry` 提供与 `OpeningReplayGuard` 同构的一次性会话绑定（形态对齐既有单条守卫），复用同一个 `ReplayBinding` 类型；`bind_once` 的入参为 `(entry, session_id, *, expires_at=None)`，`entry` 沿用 `verify_inclusion_batch` 逐项的形状与嵌套类型规则（`leaf`/`root` 均为 `bytes`、`proof` 为 `index` 是非 `bool` 整数、`siblings` 为 `bytes` 元组的 `MerkleProof` 的 `MerkleInclusionBatchEntry`，字段次序与 `verify_inclusion` 入参一致）；无参构造时待用/已消费状态隔离在本实例内存中，传入 `SQLiteReplayStore` 时待用/认领/已消费状态落在该存储中（行键域为 `b"zr/mirr/v1"`，该名称同时是摘要域与存储行键域），同文件同命名空间的独立实例（含重启后、跨进程）共享状态。绑定摘要按同一公式

```
digest = SHA-256(F(D) || F(session_id) || L(entry) || F(E))
```

计算，但域为 `D = b"zr/mirr/v1"`，`L(entry)` 是"Merkle 承诺的单叶包含证明完整批验"一节定义的逐项外层叶原字节（`_bound_merkle_inclusion_leaf`，即 `F(b"zkregion/inclusion-batch/v1") || F(Q(entry))`，其中 `Q(entry)` 逐字节复用 `MerkleInclusionReplayGuard` 摘要从 `F(leaf)` 到 `S(proof.siblings, id)` 的连续段——`F(leaf) || F(root) || F(U(proof.index)) || S(proof.siblings, id)`；**不**再套 `F`），`F`/`E` 逐字节沿用既有单条守卫。

`bind_once(entry, session_id, *, expires_at=None)` 只登记待用并返回冻结的既有 `ReplayBinding`：`entry` 或其逐层嵌套字段错型——含用 `bool` 冒充整数的 `proof.index`——一律抛唯一的 `TypeError`；`session_id` 为空、`expires_at` 或成帧计数（`proof.index`）越出 uint64、或 id 已待用/校验中（存储后端下含过期认领）/已消费（任意状态重绑）均抛 `ValueError`。

`check(entry, binding, *, now=None) -> bool` 的校验次序固定：先原子认领等值待用绑定（存储后端下认领租约过期后可由持等值绑定的检查接管并换发 token），再重算摘要确认提交的条目就是绑定时的条目，再检查期限（`now` 缺省取当前 Unix 秒，显式给出时须为非 `bool` uint64，`bool`/非整数抛 `TypeError`、越界抛 `ValueError`；有期绑定要求 `now < expires_at`），随后按叶、证明、根的次序以 `entry.leaf`、`entry.proof`、`entry.root` 原样委托 `verify_inclusion`。只有包含校验通过才消费 `session_id`；未登记、已消费、绑定不等、摘要不符、过期或包含不过一律返回 `False` 且撤销认领、把标识**复原为待用**，因此被拒的标识稍后仍可成功一次；委托校验逃出的异常与数据库错误先复原认领再原样透传（`sqlite3.Error` 不包装）。并发检查同一标识至多一次成功（首次判真、二次判假、换绑判假），且某个标识上的耗时校验不会把其他标识的登记或检查全程串行阻塞。无参构造时绑定状态不跨实例共享；`check` 的参数类型错误抛 `TypeError`，`store` 参数类型错误抛 `TypeError`。入口不改写任何输入。

存储后端下 `MerkleInclusionEntryReplayGuard` 复用 `SQLiteReplayStore` 的短事务认领、租约过期接管与 token 所有权全部机制，仅行键的域段为 `b"zr/mirr/v1"`（与同一存储上 `MerkleInclusionReplayGuard` 的 `b"zr/mir/v1"`、`MerkleInclusionBatchReplayGuard` 的 `b"zr/mibr/v1"`、`BoundMerkleInclusionBatchReplayGuard` 的 `b"zr/bmibr/v1"` 等行互不冲突）。
...
- **实例隔离或共享存储**：无参构造时，待用/校验中/已消费状态只存在于单个守卫实例内，不同实例（即便同 id）完全独立、互不阻塞；传入同一 `SQLiteReplayStore`（同文件、同命名空间）的守卫实例则经由短事务与唯一认领 token 共享三种状态，跨实例、跨进程并在重启后保持一致，认领租约过期后只能由等值 `binding` 的 `check` 接管，不同守卫域（`zr/r/v1`、`zr/sbr/v1`、`zr/skbr/v1`、`zr/rbr/v1`、`zr/rgbr/v1`、`zr/rr/v1`、`zr/rg/v1`、`zr/brg/v1`、`zr/brr/v1`、`zr/bsr/v1`、`zr/skbbr/v1`、`zr/mccr/v1`、`zr/mccbr/v1`、`zr/mcr/v1`、`zr/mir/v1`、`zr/mibr/v1`、`zr/mmr/v1`、`zr/mmb/v1`、`zr/mcbr/v1`、`zr/bccbr/v1`、`zr/bmmbr/v1`、`zr/bmibr/v1`、`zr/bobr/v1`、`zr/pbobr/v1`、`zr/rcbr/v1`、`zr/brcbr/v1`、`zr/orr/v1`、`zr/porr/v1`）的行互不冲突。
